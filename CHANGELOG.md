...
### Added
- `docs/DATA_MODEL.md` → v1.5: §25 Enterprise Tenant Offboarding & Data Egress Schema (734 lines). Closes the §16 lifecycle gap: §16.2 says "data deletion process begins per §12.3" when `tenants.lifecycle_status → 'churned'`, but §12.3 is the per-user GDPR Art. 17 individual erasure flow — not designed for employer-scale off-boarding. §25.1 design principles: privacy floor is structural (egress ENUM and worker have no code path to individual health tables); Art. 9 no-grace rule (keypoints_enc, user_health_profiles, coaching_turns, meal_logs, body_metrics hard-deleted immediately — no recovery window after employer relationship ends); two-person deletion authorisation (mirrors `nukeTenantSessions()` two-person rule in SSO_SCIM §22); billing records pseudonymized not deleted (Art. 17(3)(b) — Ukrainian Tax Code Art. 44 + EU VAT Directive 7-year mandate). §25.2 off-boarding trigger taxonomy: three triggers (`contract_expired_no_renewal`, `early_termination`, `pilot_lapsed`) with notification timing and nominal timeline table. §25.3 `tenant_offboarding_jobs` DDL: `offboarding_trigger_enum` and `offboarding_status_enum`; 9-state machine (`pending → access_revoked → export_in_progress → export_delivered → deletion_in_progress → financial_pseudonymized → attestation_issued → completed` + `failed` + `on_hold`); `chk_ofb_trigger_has_source` (pilot_lapsed requires pilot_id; others require contract_id); `chk_ofb_two_person` (authorised_by_1 ≠ authorised_by_2 — required before deletion_in_progress); per-table row count columns for attestation cross-reference. §25.4 `tenant_data_egress_packages` DDL: `egress_package_type_enum` (aggregate_wellness_report/audit_log_export/contract_documents/seat_activity_summary — exhaustive ENUM; no health-data value exists); R2 key + 30-day signed URL + SHA-256 content hash + HMAC-SHA256 package signature; download count tracking; `package_status_enum` (generating/ready/downloaded/expired/generation_failed). §25.5 `tenant_deletion_attestations` DDL: one-to-one with offboarding job (UNIQUE on job_id); `attestation_text TEXT NOT NULL` (GDPR C1.2 human-readable prose with Art. 17(3)(b) exception disclosure); `tables_deleted JSONB` (table → rows_deleted); `art17_exceptions JSONB` (table → retention_basis); SHA-256 attestation hash for tamper detection; delivery tracking; R2 archive key (7-year retention). §25.6 state machine: full transition table (8 transitions, 2 exception paths) with DEC-030 events and responsible actor per transition. §25.7 GDPR Art. 17 enterprise-scale deletion sequence: 14-row table covering all health and personal data categories — Art. 9 tables (immediate hard delete), standard personal data (30-day soft-delete then hard delete), financial records (pseudonymize, retain 7yr), audit log (retain 7yr per SOC 2 / legal discovery). §25.8 data export package specifications: per-package-type content specification; format (NDJSON per table, collected into ZIP with SHA-256 manifest); HMAC-SHA256 package signature using `OFFBOARDING_EXPORT_KEY` Workers Secret; 30-day signed URL via Cloudflare R2 pre-signed URL; privacy floor verification step before ZIP assembly (k-anonymity ≥ 15 on wellness aggregates). §25.9 eight DEC-030 HMAC-chained audit events (all 7yr retention): `enterprise.offboarding_initiated` (HIGH), `enterprise.access_revoked` (CRITICAL — SSO blocked + KV nuke), `enterprise.data_export_completed` (HIGH — includes SHA-256 per package), `enterprise.deletion_in_progress` (CRITICAL — two authoriser IDs in payload), `enterprise.art9_data_deleted` (CRITICAL — per-table counts), `enterprise.standard_data_deleted` (CRITICAL — per-table counts), `enterprise.financial_pseudonymized` (HIGH — `retention_basis: "Art17(3)(b)"`), `enterprise.deletion_attestation_issued` (CRITICAL — SHA-256 of attestation text; primary SOC 2 C1.2/P8.0 auditor evidence). §25.10 RLS policies: `form_system` full write (sole mutation path); `tenant_admin` SELECT own job status and package metadata (not package content or attestation text); `form_api` no access; `form_admin` break-glass with mandatory `incident_id`. §25.11 worker architecture (`apps/api/src/workers/offboarding/`): `orchestrator.ts` (Cloudflare Workers Cron, 15-min sweep of `pending` and `failed` jobs, state-machine advance with DEC-030 emission before each transition); `egress-package-generator.ts` (queries aggregate MVs + filtered audit log only — no SELECT grant to health tables; assembles ZIP; uploads to R2; writes `tenant_data_egress_packages` row); `deletion-worker.ts` (Art. 9 tables first, immediate hard delete via `form_system` role; standard tables 30-day soft-delete enforcement; emits per-table CRITICAL DEC-030 events); `attestation-generator.ts` (assembles attestation text from deletion counts; SHA-256 signs; writes `tenant_deletion_attestations`; delivers via Resend to tenant admin email). §25.12 SOC 2 evidence mapping: C1.2 (`enterprise.deletion_attestation_issued` CRITICAL event + `tenant_deletion_attestations` row = primary disposal evidence; evidence artefact OFB-E-001); P8.0 (`enterprise.art9_data_deleted` + `enterprise.financial_pseudonymized` = privacy-in-disposal evidence; OFB-E-002); CC6.3 (`enterprise.access_revoked` + SSO_SCIM §22 `nukeTenantSessions()` call log = access-removed-timely evidence; OFB-E-003); CC9.1 (`tenant_deletion_attestations` delivered to customer + sub-processor notification DEC-030 events = third-party risk management evidence; OFB-E-004). §25.13 implementation checklist: 12 tasks (8× P0 M4/M5, 4× P0 M5). §25.14 three open questions: OQ-OFB-01 (self-service portal vs. CSM-mediated initiation — P1, enterprise-architect + product-strategist, before enterprise GA); OQ-OFB-02 (EU-region R2 bucket for EU-domiciled tenants — P0, enterprise-architect + legal + devops-lead, before first EU tenant off-boards); OQ-OFB-03 (Art. 9 no-grace rule as absolute invariant — P0, compliance-officer + legal, before first enterprise contract signed).
