...
### Added
- `content/post-2733-identity-flexibility-data-driven-update.md` — editorial 13-хв пост «Гнучка ідентичність: як self-coached атлет переглядає власний образ у відповідь на дані, а не на зовнішній тиск». Серія 2726–2735 «Тренувальна ідентичність» · пост 8/10. Motivated reasoning (Kunda, 1990) і когнітивний дисонанс (Festinger, 1957) як механізми, що блокують оновлення ідентичності від власних даних. Три паттерни: ригідна (не оновлюється), реактивна (оновлюється від зовнішнього тиску), гнучка (оновлюється від власних даних). Ядро і периферія self-schema (Markus, 1977): ядро стабільне («я атлет, я тренуюся систематично»), периферія має бути відкритою (поточна capacity, оптимальний об'єм, готовність до розкладу). Чотири ознаки ідентичнісної жорсткості, дві пастки при оновленні (катастрофізація, реактивність на соціальний тиск), три маркери гнучкого оновлення, три практичних кроки для self-coached атлета. Структурна перевага: оновлення без соціального штрафу. Зв'язок з Victor і FORM. clinical-safety NOT_REQUIRED · sports-scientist review pending.
- `blog.html` — blog card for post-2733 (№ 2733 · Training Methodology · 13 хв · draft · 2026-06-29).

### Changed
- `STATUS.md` — Editorial series 2726–2735 updated 7/10 → 8/10; next: post-2734 (Ідентичність і тренувальний вік). Current version v9.89.0 → v9.90.0.
- `README.md` — post-2733 → draft · v9.90.0; posts 2730–2731 → draft retrofix.
...
### Added
- `docs/INCIDENT_RESPONSE.md §R-50` — R-50: Champion Login Monitor Stale (`champion_login_monitor` · job 50). Fiftieth runbook. Closes `docs/OBSERVABILITY.md §59.10` item 6 (P1/M6). Mirrors R-49 structure: trigger on AL-CHAMP-ACT-01 (no `system.champion_login_check_passed` LOW in > 49 h); severity classification (P1 base → P1 + CS page if Day-44–46 cohort active with T0-Beta gap); 5 scope queries (R-50-C1 staleness; R-50-C2 Day-44–46 active cohort ±1 day; R-50-C3 T0-Beta gap assessment via champion session count + save protocol predecessor check; R-50-C4 peer daily job H4 discriminator; R-50-C5 job registration H1 discriminator); H1–H4 root causes with resolutions; §R-50.5 escalation path (CS page for unprotected pilots below threshold); recovery steps; 3 communication templates; CHAMP-LOGIN-STALE-CHAIN-01 DEC-030 events (`system.champion_login_monitor_stale_declared` HIGH/7yr + `system.champion_login_monitor_restored` STANDARD/3yr — HTTP 422 `CHAMP_LOGIN_STALE_CHAIN_01_VIOLATION` on inversion → R-05); Zod v2 schemas (`ChampLoginStaleDeclaredSchema`, `ChampLoginRestoredSchema`); CC7.2/A1.1/CC3.2 SOC 2 auditor narratives; CHAMP-LOGIN-STALE-E-001 evidence artefact (7yr if T0-Beta gap / 3yr if no gap; `compliance/evidence/pilots/champion-login/`); 5-item implementation checklist; cross-references R-03/R-05/R-20/R-45–R-51/DEC-030. Privacy floor: both DEC-030 events carry only booleans, numeric counts, H1–H4 enum, FORM-internal `incident_id` UUID — no employee `user_id`, champion session content, tenant name, or GDPR Art. 9 data.
- `docs/INCIDENT_RESPONSE.md §R-51` — R-51: WAU Decline Monitor Stale (`wau_decline_monitor` · job 51). Fifty-first runbook. Closes `docs/OBSERVABILITY.md §60.10` item 6 (P1/M6). Key distinction from daily-job peers: weekly cadence (Monday `0 12 * * 1`, 8-day freshness) amplifies gap risk — stale window can reach 14–21 days if multiple Monday runs fail; special stale_days thresholds (> 8 confirm trigger; > 14 proactive CS note; > 21 P1 maximum urgency). 5 scope queries (R-51-C1 staleness in days; R-51-C2 mature pilots ≥ 28 days active with OQ-WAU-OBS-01 advisory minimum seat guard `contracted_seats >= 25`; R-51-C3 T0-Gamma gap — full 3-consecutive-week WAU decline SQL mirroring job 51 OBSERVABILITY §60.5 + save protocol predecessor check; R-51-C4 peer job health with days-based staleness; R-51-C5 job registration); H1–H4 root causes; §R-51.5 escalation; CHAMP-LOGIN-STALE-CHAIN-01-parallel WAU-DECLINE-STALE-CHAIN-01 events (`system.wau_decline_monitor_stale_declared` HIGH/7yr + `system.wau_decline_monitor_restored` STANDARD/3yr — `t0g_gap_found` and `t0g_gap_resolved` separate boolean fields for auditor clarity; HTTP 422 `WAU_DECLINE_STALE_CHAIN_01_VIOLATION` → R-05); Zod v2 schemas (`WauDeclineStaleDeclaredSchema`, `WauDeclineRestoredSchema`); CC7.2/A1.1/CC3.2 auditor narratives; WAU-DECLINE-STALE-E-001 evidence artefact (7yr if T0-Gamma gap / 3yr if no gap; `compliance/evidence/pilots/wau-decline/`); 5-item implementation checklist; cross-references R-03/R-05/R-20/R-45–R-50/DEC-030. Privacy floor: both events carry only booleans, numeric counts, H1–H4 enum, FORM-internal UUID — no employee `user_id`, WAU values, tenant name, or GDPR Art. 9 data.

### Changed
- `docs/OBSERVABILITY.md` — v5.6.0 → v5.7.0. §59.10 item 5 → `[x] Done` (job 50 registered §12.6 v2.5 patch). §59.10 item 6 → `[x] Done` (R-50 authored this pass). §60.10 item 5 → `[x] Done` (job 51 registered §12.6 v2.5 patch). §60.10 item 6 → `[x] Done` (R-51 authored this pass). §12.6 v2.5 patch: `champion_login_monitor` job 50 and `wau_decline_monitor` job 51 added to canonical pg_cron registry; freshness note extended to cover both; INCIDENT_RESPONSE R-50/R-51 cross-refs live.
- `VERSION` — 9.88.1 → 9.89.0.
...
### Added
- `content/post-2732-identity-and-coaching-without-losing-autonomy.md` — editorial 13-хв пост «Ідентичність і коучинг: як self-coached атлет взаємодіє з тренером, консультантом або ментором без втрати автономії». Серія 2726–2735 «Тренувальна ідентичність» · пост 7/10. Парадокс автономії: чому self-coached атлет переживає звернення до тренера як здачу (ідентичнісний капкан і його ціна). Що насправді означає «тренуватись самостійно» — «self-coached» як архітектурний контроль, а не ізоляція від інформації. Три функції зовнішньої експертизи без загрози автономії: технічний аудит (отримуєш дані, не передаєш управління), структурна консультація (затверджуєш план, а не виконуєш чужий), калібрування рішень (вирішуєш маючи більше інформації). Де self-coached атлети систематично уникають допомоги і що це коштує (рання корекція техніки, перехід між стадіями, реакція на незвичні сигнали, ретроспективний аналіз даних). Як ідентичнісна загроза спотворює оцінку якості допомоги (дискредитація, надмірна вага поганого досвіду). Різниця між «бути під тренером» і «консультуватись з тренером». Практичний фрейм: формулювати конкретне запитання, розрізняти дані і рекомендацію, не відмовлятись від зворотного зв'язку через прецедент, залишати рішення за собою явно. clinical-safety NOT_REQUIRED · sports-scientist review pending.
- `blog.html` — blog card for post-2732 (№ 2732 · Training Methodology · 13 хв · draft · 2026-06-29).

### Changed
- `STATUS.md` — Editorial series 2726–2735 updated 6/10 → 7/10; next: post-2733 (Гнучка ідентичність: як self-coached атлет переглядає власний образ у відповідь на дані).
- `README.md` — post-2732 → draft · v9.88.0 (title updated to match actual post).
- `VERSION` — 9.87.0 → 9.88.0.
...
### Added
- `content/post-2729-comparison-identity-social-positioning.md` — Editorial series 2726–2735 «Тренувальна ідентичність» пост 4/10. Соціальне порівняння (Festinger, 1954) як автоматичний когнітивний механізм в умовах відсутності об'єктивного стандарту: тренування — саме така умова. Три напрямки порівняння: висхідне (upward) — ідентичнісний дискомфорт і передбачувані реакції (необґрунтоване збільшення обсягу/інтенсивності, зміна вправ за соціальним маркером, а не програмним обґрунтуванням); спадне (downward) — короткострокове відчуття компетентності, довгостроково заморожує прогрес через самовдоволення; латеральне — найнебезпечніше, бо сприймається як найбільш «релевантне». Механізм: порівняння → ідентичнісне позиціонування → тренувальне рішення (середня ланка не усвідомлюється). Зал як середовище порівняння: соціальна ієрархія через вагу на штанзі, ефект аудиторії (Zajonc, 1965), «постійні» атлети як неофіційні референсні точки. Instagram: систематична фільтрація вгору, відсутність контексту, постійне тригерування, ілюзія доступності. Де порівняння є корисним: калібрування «нормального», технічні маяки, натхнення без ідентичнісного тиску. Self-coached атлет і структурна перевага: часткова функція зовнішнього референсу в умовах відсутності коуча — корисне порівняння vs шкідливе. Практичний тест із чотирьох питань для розрізнення аналітичного рішення і реакції на ідентичнісний дискомфорт. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2729 card added at top of feed.

### Changed
- `STATUS.md` — Editorial series 2726–2735 progress updated to 4/10; next: post-2730 (Новачок → intermediate → advanced: як перехід тренувального статусу вимагає зміни ідентичності — і чому цього не відбувається автоматично).
- `README.md` — post-2729 → draft; series 2726–2735 progress: 4/10.
- `VERSION` — 9.84.0 → 9.85.0.
...
### Added
- `content/post-2728-identity-and-break-week-crisis-restructure.md` — Editorial series 2726–2735 «Тренувальна ідентичність» пост 3/10. Фізіологічна реальність тижневої перерви: intermediate і advanced атлети не втрачають суттєвих силових адаптацій за 7 днів (Mujika & Padilla, 2000; Hortobágyi et al., 1993) — нейрональні адаптації зберігаються значно довше структурних. Ідентичнісна реакція непропорційна фізіологічному ефекту: Athletic Identity (Brewer et al., 1993) — що сильніша ідентифікація, то гостріша реакція на перерву. Чотири типи перерви і різна природа реакції: вимушена (хвороба/травма), зовнішні обставини, запланований deload, незапланована. Підлога послідовності: «не пропускай два поспіль» — операційний орієнтир, що відокремлює ідентичнісний дискомфорт від програмно значущої паузи. Дві помилки після перерви: катастрофізація і надолужування — обидві мають ідентичнісну, не програмну природу. Реструктуруючі фрейми: «тиждень відбувся без тренування» vs. «я пропустив»; «продовжую» vs. «надолужую»; перерва як регуляція vs. зрада. Коли реакція є адаптивним сигналом (повторюваний патерн, відсутність бажання). clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2728 card added at top of feed.

### Changed
- `STATUS.md` — Editorial series 2726–2735 progress updated to 3/10; next: post-2729 (Порівняння і ідентичність: де соціальне позиціонування формує тренувальні рішення без усвідомлення).
- `README.md` — post-2728 → draft; series 2726–2735 progress: 3/10.
- `VERSION` — 9.83.0 → 9.84.0.
...
### Added
- `content/post-2727-strength-athlete-identity-category-vs-analysis.md` — Editorial series 2726–2735 «Тренувальна ідентичність» пост 2/10. «Я силовий атлет» як self-schema (Markus, 1977): когнітивна структура, що прискорює обробку профільної інформації та встановлює нормативну рамку «що роблять такі люди, як я». Де категорія допомагає: фільтрація шуму, евристика при невизначеності, спеціалізація уваги. Чотири помилки, де категорія замінює аналіз: (1) відмова від гіпертрофійної роботи попри те, що CSA є субстратом силової продукції (Folland & Williams, 2007); (2) ідентичнісний замок «Я присідаю» — частота і пріоритет рухів стають образом, а не програмним рішенням; (3) поверхнева аргументація при відхиленні «некатегоріальних» методів (темпова робота, band work, mobility); (4) підвищений поріг визнання помилок блоку. Intermediate trap: зона 2–4 роки практики — ідентичність міцна, методологічна глибина ще недостатня. Практичний тест: три запитання для розрізнення аналізу і категорії. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2727 card added at top of feed.

### Changed
- `STATUS.md` — Editorial series 2726–2735 progress updated to 2/10; next: post-2728 (Ідентичність і перерва — чому пропуск тижня відчувається як криза і як реструктурувати цю логіку).
- `VERSION` — 9.82.1 → 9.83.0.
...
### Added
- `content/post-2726-training-identity-what-it-is-and-where-it-goes-wrong.md` — Editorial series 2726–2735 «Тренувальна ідентичність» post 1/10. «Тренувальна ідентичність: що це, чому вона формується — і де вона починає командувати замість тебе». Розрізнення тренувальної ідентичності («ким я є як атлет») і тренувальної цілі («що я хочу досягти»). Athletic Identity Measurement Scale (Brewer, Van Raalte & Linder, 1993): сильна ідентичність корелює зі стабільною поведінкою і гострою реакцією на загрозу образу. Три механізми формування ідентичності: звикання через повторення (розклад → «твій розклад»), прийняття ярлику (норми субкультури → внутрішні вимоги), соціальне позиціонування (відступ від позиції = зниження). Чотири операційні точки, де ідентичність заміщає аналіз: частота/об'єм (4 дні замість 5 при реальному перевантаженні — проживається як провал), деловантаження (навмисне «менше» суперечить «тренуюся важко»; Mujika & Padilla, 2003), вибір програми (атлети обирають те, що підтверджує, а не оптимальне), реакція на стагнацію (захист образу себе переважає над аналізом блоку). Структурна проблема self-coached практики: без тренера ідентичність — єдиний стабільний вхідний параметр при неоднозначних даних. Ідентичність як ресурс: стабільність на ідентичнісній основі перевищує мотиваційну (Lally et al., 2010; Duckworth et al., 2007). clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — card for post-2726 added at top of feed.

### Changed
- `README.md` — Editorial series 2726–2735 «Тренувальна ідентичність» added in progress · v9.82.0 with 10-post table; post-2726 → draft; stale series 2721–2730 marked superseded (post numbers 2721–2725 consumed by авторегуляція).
- `STATUS.md` — series 2716–2725 Next updated → «Editorial 2726–2735 «Тренувальна ідентичність»»; new row 2726–2735 added: 1/10 IN PROGRESS · post-2726 draft · v9.82.0. Current version v9.81.0 → v9.82.0.
- `VERSION` — 9.81.0 → 9.82.0.
...
### Added
- `docs/COST_MODEL.md §46` — Pilot Activation Recovery: CSM Save Protocol (OQ-21 Resolution, DEC-083). Resolves the P0 OQ-21 gap that blocked first enterprise pilot go-live (deadline: Q3 2026 — 4 days). §46.1 purpose and tier scope (Growth+ full playbook; Starter digital-only triage). §46.2 three trigger conditions: T0-Alpha (Day-14 activation rate < 30% — §21.7 primary signal, P0), T0-Beta (Day-45 admin champion logins < 2, P1), T0-Gamma (three consecutive weekly WAU-rate declines ≥ 10 pp, P1). §46.3 six-step T+0 through T+30 playbook: Step 1 protocol trigger (≤ 2 h; DEC-030 event emission; CRM Save Ticket); Step 2 diagnostic (IT admin call + HR champion call; T+3 Diagnostic Summary; aggregate-only data sources); Step 3 intervention package (six pre-approved interventions: KO re-call, Victor demo workshop, IT sprint, content re-seeding, pilot extension, bespoke — approval authority by intervention type); Step 4 weekly pulse checkpoints (T+8/T+15/T+22/T+29; `enterprise.pilot_save_protocol_updated` STANDARD/3yr emitted per checkpoint); Step 5 recovery assessment (SAVED ≥ 40% at T+14 or ≥ 50% at T+28; MANAGED-DOWN 30–39%; ABANDON < 30% with no trend); Step 6 outcome event (PILOT-SAVE-CHAIN-01 enforced). §46.4 three-tier escalation tree: L1 CSM (all deals, T+0), L2 CS Lead (ACV > $50k or T+15 if < 25%), L3 Founder (ACV > $150k or any ABANDON). §46.5 save economics: L1 COGS $750/attempt [ESTIMATE]; break-even save success rate 2.1%; ROI 9.3× at 20% save rate on Growth 300-seat ACV. §46.6 exit conditions table (SAVED/MANAGED-DOWN/ABANDON with seat-count and DPA notes). §46.7 four DEC-030 HMAC-chained events: `enterprise.pilot_save_protocol_triggered` (HIGH/7yr/CC3.2/CC7.2), `enterprise.pilot_save_protocol_updated` (STANDARD/3yr/CC7.2), `enterprise.pilot_saved` (HIGH/7yr/CC3.2/CC7.2/CC9.2), `enterprise.pilot_abandoned` (HIGH/7yr/CC3.2/CC9.2); PILOT-SAVE-CHAIN-01 ordering invariant (45-day predecessor window, HTTP 422 `PILOT_SAVE_CHAIN_01_VIOLATION`; `_updated` suppressed after outcome event). §46.8 two SOC 2 evidence artefacts: SAVE-E-001 (CC3.2/CC7.2/CC9.2, annual, 3yr — trigger + outcome export; zero-event affirmative attestation); SAVE-E-002 (CC7.2/A1.1, annual, 3yr — pg_cron job 49 run-history continuity). §46.9 twelve-item implementation checklist: 6× P0/M4 (AUDIT_LOG_SCHEMA event registration, PILOT-SAVE-CHAIN-01 Worker, Admin Console trigger button, pg_cron job 49 `pilot_activation_monitor`, SOC2_READINESS §79.4 registration, DEC-083 DECISION_LOG); 3× P1/M5 (Save Protocol Dashboard panel, AL-SAVE-01 PagerDuty routing, ENTERPRISE_ONBOARDING §3 documentation); 3× P2/M10–M24+ (actual save-rate review, SAVE-E-001 first filing, T0-Gamma false-positive assessment). §46.10 OQ-21 formal resolution. Privacy floor throughout: aggregate activation rates and pilot_id UUIDs only — no individual employee `user_id`, name, health data, session content, or GDPR Art. 9 data; `tenant_manager` (HR) excluded from all save-protocol data; HR never sees which employees are inactive. TOC extended with §45 and §46 entries. §21.7 OQ-21 forward reference patched to §46 (DEC-083). §21.10 OQ-21 status: 🟢 Resolved.

### Changed
- `docs/COST_MODEL.md` — v2.19 → v2.20.0. §46 Pilot Activation Recovery: CSM Save Protocol added. TOC extended with §45 (retroactive) and §46 entries. §21.7 "CSM save protocol trigger" note patched: "See OQ-21" → "§46 (DEC-083, v2.20.0)". §21.10 OQ-21 status updated: pending → 🟢 Resolved (§46, v2.20.0, 2026-06-27, DEC-083).
- `VERSION` — 9.79.0 → 9.80.0.
...
### Added
- `content/post-2721-intensity-autoregulation-when-plan-says-heavy.md` — Series 2716–2725 «Авторегуляція тренування» post 6/10. Авторегуляція інтенсивності: чим вона відрізняється від авторегуляції об'єму; три рівні задання інтенсивності (% від 1RM, RPE, RIR) і їх авторегуляційна інтерпретація; три сигнали першого підходу (RPE відносно плану, технічна якість кінця підходу, швидкість повторень); три рішення (А — підвищити, B — залишити, C — знизити) з конкретними умовами і межами; особливий кейс «план каже важко, тіло не відчуває опору»; три абсолютні межі авторегуляції інтенсивності; зв'язок між авторегуляцією і довгостроковою прогресією. clinical-safety: NOT_REQUIRED.
- `blog.html` — card for post-2721 added at top of feed.

### Changed
- `STATUS.md` — series 2716–2725 progress 5/10 → 6/10; next: post-2722 (Авторегуляція у багатотижневому блоці: як накопичена стомленість змінює правила гри). Total posts updated.
- `README.md` — posts 2719, 2720 → draft; post-2721 topic updated and marked draft · v9.79.0.
- `VERSION` — 9.78.0 → 9.79.0.
...
### Added
- `content/post-2719-volume-autoregulation-set-count-adjustment.md` — Series 2716–2725 «Авторегуляція тренування» post 4/10. Авторегуляція об'єму: як self-coached атлет коригує кількість підходів, не зламавши план. Рамка MEV/MV/MAV/MRV; RPE-drift як інструмент між підходами (нормальний +0.5–1, тривожний +2+); протокол рішення після останнього планового підходу (умови зупинки і умови продовження); добовий і тижневий рівні коригування; чотири типові помилки авторегуляції об'єму. clinical-safety: NOT_REQUIRED.
- `content/post-2720-between-session-autoregulation-recovery-planning.md` — Series 2716–2725 «Авторегуляція тренування» post 5/10. Авторегуляція між сесіями: як планувати наступне тренування на основі відновлення. Три паралельні процеси відновлення (м'язове, нейром'язове, системне); протокол суб'єктивної оцінки (три питання 1–5); чотири сценарії A/B/C/D з конкретними діями; HRV як другий рівень (rolling average, порогові зміни ±5%/±10%); правило гнучкого графіку; інтеграція у тижневий ритм; посилання на DEC-013 (правило «не пропускай двічі»). clinical-safety: NOT_REQUIRED.
- `blog.html` — cards for posts 2719 and 2720 added at top of feed.

### Changed
- `STATUS.md` — series 2716–2725 progress 3/10 → 5/10; next: post-2721 (Авторегуляція інтенсивності: коли план каже «важко», а тіло ні). Total posts updated.
- `VERSION` — 9.77.1 → 9.78.0.
...
### Added
- `content/post-2718-weight-selection-algorithm-in-session.md` — Series 2716–2725 «Авторегуляція тренування» post 3/10. Авторегуляція вибору ваги: рішення у залі крок за кроком. Розминка як перший тест готовності (три сигнали: швидкість, техніка, RPE); алгоритм першого робочого підходу з таблицею рішень (RIR вищий/в нормі/нижчий за плановий з відсотками коригування); три операційні моделі (фіксована вага, wave корекція, авторегуляція по сесії); дві систематичні помилки (надмірний консерватизм і агресивне підвищення); особливий кейс повернення після перерви (алгоритм двох тижнів); мінімальний протокол — чотири кроки, 30 секунд на підхід. clinical-safety: NOT_REQUIRED.
- `blog.html` — card for post-2718 added at top of feed.

### Changed
- `README.md` — editorial series 2716–2720 (proposed, superseded) replaced with 2716–2725 «Авторегуляція тренування»; posts 2716–2718 marked draft; posts 2719–2725 proposed.
- `VERSION` — 9.76.0 → 9.77.0.
...
### Added
- `content/post-2716-autoregulation-principle-adaptive-training.md` — Series 2716–2725 «Авторегуляція тренування» post 1/10. Авторегуляція як принцип адаптивного виконання для self-coached атлета. Три місця, де фіксований план ламається (нелінійність відновлення, нелінійність відповіді, психологічна ціна неуспіху); APRE-походження концепту (DeLorme–Wilson, Mann et al.); три рівні авторегуляційної рамки (каркас, діапазон, зворотний зв'язок); два типи авторегуляції (RPE/RIR vs. HRV/VBT); зв'язок із принципом MED із серії 2706–2715. clinical-safety: NOT_REQUIRED.
- `content/post-2717-rpe-rir-autoregulation-language.md` — Series 2716–2725 «Авторегуляція тренування» post 2/10. RPE і RIR як мова авторегуляції. Різниця між RPE (зусилля) і RIR (запас повторень); дослідницька база: Helms et al. 2016 (±1 повтор точність у RIR 1–4), Zourdos et al. 2016 (±5% 1RM передбачення), Gillies et al. 2021 метааналіз (систематична помилка 1.2 повтори у бік заниження); три механізми систематичної помилки; зони обмеженої валідності (нові вправи, RIR 5+, технічна vs. м'язова відмова); протокол calibration (якорний підхід, ретроактивна перевірка, лог кожного підходу). clinical-safety: NOT_REQUIRED.
- `blog.html` — cards for posts 2716–2717 added at top of feed.

### Changed
- `VERSION` — 9.75.1 → 9.76.0.
...
### Added
- `content/post-2715-minimalist-practice-synthesis.md` — Block 2706–2715 «Мінімалістична практика» пост 10/10 · SYNTHESIS. «Синтез 2706–2715: мінімалістична практика як системний вибір, а не тимчасова стратегія». Мінімалізм — не компроміс через брак ресурсів, а принцип ефективного мінімуму: найкоротший шлях до адаптації є оптимальним. П'ять вимірів мінімалізму: (1) об'єм і частота — MED, аудит «необхідне vs звичне», мінімально ефективний тиждень; (2) складність і вибір вправ — надмірна деталізація і її реальні витрати, 9 рухових патернів vs 15–22 вправи; (3) прогресія — chaos accumulation і принцип ізольованої змінної; (4) технологічний стек — одне вікно рішень, де смарт-годинник реально корисний і де шкодить; (5) рутина і стабільність — optimization trap і чотири законних тригери для зміни програми. Чотири найпоширеніші помилки self-coached атлета. Операційна рамка: до/під час/після блоку. Один аргумент: MED — не нижня межа, це ціль. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — картка для поста 2715 додана на початок стрічки.
- `README.md` — post-2715 → draft · SERIES COMPLETE; Editorial series 2751–2755 «Тренувальна стійкість і ретроспективний аналіз» proposed (5 нових тем).

### Changed
- `STATUS.md` — серія 2706–2715 прогрес 9/10 → **10/10 COMPLETE**; поточна версія v9.74.0 → v9.75.0; Next: Editorial 2716–2720 «Тренування як конкурентний вибір».
- `VERSION` — 9.74.0 → 9.75.0.
...
### Added
- `content/post-2713-minimalist-tech-stack-one-tool.md` — Block 2706–2715 «Мінімалістична практика» пост 8/10. «Мінімалістичний технологічний стек: один надійний інструмент проти повної екосистеми». Чотири функції тренувального стеку (планування, виконання, зберігання, аналіз) і чому одного інструменту достатньо. Analysis paralysis від надлишку даних. Три архетипи — фізичний щоденник, додаток, таблиця — де кожен ламається. Смарт-годинник: HRV/ЧСС/сон vs readiness score/об'єм/рекомендації. Принцип «одного вікна рішень». Аудит стеку раз на квартал. clinical-safety: NOT_REQUIRED.
- `content/post-2714-stable-routine-vs-constant-optimization.md` — Block 2706–2715 «Мінімалістична практика» пост 9/10. «Стабільна рутина проти постійної оптимізації: чому незмінна структура частіше перемагає». Optimization trap: зміна без ізоляції змінної = активність без навчання. Три переваги стабільної структури: decision fatigue, валідність вимірювання, автоматизація адгезії. Різниця між стабільною рутиною і застоєм. Чотири законних тригери зміни. Постійна оптимізація як temporal discounting. Один рік стабільної рутини vs 8–10 незавершених програм. clinical-safety: NOT_REQUIRED.
- `blog.html` — картки для постів 2713 і 2714 додані на початок фіду.

### Changed
- `STATUS.md` — Block 2706–2715 progress оновлено до 9/10; наступний: post-2715 (Синтез «Мінімалістична практика»).
- `VERSION` — 9.73.1 → 9.74.0.
...
### Added
- `content/post-2712-progression-without-chaos-accumulation.md` — Серія 2706–2715 «Мінімалістична практика» пост 7/10. Прогресія без накопичення хаосу: як додавати навантаження не додаючи складності. Різниця між принципом прогресивного перевантаження і протоколом — більшість атлетів плутає їх і замінює один протокол іншим, паралельно змінюючи ще кілька змінних. Поняття chaos accumulation: коли в один тижневий перехід змінюється вага + об'єм + темп + вправа — ізоляція сигналу стає неможливою. Ієрархія первинних змінних прогресії: інтенсивність (рівень 1) → об'єм (рівень 2) → щільність (рівень 3) → технічна складність рухового завдання (рівень 4). Чому boredom є специфічно поганим тригером для зміни програми: незалежний від адаптаційного стану, спрямований у бік ускладнення, маскує реальний стан даних. Операційний принцип: між тижнем A і тижнем B змінюється рівно одна первинна змінна. Два ілюстративні сценарії: лінійна прогресія у силовому блоці (8 тижнів, первинна змінна — вага) і хвилеподібна у гіпертрофічному (первинна змінна — підходи, вага заздалегідь визначена). Три наслідки порушення принципу: неможливість навчання (що спрацювало?), переоцінка прогресу (більше різноманітності ≠ вищий ефективний об'єм), неможливість відтворення. clinical-safety: NOT_REQUIRED · sports-scientist review pending.
- `blog.html` — картка для поста 2712 додана на початок стрічки.
- `README.md` — post-2712: proposed → draft; файл прив'язаний. Editorial series 2741–2750 «Прогресія і тренувальна логіка» запропонована (10 нових тем).

### Changed
- `STATUS.md` — серія 2706–2715 прогрес 6/10 → 7/10; поточна версія v9.72.0 → v9.73.0; next: post-2713.
- `VERSION` — 9.72.0 → 9.73.0.
...
### Added
- `content/post-2710-program-complexity-vs-effectiveness.md` — Серія 2706–2715 «Мінімалістична практика» пост 5/10. Складність програми проти ефективності: де надмірна деталізація починає шкодити. Три реальні витрати надмірної складності (когнітивне навантаження, нижча адгезія, розмитий сигнал навчання). Виправдана vs сумнівна vs шкідлива складність. Дані Krieger 2010 і Haakonssen: після контролю інтенсивності і тижневого об'єму додаткова структурна складність пояснює незначну частку варіації результатів. Два практичних тести на переускладненість. clinical-safety: NOT_REQUIRED · sports-scientist review pending.
- `content/post-2711-exercise-selection-minimalism-movement-patterns.md` — Серія 2706–2715 «Мінімалістична практика» пост 6/10. Мінімалізм у виборі вправ: скільки рухових патернів реально потрібно. 9 ключових патернів (горизонтальний/вертикальний поштовх, горизонтальна/вертикальна тяга, петля стегна, присідання, однобічні рухи нижньої частини, перенесення, кор-стабілізація). Критична різниця між варіацією (деталі всередині патерну) і диференціацією (новий функціональний патерн). Мінімально самодостатня програма: 7 вправ для покриття всіх патернів. Ефекти переходу: технічна якість, відслідковуваність прогресії, передбачуваність відновлення, стійкість при відхиленнях. clinical-safety: NOT_REQUIRED · sports-scientist review pending.
- `blog.html` — картки для постів 2710 та 2711 додані на початок стрічки.

### Changed
- `VERSION` — 9.71.1 → 9.72.0.
...
### Added
- `content/post-2709-minimum-effective-week.md` — Block 2706–2715 series 4/10. «Мінімальний ефективний тиждень: структура, що підтримує силу і гіпертрофію при стиснутому часі». Два різних питання: оптимальна частота vs мінімальна ефективна — різні відповіді, різна практична логіка. McLester et al. (2000): 1 vs 3 рази/тиждень при рівному об'ємі обидва дають значущий силовий приріст. Ralston et al. (2017): при контролі об'єму частота 1–3 рази не дає значущої різниці в силових результатах. Heaselgrave et al. (2019): 6 підходів/тиждень виробляють гіпертрофічну відповідь. Androulakis-Korakakis et al. (2020): мінімальна ефективна доза для тренованих атлетів. Різниця між підтримним мінімумом і мінімальним ефективним: напрямок адаптації. Дві конфігурації стиснутого тижня: full-body ×2 або три короткі сесії. Ієрархія що захищати першим: інтенсивність → частота патерну → специфічність. Чотири типових помилки при стиснутому тижні. Як мати готовий мінімальний шаблон заздалегідь. clinical-safety NOT_REQUIRED.
- `blog.html` — post-2709 card added at top of feed.

### Changed
- `README.md` — post-2709: proposed → draft; file linked.
- `STATUS.md` — серія 2706–2715 прогрес 3/10 → 4/10; next: post-2710.
- `VERSION` — 9.70.0 → 9.71.0.
...
### Added
- `content/post-2707-necessary-vs-habitual-weekly-training-audit.md` — Block 2706–2715 series 2/10. «Як self-coached атлет відрізняє «необхідне» від «звичного»: аудит тренувального тижня». Два статуси тренувальних елементів: необхідний (конкретна адаптаційна ціль) vs звичний (є тому що завжди був). Чотири механізми маскування: відчуття завершеності, ідентитет вправи, інерція шаблону, сліпота до дублювання. Чотири питання аудиту: що адаптується? що без цього на 4 тижні? чи є дублювання? чи додав би ти це з нуля? Типові знахідки: розпухла розминка, надмірна ізоляція, «здоров'яний» блок без мети, кардіо як ритуал. Ідентитетна пастка. Покроковий процес аудиту без руйнування програми. clinical-safety NOT_REQUIRED.
- `content/post-2708-volume-reduction-without-detraining.md` — Block 2706–2715 series 3/10. «Скорочення без деградації: як зменшити обсяг і зберегти адаптацію». Зменшення обсягу і детренованість — різні явища. Bickel et al. (2011): 1/3 від початкового об'єму зберігає силу і гіпертрофію на 8 тижнях при збереженій інтенсивності. Ogasawara et al. (2011): адаптації після детренованості відновлюються швидше, ніж будувались. Різниця між обсягом для побудови і підтримки. Операційна логіка підтримного блоку: зберігати інтенсивність, знижувати підходи, не збивати частоту. Стратегічні контексти: між блоками, стрес, стиснутий час, запланований деакумуляційний тиждень. Stults-Kolehmainen & Sinha (2014). Що відстежувати. Психологічна перешкода «менше = поразка». clinical-safety NOT_REQUIRED.
- `blog.html` — posts 2707–2708 added at top of feed.

### Changed
- `README.md` — posts 2707, 2708: proposed → draft; files linked.
- `STATUS.md` — серія 2706–2715 прогрес 1/10 → 3/10; next: post-2709.
- `VERSION` — 9.68.0 → 9.70.0.
...
### Added
- `content/post-2706-minimum-effective-dose-training.md` — Block 2706–2715 series 1/10. «Мінімально ефективна доза у тренуванні: що наука каже про нижню межу стимулу». Механосенсорна відповідь і mTORC1 як фізіологічна основа нижньої межі стимулу. Androulakis-Korakakis et al. (2020): мінімальна ефективна доза для тренованих атлетів — 1–5 важких підходів дають вимірюваний силовий прогрес. Schoenfeld et al. (2017): дозо-відповідна крива для гіпертрофії, нелінійна і несиметрична. Krieger (2010): один підхід кращий за нуль. Heaselgrave et al. (2019): 6 підходів/тиж ще виробляють гіпертрофічну відповідь. MEV vs. підтримний об'єм — різниця між прогресом і збереженням. Залежність MEV від тренувального віку, м'язової групи і стану блоку. Таблиця MEV/MRV по групах (RP-орієнтири). clinical-safety PASS.
- `blog.html` — post-2706 added at top of feed.
- `README.md` — Editorial series 2731–2740 «Тренування в умовах невизначеності» proposed (posts 2731–2740). post-2706: proposed → draft.

### Changed
- `STATUS.md` — новий рядок 2706–2715: 1/10 IN PROGRESS; версія v9.67.0 → v9.68.0.
- `VERSION` — 9.67.0 → 9.68.0.
...
### Added
- `content/post-2701-accumulated-overload-early-signals.md` — Block 2696–2705 series 6/10. «Як тіло сигналізує про накопичену перевантагу: ранні ознаки, які зникають, якщо їх не читати». П'ять ранніх суб'єктивних сигналів накопиченої функціональної перевантаги (НФП), що передують об'єктивним даним на 5–14 днів: мотиваційний зсув, зміна якості сну до падіння HRV, розминочна діагностика, емоційна реактивність, зміна апетиту. Meeusen et al. 2013 (ECSS/ACSM Overtraining Consensus Statement). Адаптаційна сліпота як механізм зникнення сигналів. Розрізнення ФП/НФП/синдрому перетренованості. clinical-safety CONDITIONAL PASS — §«Коли зупинитись» без скорочень при публікації.
- `content/post-2702-breath-as-data-training-signals.md` — Block 2696–2705 series 7/10. «Breath as data: що зміна дихального патерну говорить про стан під час підходу». Чотири дихальні сигнали всередині підходу: мимовільний звук на верхній точці, скорочення міжповторних пауз, ротове дихання у «помірних» підходах, рання затримка дихання. Маневр Вальсальви і внутрішньочеревний тиск: Harman et al. 1988, Brown & McGill 2017. Між підходами: частота дихання як маркер відновлення. Парадоксальне дихання як стоп-сигнал. Протокол розвитку дихальної уваги. clinical-safety NOT_REQUIRED.
- `content/post-2703-chronic-stress-subjective-readiness.md` — Block 2696–2705 series 8/10. «Хронічний стрес і суб'єктивна готовність: чому тіло може «брехати» системно». Два типи систематичної похибки при алостатичному дрейфі: переоцінка готовності (нова «норма» при зниженій ємності) і недооцінка (сенсибілізація). McEwen 1998/2000 — алостатичне навантаження. Stults-Kolehmainen & Sinha 2014. RPE, RIR, мотиваційний сигнал як ненадійні при дрейфі базової лінії. Рекалібрація через лог. clinical-safety CONDITIONAL PASS — §«Де межа» без скорочень при публікації.
- `content/post-2704-subjective-vs-objective-when-to-trust.md` — Block 2696–2705 series 9/10. «Коли суб'єктивна оцінка надійніша за об'єктивну — і коли навпаки». Де суб'єктивна перевершує: Claudino et al. 2017 (DALDA/RESTQ-Sport ≥ HRV чутливість до НФП), внутрішньосесійна втома, технічна якість, зовнішнє навантаження, ранні ознаки травми. Де об'єктивна надійніша: хронічна акумуляція, якість сну (Arnal et al. 2015), HRV при ≥28-денній базовій лінії (Buchheit 2014), довгострокові тренди. Дерево рішень. Калібрувальний розрив 6–12 місяців. clinical-safety NOT_REQUIRED.
- `blog.html` — posts 2701–2704 added at top of feed.

### Changed
- `README.md` — posts 2701–2704: proposed → draft; files and clinical-safety status noted.
- `VERSION` — 9.65.1 → 9.66.0.
...
### Added
- `content/post-2700-subjective-recovery-three-questions.md` — Editorial series 2696–2705 «Тіло як інструмент вимірювання» (пост 5/10). Суб'єктивна оцінка відновлення: три питання, що дають більше ніж Readiness Score. Де Readiness Score системно сліпий: стрес без тренувального генезу (HRV реагує на вегетативний дисбаланс будь-якого походження), сенсорні похибки, ефект відскоку після накопиченого дефіциту, динаміка протягом дня. Три питання: Q1 — нейромоторна і мотиваційна готовність («чи хотів би я зараз важкий підхід?»; Marcora 2008 і Boksem & Tops 2008 про зв'язок ЦНС-перевантаги і мотиваційного виходу); Q2 — пропріоцептивний зонд (перший розминальний сет як найчистіший знімок стану до накопичення психомоторної інерції; Enoka 2015); Q3 — контекстні змінні («але» — інформація, яку датчик принципово не може вловити). Дані про надійність суб'єктивної оцінки: Claudino et al. (2017) і Saw et al. (2016) — суб'єктивні маркери виявляють ознаки перетренованості на 1–2 дні раніше за HRV. Інтерпретаційна матриця «Readiness Score × Три питання» з п'ятьма кейсами. Де система дає збій: зміщена базова лінія при хронічному дефіциті, мотиваційна інерція, брак пропріоцептивного досвіду у початківців. Як Victor використовує суб'єктивну оцінку відновлення без обов'язкової анкетизації. clinical-safety NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `README.md` — Editorial series 2721–2730 «Тренувальна ідентичність: як self-coached атлет думає про себе як про атлета — і чому це має значення» proposed · v9.65.0 (posts 2721–2730, 10 topics).

### Changed
- `blog.html` — post-2700 card added at top of feed.
- `STATUS.md` — Block 2696–2705 progress 4/10 → 5/10; next: post-2701. Version updated to v9.65.0.
- `README.md` — post-2700 → draft · v9.65.0.
- `VERSION` — 9.64.0 → 9.65.0.
...
### Added
- `content/post-2699-reading-muscle-soreness.md` — Editorial series 2696–2705 «Тіло як інструмент вимірювання» (пост 4/10). Як читати м'язову болісність: що DOMS реально вимірює і де атлети систематично читають його неправильно. Фізіологія DOMS: саркомерний стрес і запальна відповідь — чому «молочна кислота» є відкинутим міфом. Три речі, які болісність не вимірює: гіпертрофічний стимул (Schoenfeld & Contreras 2013), рівень нейром'язового відновлення (Paulsen et al. 2012 — DOMS зникає раніше за силовий дефіцит), якість тренування. Repeated bout effect: чому адаптований атлет майже не відчуває DOMS — і чому це норма, а не тривожний сигнал. Що DOMS реально сигналізує: новизна руху/діапазону, ексцентричний акцент, стрибок обсягу (Nosaka & Clarkson 1995). Три помилки використання болісності як метрики: «більше болить = кращий стимул», «немає болісності = даремно», «болісність зникла = можна накривати». Обов'язкове клінічне розрізнення DOMS (дифузний, м'язовий, тупий, двобічний, пік 24–72 год) від гострого/суглобового/наростаючого болю (сигнал зупинитись). Як Victor враховує болісність у лозі без перетворення її на тверде обмеження. clinical-safety: CONDITIONAL PASS (§«Коли болісність — не болісність» є обов'язковою частиною публікації). sports-scientist review pending. Blog card added.

### Changed
- `blog.html` — post-2699 card added at top of feed.
- `STATUS.md` — Block 2696–2705 progress 3/10 → 4/10; next: post-2700 (Суб'єктивна оцінка відновлення: три питання, що дають більше ніж Readiness Score). Version updated to v9.64.0.
- `README.md` — post-2699 → draft · v9.64.0.
- `VERSION` — 9.63.1 → 9.64.0.
...
### Added
- `content/post-2698-proprioceptive-accuracy.md` — Editorial series 2696–2705 «Тіло як інструмент вимірювання» (пост 3/10). Пропріоцептивна точність: чому «відчуття техніки» і технічна якість не одне і те саме. Фізіологічна база: м'язові веретена, органи Гольджі, суглобові рецептори — «внутрішня модель тіла» (Proske & Gandevia 2012). Три критичні зони ненадійного «відчуття техніки»: новий рух без референсів (Newell 1985), накопичена втома (Hurley 1998: −15–25% точності відтворення суглобового кута), звичний компенсаційний патерн. Парадокс досвіду: expert blindspot і автоматизація руху (Ericsson 1996). Як будувати технічний контроль без постійного тренера: відео, зовнішні маркери, калібрувальна ізольована практика. Зовнішній зворотний зв'язок vs. автономна практика (Dieën 2017). Як Victor відстежує технічні відмови в лозі і рекомендує відеосесії при прогресії навантаження. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.

### Changed
- `blog.html` — post-2698 card added at top of feed.
- `STATUS.md` — Block 2696–2705 progress updated to 3/10; next: post-2699 (Як читати м'язову болісність).
- `README.md` — post-2698 → draft · v9.63.0.
- `VERSION` — 9.62.1 → 9.63.0.
...
### Added
- `content/post-2697-subjective-readiness-assessment.md` — Editorial series 2696–2705 «Тіло як інструмент вимірювання» (пост 2/10). Суб'єктивна оцінка готовності до тренування: що вона реально вимірює і де помиляється. Фізіологічна основа «готовності» — чотири системи: нейром'язова, метаболічна, ЦНС, психологічна. Дослідницька база (Saw, Main & Gastin 2016 — систематичний огляд 56 досліджень: суб'єктивні маркери r = −0.60–0.85 vs. навантаження; Hooper & Mackinnon 1995 — ранні ознаки перетренованості через wellbeing-анкети; Laurent 2011 — суб'єктивна якість сну vs. HRV у хокеїстів). Де точна: накопичена втома, якість сну, алостатичне навантаження. Де помиляється: «фальшива свіжість» після deload, ранкова оцінка вечірніх тренувань (Buchheit 2014), мотиваційна vs. фізіологічна готовність, контекстний ефект очікувань, хронічний стрес. Практичний алгоритм 4 питань. Як Victor відстежує тренди, а не разові оцінки. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.

### Changed
- `blog.html` — post-2697 card added at top of feed.
- `STATUS.md` — Block 2696–2705 progress updated to 2/10; next: post-2698 (Пропріоцептивна точність).
- `README.md` — post-2697 → draft · v9.62.0.
- `VERSION` — 9.61.0 → 9.62.0.
...
### Added
- `content/post-2696-rpe-calibration-what-8-means.md` — Editorial series 2696–2705 «Тіло як інструмент вимірювання» (пост 1/10). RPE калібрування: чому «8 з 10» для початківця і досвідченого атлета — різні числа. Фізіологія суб'єктивної оцінки зусилля (Hackett 2012, Zourdos 2016); систематичне переоцінювання новачками близькості до відмови; нейром'язові механізми RPE-точності; дрейф шкали при зміні контексту (нова вправа, після деload, монотонний блок); 4-крокова методика калібрування; RIR vs. RPE — операційна різниця; як Victor відстежує відхилення від власного базового рівня атлета замість абсолютного числа. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.

### Changed
- `README.md` — post-2696 → `draft · v9.61.0`; Editorial series 2716–2720 (Тренування як конкурентний вибір: де self-coached атлет систематично обирає неоптимально) proposed · v9.61.0.
- `STATUS.md` — v9.60.0 → v9.61.0; 2696–2705 row added (series started 1/10); roadmap: series 2716–2720 added.
- `VERSION` — 9.60.0 → 9.61.0.
...
### Added
- `content/post-2695-tech-literacy-synthesis-ten-principles.md` — **EDITORIAL SERIES 2686–2695 COMPLETE 10/10.** Synthesis: десять операційних принципів технологічної грамотності self-coached атлета: (1) технологія — мультиплікатор, не коректор; (2) три рівні персоналізації — комбінаторний/адаптивний/контекстуальний (третій недоступний); (3) дані без моделі — шум; (4) AI корисний у структурі, не в судженні; (5) алгоритм бачить патерн, не причину; (6) вибір інструменту — вибір видимості; (7) залежність vs. залученість (тест — тренуватись 4 тижні без інструменту); (8) майбутнє AI — масштабовані завдання, не контекстуальне судження; (9) відеоаналіз з умовами використання; (10) технологічна грамотність — знати, що потрібно. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `content/post-2671-deload-as-architectural-element.md` — Серія 2666–2675 (пост 6/10). Деload як архітектурний елемент: не відпочинок, а системна вимога. Механізм суперкомпенсації; диференційовані часові рамки відновлення (нейром'язове 3–5 днів / структурне 5–10 днів / ЦНС — найповільніше). Різниця між деload і «легким тижнем»: знижуй об'єм 40–60%, не інтенсивність. Два режими: плановий (кожні 3–4 тижні) та авторегульований (6 операційних сигналів). Три помилки. Архітектурна перспектива: деload — точка суперкомпенсації, навколо якої будується мезоцикл. Четвертий важіль управління навантаженням (доповнює пост-2670). clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — cards for posts 2695 (series COMPLETE banner) and 2671 added at correct positions.

### Changed
- `README.md` — post-2695 → `draft · v9.60.0`; post-2671 → `draft · v9.60.0`.
- `STATUS.md` — v9.58.0 → v9.60.0; 2686–2695 row: 8/10 → **10/10 COMPLETE**; 2666–2675 row: 5/10 → 6/10.
- `VERSION` — 9.59.0 → 9.60.0.
...
### Added
- `content/post-2694-ai-coaching-future-reality-vs-marketing.md` — Series 2686–2695 «Технологічна грамотність self-coached атлета» post 9/10. «Майбутнє AI-коучингу: що вже реально, що залишається маркетингом». Чесна карта: що AI-тренери реально вміють у 2026 (прогресія на основі журналу, адаптація до пропущених сесій, шаблони з персоналізацією, LLM-чат); три структурні обмеження, що залишаються незалежно від покоління технології (контекст за межами зафіксованих даних, відповідальність і довіра, корекція в реальному часі); маркетинговий словник розшифрований («персоналізований план», «адаптується в реальному часі», «AI-коучинг», «як наявність особистого тренера»). Реальний прогрес 2023–2026: LLM-якість відповідей, комп'ютерний зір для аналізу техніки, ширша інтеграція даних. Реалістичний горизонт 1–2 роки (відеоаналіз у реальному часі як стандарт) і 3–5 років (мультимодальна система — але структурні обмеження залишаються). Для кого підходить (базова компетентність, потреба у структурі, логістичне/фінансове обмеження) vs. для кого недостатній (старт з нуля, больові паттерни, потреба в людській відповідальності). Практично: інструмент структурування, а не замінник думки. clinical_safety_review: NOT_REQUIRED.
- `blog.html` — post-2694 card added at top of feed.

### Changed
- `README.md` — post-2694 status: proposed → draft · v9.58.0.
- `STATUS.md` — Series 2686–2695 progress updated 8/10 → 9/10. Current version v9.57.0 → v9.58.0.
- `VERSION` — 9.57.0 → 9.58.0.
...
### Added
- `content/post-2693-technology-dependency-training.md` — Series 2686–2695 «Технологічна грамотність self-coached атлета» post 8/10. «Технологічна залежність у тренуванні: де вона корисна, де стає перешкодою». Три типи залежності: від даних (журнал без читання), від зовнішньої валідації (неможливість приймати рішення без системи), від оптимізації (управління стеком > саме тренування). Чотири діагностичні питання. Три шари технологічного стеку (шар 1: тільки журнал — необхідний мінімум; шар 2: відео, сон, таймер, e1RM; шар 3: wearables/AI/VBT). Тест: 4 тижні з шаром 1. Ключова межа: корисна залежність підвищує агентність, проблемна — знижує. clinical_safety_review: NOT_REQUIRED.
- `blog.html` — post-2693 card added at top of feed.
- `README.md` — Editorial series 2706–2715 «Мінімалістична практика: принцип ефективного мінімуму для self-coached атлета» proposed · v9.56.0.

### Changed
- `README.md` — post-2693 status: proposed → draft · v9.56.0.
- `STATUS.md` — Series 2686–2695 progress updated 7/10 → 8/10. Current version v9.55.0 → v9.56.0.
...
### Added
- `content/post-2692-when-algorithm-fails-autoregulation-logic.md` — Series 2686–2695 «Технологічна грамотність self-coached атлета» post 7/10. «Коли алгоритм помиляється: де autoregulation-логіка не спрацьовує на живих даних». Шість структурних збоїв: (1) холодний старт — базова лінія калібрується на стресовому стані; (2) неможливість розрізнити причину низького HRV (недовідновлення vs. зовнішній фактор vs. артефакт); (3) протиріччя з плановою фазою накопичення — functional overreaching виглядає як поганий сигнал; (4) зовнішній контекст без кодування — хронічний стрес ≠ гострий; (5) застаріла базова лінія при зростанні тренованості; (6) ефект очікування — система частково вимірює наслідки власних рекомендацій. Умови надійної рекомендації vs. умови що потребують власної поправки атлета. clinical_safety_review: NOT_REQUIRED.
- `blog.html` — post-2692 card added at top of feed.
- `docs/OBSERVABILITY.md §57` — Amendment Rate Change Compliance Observability (v5.4.0). Closes the amendment rate compliance monitoring gap between annual ADM-E-001/ADM-E-002 filings (SOC2_READINESS §118): new pg_cron job 48 `amendment_rate_compliance_monitor` (`0 7 1 * *`; monthly; 33-day freshness) with sweep 1 TU-CHAIN-01 chain integrity check (`billing.rate_updated` → `enterprise.contract_amended` pairing per COST_MODEL §45.5) and sweep 2 price floor forensic verification (`enterprise_contracts.rate_per_seat_usd ≥ §31.5 floor`). Three new DEC-030 events: `security.amendment_chain_gap_detected` CRITICAL/7yr, `security.amendment_floor_breach_detected` CRITICAL/7yr, `system.amendment_rate_check_passed` LOW/1yr (AMEND-MONITOR-CHAIN-01 ordering invariant: all-clear suppressed when CRITICAL emitted). Three alert rules: AL-AMEND-01 (P0; PagerDuty `form-security` + `form-compliance`), AL-AMEND-02 (P0), AL-AMEND-03 (P1 stale; PagerDuty `form-devops`). Three SLOs: AMEND-SLO-01/02/03. Evidence artefact AMEND-OBS-E-001 (CC5.2/CC6.1/A1.1, annual, 3yr; SOC2_READINESS §124 registration pending). Unknown-tier NULL bypass protection. Metabase dashboard spec (6 panels; compliance_officer + form_admin only). §6.2 `amendment_rate_health` subsection inserted. §12.6 job 48 registered (v2.3 patch). Privacy floor throughout: aggregate counts or FORM-internal UUIDs only — no employee user_id, name, email, health value, rate value, or GDPR Art. 9 data.

### Changed
- `README.md` — post-2692 status: proposed → draft · v9.55.0.
- `STATUS.md` — Current version v9.54.0 → v9.55.0.
- `docs/OBSERVABILITY.md` — v5.3.1 → v5.4.0. §6.2 `amendment_rate_health` subsection added after `scim_provisioning_compliance` and before `sca_vulnerability_monitoring`. §12.6 job 48 row added; freshness window note extended for job 48; v2.3 patch note added. TOC §57 entry added.
...
### Added
- `content/post-2688-video-analysis-vs-coach-eye.md` — Series 2686–2695 cluster «Технологічна грамотність self-coached атлета» post 3/10. «Відеоаналіз проти тренерського ока: чого не бачить камера, чого не бачить тренер». Editorial-brutalist 13-хв формат. Що бачить камера: кінематика — кути суглобів, симетрія (фронтальна площина), хронологія руху, технічний дрейф між підходами; Zhou et al. (2021) JSCR — сагітальний кут зіставний із тренерською оцінкою за тими самими кінематичними критеріями. Що камера не бачить: пропріоцептивний стан і якість м'язового скорочення (потрібна ЕМГ), рівень і розподіл втоми, біль і дискомфорт, намір і увагу (Bernstein 1967 — рух треба описувати через завдання, не тільки через кінематику), контекст тренування. Що бачить тренер і не бачить камера: рух у трьох площинах одночасно; динаміка між підходами без відеоряду; стан між підходами — дихання, впевненість; живий зворотний зв'язок у реальному часі (Schmidt & Lee 2014 — immediacy ефективніший для моторного навчання). Що не бачить тренер: лонгітюдний відеозапис для прямого порівняння; slow-motion фази за 50–80 мс; власні очікувальні упередження. Спільна сліпа зона: перенесення техніки при стомленні — обидва найкраще бачать «свіжі» підходи, а не 6-й при субмаксимальних вагах. Практичний протокол для self-coached атлета: сагітальний + фронтальний кут; штатив, не ручне утримання; знімати перший і останній підходи блоку (дельта між ними — діагностика стійкості техніки до втоми); 2–3 конкретних критерії, не «загальне враження»; однакові умови між сесіями для валідного порівняння; відео показує кінематику — симптоматику не пояснює. clinical-safety: NOT_REQUIRED.
- `blog.html` — post-2688 card added at top of feed.

### Changed
- `README.md` — Series 2686–2695: post-2688 → draft · v9.50.0; Editorial series 2696–2705 (Тіло як інструмент вимірювання: суб'єктивні сигнали без приладів) proposed · v9.50.0; 10 нових тем додано до roadmap.
- `STATUS.md` — Series 2686–2695 progress 2/10 → 3/10; version v9.49.0 → v9.50.0.
- `VERSION` — 9.49.0 → 9.50.0.
...
### Added
- `content/post-2687-wearable-data-without-self-deception.md` — Series 2686–2695 cluster «Технологічна грамотність self-coached атлета» post 2/10. «Як читати дані wearable без самообману: що реально говорять Garmin, Whoop, Oura». Editorial-brutalist 13-хв формат. Фізичний базис вимірювань: оптичний PPG, акселерометр, температурний сенсор — всі «scores» є алгоритмічною інтерпретацією, не прямим вимірюванням. HRV: де надійна (тижневий rolling trend, персональний baseline, гострі аномалії як ранній сигнал хвороби), де ненадійна (одноденний дроп, міжособистісне порівняння, вимірювання під час активності). Фази сну: Chinoy et al. (2021) *Sleep* — ~90% точність «сон vs. неспання», але 30–40% false positive/negative для N1/REM; тотальна тривалість надійніша за розбивку фаз. Garmin: Body Battery і Training Status — VO2max estimate як розрахункова, а не виміряна величина; що реально корисно (тижневий Training Load тренд). Whoop: Strain Score надійний для кардіо-навантаження, але сліпий до механічного; Recovery Score research-backed, але без м'язової готовності. Oura: Readiness Score з найскладнішим consumer-алгоритмом; RHR і базальна температура — найнадійніші довгострокові маркери. П'ять пасток самообману: (1) підтверджувальне упередження — дані підтверджують вже прийняте рішення; (2) score-чейсінг — оптимізація Readiness ≠ оптимізація тренованості; (3) readiness paralysis — очікування «зеленого дня» при структурно жовтому стані; (4) довіра до числа більше ніж до тіла; (5) ігнорування контексту вимірювання (алкоголь, переліт, спека). Практичний протокол: тренд замість точки (7–14 day rolling); суб'єктивна оцінка до відкриття додатку — первинний сигнал; одна метрика замість дашборду; структурні зміни vs. добовий шум. Ієрархія сигналів: RPE після розмінки → HRV тижневий тренд → тижневе навантаження → одноденний score. Позиція FORM: wearable-дані — один шар контексту, не директива; Victor пояснює механізм рекомендації, а не просто видає колір. clinical-safety: NOT_REQUIRED.
- `blog.html` — post-2687 card added at top of feed.

### Changed
- `README.md` — Series 2686–2695 progress: post-2687 → draft · v9.49.0.
- `STATUS.md` — Series 2686–2695 progress 1/10 → 2/10; version v9.48.1 → v9.49.0.
- `VERSION` — 9.48.1 → 9.49.0.
...
### Added
- `content/post-2686-ai-coach-vs-personal-trainer.md` — Series 2686–2695 post 1/10 «Технологічна грамотність self-coached атлета». «AI-coach проти персонального тренера: чесний trade-off без маркетингу». Editorial-brutalist 13-хв формат. Деконструкція обох інструментів: що PT реально робить (технічний зворотний зв'язок у реальному часі, контекстуальне рішення, підзвітність) і де не справляється (масштаб, доступність, ціна). Що AI-coach робить (аналіз даних без втоми, 24/7 доступність, прозорість рішень) і де сліпий (живий рух, відносини). Три осі вибору: безпека техніки vs. масштабованість; відносини vs. аналіз; вирішити зараз vs. навчитись читати. Де кожен виграє без суперечок. Гібридний підхід. clinical-safety: NOT_REQUIRED.

### Changed
- `blog.html` — post-2686 card added at top of feed.
- `README.md` — Editorial series 2686–2695 (Технологічна грамотність self-coached атлета) proposed; post-2686 → draft · v9.48.0.
- `STATUS.md` — post-2686 entry added to content engine row; version v9.47.0 → v9.48.0.
- `VERSION` — 9.47.0 → 9.48.0.
...
### Added
- `content/post-2670-volume-vs-intensity-decision-framework.md` — Series 2666–2675 post 5/10. «Коли збільшувати об'єм, коли інтенсивність, а коли не чіпати нічого». Editorial-brutalist 14-хв формат. Три важелі прогресивного перевантаження: об'єм (MEV-MAV-MRV, Krieger 2010; Schoenfeld et al. 2017), інтенсивність (нейром'язові адаптації, Rhea et al. 2003 meta-analysis), і «не змінювати нічого» (адаптаційна затримка + сигнально-шумове співвідношення). Алгоритм трьох питань: етап блоку → відновлювальна ємність → мета (гіпертрофія vs. сила). Матриця рішень для семи типових ситуацій. Чотири помилки: паралельна зміна двох змінних, ігнорування RPE-дрейфу, хронічне втручання, плутанина між застоєм і повільним прогресом. Правило одного важеля. clinical-safety: NOT_REQUIRED.

### Changed
- `blog.html` — post-2670 card added at top of feed.
- `STATUS.md` — Series 2666–2675 progress 4/10 → 5/10; version v9.46.1 → v9.47.0.
- `README.md` — post-2670 → draft · v9.47.0.
- `VERSION` — 9.46.1 → 9.47.0.
...
### Added
- `content/post-2669-weekly-structure-without-template.md` — Series 2666–2675 post 4/10. «Побудова тренувального тижня без готового шаблону: мінімальні вимоги до структури». Editorial-brutalist 13-хв формат. Архітектура тижня від чотирьох вимог: частота стимулу, відновлення, реалізація пріоритету, відновлювальна ємність. П'ять змінних структури (кількість днів, тривалість, розподіл тіла, позиція пріоритетного руху, розподіл об'єму). Мінімальні моделі: 2/3/4 дні. Чотири типові структурні помилки. Алгоритм побудови з нуля — 5 кроків. clinical-safety: NOT_REQUIRED.

### Changed
- `blog.html` — post-2669 card added at top of feed.
- `STATUS.md` — Series 2666–2675 progress 3/10 → 4/10; version v9.45.0 → v9.46.0.
- `README.md` — post-2669 → draft · v9.46.0.
- `VERSION` — 9.45.0 → 9.46.0.
...
### Added
- `content/post-2668-priority-strength-hypertrophy-endurance.md` — Series 2666–2675 post 3/10. «Як self-coached атлет вибирає пріоритети: сила, гіпертрофія, витривалість — не одночасно». Editorial-brutalist 14-хв формат. Ефект інтерференції (AMPK vs mTOR): механізм неможливості паралельного розвитку всіх якостей. Різниця між розвивальним і підтримувальним стимулом. Три якості та їх взаємні відносини (сила×гіпертрофія, сила×витривалість, гіпертрофія×витривалість). Чотири питання архітектора при виборі пріоритету. Три практичні сценарії (силовий / гіпертрофічний / аеробний пріоритет). Тривалість блоку (6–8 тижнів мінімум). Помилка декларованого проти стимульного пріоритету. clinical-safety: NOT_REQUIRED.

### Changed
- `blog.html` — post-2668 card added at top of feed.
- `STATUS.md` — Series 2666–2675 progress 2/10 → 3/10; version v9.44.0 → v9.45.0.
- `README.md` — post-2668 → draft · v9.45.0.
- `VERSION` — 9.44.1 → 9.45.0.
...
### Added
- `content/post-2667-principles-vs-templates.md` — Series 2666–2675 post 2/10. «Принципи проти шаблонів: чому розуміння механізму важливіше за вибір програми». Editorial-brutalist 13-хв формат. Горизонтальне переміщення між програмами як симптом роботи на рівні шаблонів. П'ять фундаментальних принципів: прогресивне перевантаження, специфічність, відновлення як частина стимулу, варіація, індивідуальність відповіді. Чотири питання для перевірки власної програми (частота, прогресія, пропуски, об'єм). Три рівні оволодіння принципами: розпізнавання → адаптація → проектування. Практичний метод читання принципів у чужій програмі. clinical-safety: NOT_REQUIRED.

### Changed
- `blog.html` — post-2667 card added at top of feed.
- `STATUS.md` — Series 2666–2675 progress 1/10 → 2/10; version v9.42.0 → v9.44.0.
- `README.md` — post-2667 → draft · v9.44.0.
- `VERSION` — 9.43.0 → 9.44.0.
...
### Added
- `content/post-2666-executor-vs-architect.md` — Серія 2666–2675 «Від атлета-виконавця до атлета-архітектора» (пост 1/10). «Виконавець і архітектор: де self-coached атлет знаходиться зараз — і де має бути». Визначення двох режимів: виконавець слідує чужій програмі без критерію, архітектор знає пріоритет, читає сигнали, розуміє обмеження власної системи і мислить мезоциклами. Проблема виконавця не в програмі — у відсутності алгоритму для ситуацій поза планом (пропуск тижня, зупинка прогресу, зміна контексту). Три конкретні ситуації до/після. П'ять запитань для самодіагностики режиму. Мінімальний поріг архітектурного мислення: три речі — формулювання пріоритету блоку, один критерій рішення «продовжувати/коригувати», читання блоку після завершення. Перехід між режимами нелінійний — ключ у тому, щоб знати, в якому режимі ти знаходишся. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — card for post-2666 added at top of feed.

### Changed
- `README.md` — post-2666 updated proposed → draft in series 2666–2675 roadmap table; v9.42.0 ref.
- `STATUS.md` — version updated v9.41.0 → v9.42.0; post-2666 noted in content engine table.
- `VERSION` — 9.41.0 → 9.42.0.
...
### Added
- `content/post-2662-block-comparison-methodology.md` — Серія 2656–2665 «Тренувальний аналіз у реальному часі» (пост 7/10). «Як порівняти два блоки: методологія, яка не бреше». Три причини, чому блоки важко порівнювати (умови не ідентичні, адаптація запізнюється, різні цілі мають різні маркери). Крок 1: визнач ціль блоку ретроспективно (обсяговий/інтенсивний/реалізаційний). Крок 2: три числа — тоннаж по паттерну (сума, не середнє), E1RM перший vs. останній робочий тиждень, RPE-дрейф по тижнях. Крок 3: чотири контекстуальні питання (пропуски, зовнішній стрес, базовий рівень, зміни сну/режиму). Крок 4: один висновок → одна дія. Пастка «кращого блоку» для копіювання: адаптація обмежує повторюваність. Мінімальний протокол документування: що фіксувати на початку і в кінці блоку (10 хвилин). clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2663-when-to-change-program-data-signals.md` — Серія 2656–2665 (пост 8/10). «Коли змінювати програму: сигнали в даних, які кажуть «не зараз», і ті, що кажуть «пора»». Дві категорії помилок: змінити занадто рано (до завершення адаптаційного циклу) і залишитися занадто довго. Сигнали «не зараз»: E1RM рухається навіть повільно; RPE стабільний або знижується; щойно після деловантаження; програма не виконується повністю (>85%); ще в середині блоку. Сигнали «пора»: E1RM стагнує 3+ блоки при повному виконанні; RPE не відновлюється після деловантаження; технічна деградація без покращення кілька блоків; суглобовий дискомфорт що не зникає після двох деловантаженнів; мета змінилась. «Змінити програму» ≠ повна заміна: часто достатньо прогресійної моделі або тривалості блоку. Протокол рішення: одне запитання + чотири умови перед зміною. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2664-long-horizon-monthly-mesocycle-review.md` — Серія 2656–2665 (пост 9/10). «Довгий горизонт: що читати в даних за місяць і за мезоцикл». Два різних горизонти: місячний (тактичний, 4–5 тижнів) і мезоцикловий (стратегічний, 3–6 місяців). П'ять запитань місячного огляду: виконання %, RPE-тренд, E1RM відповідно до типу блоку, розрив план/реальність, що здивувало. Мезоцикловий огляд: нерівномірність адаптації (чутливі/нечутливі паттерни), систематичні прогалини в adherence, сезонна варіація. П'ять патернів у річних даних: E1RM-тренд, поляризація об'єм/інтенсивність, реактивні vs. планові деловантаження, відношення робочий/відновлювальний час, відповідність цілей і реальності. Мінімальний протокол: 15 хв/місяць, 60 хв двічі на рік. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2665-personal-analytics-system-synthesis.md` — Серія 2656–2665 (пост 10/10 — фінал). «Твоя персональна система аналізу тренувань: синтез серії 2656–2665». Десять принципів серії в стислому форматі. Три рівні системи: мінімальний (15 хв/тиждень — три числа + одне рішення), блоковий (30 хв/блок — порівняння і критерій зміни), мезоцикловий (60 хв двічі на рік — стратегія і калібрування). Де система обмежена: техніка в реальному часі, анатомічна специфіка, медичний контекст. Одна зміна, що варта більше за всю систему: одне чітке запитання щотижня і чесна відповідь. **EDITORIAL SERIES 2656–2665 COMPLETE 10/10.** clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — cards for posts 2662–2665 added at top of feed.

### Changed
- `README.md` — posts 2662–2665 updated proposed → draft in series 2656–2665 roadmap table with actual titles and file refs; v9.41.0 refs. Series 2656–2665 marked COMPLETE.
- `STATUS.md` — version updated v9.40.0 → v9.41.0; series 2656–2665 COMPLETE 10/10 noted.
- `VERSION` — 9.40.0 → 9.41.0.
...
### Added
- `content/post-2661-minimal-weekly-review-three-metrics.md` — Серія 2656–2665 «Тренувальний аналіз у реальному часі» (пост 6/10). «Мінімальний тижневий огляд self-coached атлета: три метрики замість двадцяти». Чому двадцять метрик дають шум, а не інсайт; три числа з 80% сигналу: тоннаж за пріоритетним паттерном, RPE на контрольному навантаженні, суб'єктивна якість тижня 1–10; протокол 15 хвилин, одне рішення на виході (продовжувати / зменшити / переглянути); пастка «нова метрика вирішить проблему»; правило тренду (три тижні поспіль — сигнал; одна точка — шум); замінники тоннажу для витривалісного та навичкового тренування. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — card for post-2661 added at top of feed.

### Changed
- `README.md` — post-2661 updated proposed → draft in series 2656–2665 roadmap table; v9.40.0 ref.
- `STATUS.md` — version updated v9.39.0 → v9.40.0; series 2656–2665 progress: 6/10 noted.
- `VERSION` — 9.39.0 → 9.40.0.
...
### Added
- `content/post-2656-reading-training-journal-conclusions.md` — Серія 2656–2665 «Тренувальний аналіз у реальному часі» (пост 1/10). «Як читати власний тренувальний журнал: від запису до висновку». Три рівні читання (після сесії, щотижневий, кінець блоку); сигнал vs. шум у силових даних (тренд vs. точка); читання суб'єктивних записів (кластери, кореляція сон→відчуття); мінімальний протокол перегляду; чотири помилки, які перетворюють читання в самозаспокоєння. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2657-rpe-trend-four-weeks.md` — Серія 2656–2665 (пост 2/10). «RPE-тренд за 4 тижні: що він каже, чого не каже, де шум». Ізольований RPE — беззмістовна цифра; три принципи надійного відстеження (порівнювати однакове, горизонт 3+ тижні, реагувати на тренд а не точку); що сигналізує зростаючий vs. знижувальний vs. хаотичний RPE; п'ять обмежень (дрейф шкали, нова техніка, переоцінка, мотиваційний шум); практична калібровка. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2658-program-progress-without-1rm.md` — Серія 2656–2665 (пост 3/10). «Як визначити, чи прогресує програма — без 1ПМ-тестування». П'ять маркерів: тоннаж-тренд, E1RM (формула Epley, найточніший 3–8 повт. при RPE 7–9), RPE при стандартному протоколі (контрольна вправа), технічна якість при стомленні (відеопорівняння), швидкість відновлення між сесіями. Що робити якщо жоден маркер не показує прогресу. Коли 1ПМ-тестування таки потрібне і чому потребує пікування. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2659-deload-data-deadaptation-vs-recovery.md` — Серія 2656–2665 (пост 4/10). «Деловантаження в даних: як виглядає деадаптація, а як — відновлення». Деадаптація (2–3+ тижні повного спокою) vs. деловантаження (знижений стимул, адаптації зберігаються). Як виглядає деловантаження в журналі: тоннаж −40–60%, RPE нижчий, суб'єктивно «замало». Паттерн «падіння + стрибок» після деловантаження як підтвердження ефективності. Ознаки справжньої деадаптації. Планове vs. реактивне деловантаження. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2660-technical-regression-in-logs.md` — Серія 2656–2665 (пост 5/10). «Технічна регресія в логах: коли «важче стало» — не прогрес». Двозначність сигналу «важче»; механізм непомітної технічної деградації; непрямі індикатори (зростаючий RPE без причини, нестандартний дискомфорт після вправи, варіабельність результату) і прямий (відеозапис). Регресія від втоми vs. від браку навичок. Мінімальний протокол (оцінка якості 1–5 або коротка нотатка). Концепція «технічного порогу» в журналі. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — cards for posts 2656–2660 added at top of feed.

### Changed
- `README.md` — posts 2656–2660 updated proposed → draft in series 2656–2665 roadmap table; v9.39.0 refs.
- `STATUS.md` — version updated v9.38.0 → v9.39.0; series 2656–2665 progress: 5/10 noted.
- `VERSION` — 9.38.0 → 9.39.0.
...
### Added
- `content/post-2653-training-identity-decision-making.md` — Block 2646–2655 series «Методологія самокоучингу» пост 8/10. «Тренувальна ідентичність і прийняття рішень: хто ти як атлет — і чому це має значення». Тренувальна ідентичність як операційний конструкт (не мотиваційний). Три типи неузгодженості: ідентичність рівня, ідентичність стилю, ідентичність реакції на труднощі. Аудит автоматичних рішень (4 тренування). Перевірка відповідності ідентичності реальній поведінці. Навмисне використання: дифузний self-concept → операційна умова рішення. Три зони ризику: плато, початок блоку, повернення після паузи. clinical-safety: NOT_REQUIRED.
- `blog.html` — картка post-2653 додана на початок блогу.

### Changed
- `README.md` — post-2653 → draft · v9.36.0 → post-2653-training-identity-decision-making.md.
- `STATUS.md` — post-2653 додано; version v9.35.0 → v9.36.0.
...
### Added
- `content/post-2651-victor-operational-boundary.md` — Block 2646–2655 series «Методологія самокоучингу» пост 6/10. «Де Victor корисний, де атлет мусить вирішувати сам: операційна межа AI-coach». Три структурні переваги AI-coach (систематична пам'ять без упередженості, паттерн-аналіз, нейтральність); три архітектурні обмеження (не бачить атлета, не відчуває контекст, не несе відповідальності); операційна межа (Victor вирішує обсяг/прогресію/деловантаження; атлет — пріоритизацію і медичний контекст без делегування); дві помилки (ігнорування vs надделегування); рамка Victor-аналітик / атлет-ЛПР. clinical-safety: NOT_REQUIRED.
- `blog.html` — картка post-2651 додана на початок блогу.

### Changed
- `README.md` — post-2651 → draft · v9.34.0 → post-2651-victor-operational-boundary.md.
- `STATUS.md` — post-2651 додано; version v9.33.1 → v9.34.0.
- `VERSION` — 9.33.1 → 9.34.0.
...
### Added
- `content/post-2649-when-to-change-program-vs-how-to-read-it.md` — Block 2646–2655 series «Методологія самокоучингу» post 4/10. Коли змінювати програму, а коли — спосіб її читати. Два типи проблем: проблема програми vs. проблема читання — і чому реакція «нова програма» частіше вирішує другу неправильним методом. Чотири типи відхилень у виконанні: технічне (RPE виконаний не так), контекстуальне (додані/пропущені сесії), інтерпретаційне (акумуляція прийнята за збій), часове (оцінка до адаптаційного горизонту). Коли зміна програми виправдана: невідповідність рівню, умовам, зміна цілі. Алгоритм 3 кроки: аудит виконання → перевірка горизонту → відповідність умовам/меті. Ціна зміни: скидання адаптаційного таймера, втрата даних для самонавчання, психологічна ціна (Lally et al. 2010). Ralston et al. 2017; Ogasawara et al. 2013; Schoenfeld 2010; Loenneke et al. 2020. clinical-safety: NOT_REQUIRED.
- `blog.html` — картка post-2649 додана на початок блогу.

### Changed
- `README.md` — post-2649 → draft · v9.32.0.
- `VERSION` — 9.31.0 → 9.32.0.
...
### Added
- `content/post-2646-self-coached-body-model.md` — Серія 2646–2655 «Методологія самокоучингу» пост 1/10. Як self-coached атлет будує модель власного тіла. Різниця між «знаю» і «відчуваю» як основна перевага методичного тренування. Три типи даних: об'єктивні структурні, суб'єктивні поточні, непрямі виведені. Проблема надлишку інформації (Jacoby et al. 1974; Eppler & Mengis 2004 — data overload). Сигнал: навантаження по ключових вправах, RPE підходу, якість сну (3-рівнева), технічна нотатка, загальна оцінка сесії (1–5). Шум: щоденна вага, DOMS (Schoenfeld & Contreras 2013), мотивація як маркер готовності (Marcora et al. 2009), Body Battery на разовій основі (Hautala et al. 2010). Мінімальна система: 2–3 хв після сесії + 10–15 хв раз на 4 тижні. clinical-safety: NOT_REQUIRED.
- `blog.html` — post-2646 card added at top of feed.

### Changed
- `README.md` — post-2646 → draft; серія 2646–2655 progress: 1/10.
- `STATUS.md` — серія 2646–2655 розпочата (1/10); post-2646 added v9.29.0.
- `VERSION` — 9.28.0 → 9.29.0.
...
### Added
- `content/post-2641-morning-vs-evening-training-chronobiology.md` — Серія 2636–2645 пост 6/10. Ранкові vs вечірні тренування: хронобіологія циркадних ритмів і тренувальна продуктивність. Рочестер et al. (2009) — 3–7% різниця 1RM. Хааф et al. (2015, Current Biology) — circadian entrainment за 3–4 тижні нівелює різницю. Що важливіше: консистентність часу, якість виконання (Marcora et al., 2009), ментальна присутність. Компенсація ранкового «мінусу»: подовжений розігрів, 45–60 хв між підйомом і тренуванням, кофеїн. Хронотип як реальне обмеження (D6–D7 пізній). clinical-safety: NOT_REQUIRED.
- `content/post-2642-time-efficient-training-30-40-minutes.md` — Серія 2636–2645 пост 7/10. Тренування з мінімальним часом: MED-логіка для 35-хвилинної сесії. Schoenfeld et al. (2019) — 5–9 підходів з RIR ≤2 зіставні з 10+. Анатомія 75-хвилинної сесії і зони скорочення. Структура 35-хвилинної сесії: 5+20–25 хв. Compound-first вибір вправ. Помилки: темп замість скорочення, пропуск розігріву, скорочення ваги замість обсягу. Rhea et al. (2003) — частота 3×/тиждень > обсяг на сесію. clinical-safety: NOT_REQUIRED.
- `content/post-2643-one-set-vs-three-sets-efficiency.md` — Серія 2636–2645 пост 8/10. Один підхід або три: нелінійна функція тренувального обсягу. Krieger (2010) — 3–4 підходи = 80–90% максимального ефекту, 1 підхід = 50–60%. Різниця між силою (нейронний сигнал) і гіпертрофією (обсяг критичний). Коли 1 підхід виправданий: підтримувальний блок (Bickel et al., 2011), дефіцит часу, singles. Тижневий обсяг vs. підхідний: 1×3 дні = 3×1 день за кількістю стимулів. clinical-safety: NOT_REQUIRED.
- `content/post-2644-training-partner-self-coached-independence.md` — Серія 2636–2645 пост 9/10. Self-coached атлет і тренувальний партнер: структура незалежного партнерства. Що дає: страховка (psychophysiological safety margin), зовнішня оцінка техніки, конкурентний ефект (Zajonc 1965). Що не дає: заміну програмування, мотивацію, обʼєктивну оцінку без кваліфікації. Принципи: власний план завжди свій, роль партнера визначена заздалегідь, тренування незалежні від розкладу партнера. Адаптація solo: rack pins, відеозйомка з двох кутів. clinical-safety: NOT_REQUIRED.
- `content/post-2645-synthesis-2636-2645-decision-map.md` — Серія 2636–2645 пост 10/10 — завершення. Синтез: карта рішень для нестандартних тижнів. Три рівні адаптації (повна / скорочена / підтримувальна). Алгоритм рішення для будь-якої нестандартної ситуації. Принцип мінімального збереження системності. П'ять практичних принципів. Анонс серії 2646–2655. clinical-safety: NOT_REQUIRED.
- `blog.html` — posts 2641–2645 added at top of feed.

### Changed
- `STATUS.md` — серія 2636–2645 завершена (10/10); наступна: 2646–2655 (методологія самокоучингу).
- `README.md` — posts 2641–2645 → draft; серія 2636–2645 COMPLETE.
- `VERSION` — 9.27.1 → 9.28.0.
...
### Added
- `content/post-2640-training-under-chronic-stress.md` — Editorial series post 2640. «Тренування при хронічному стресі: де межа між корисним стресором і зайвим навантаженням». Серія 2636–2645: Тренувальні рішення у реальному житті (пост 5/10). HPA вісь і фізіологія стресової відповіді — чому організм не розрізняє джерело стресу. Хронічно підвищений кортизол: пригнічення mTOR-сигналінгу (−15–30% анаболічної відповіді), антагонізм тестостерону, порушення SWS/GH-секреції, підвищений запальний фон (Saariaho et al. 2018; Kreider et al. 2012). Концепція аллостатичного навантаження (МакЮен) — спільний рахунок відновлення для всіх стресорів. Різниця між гострим стресом (1–3 дні, корекція RPE/обсягу) і хронічним (1+ тижні, системна зміна стратегії). Сигнали системного перевантаження: підвищений базовий RPE при розминці, відсутність відновлення після сну, пульс у спокої +5–10 bpm. Три стратегії: Стратегія 1 (мінімально ефективна доза — −30–40% обсягу, RPE ≤7), Стратегія 2 (протокол збереження — 2 сесії/тиждень, 70–75%), Стратегія 3 (заплановане скорочення). Практичний алгоритм: три параметри → сумарний рахунок → вибір стратегії. clinical-safety: CONDITIONAL PASS (фокус на HPA axis і тренувальних рішеннях, не психіатрична порада). Blog card added.
- `blog.html` — post-2640 card added at top of feed.

### Changed
- `README.md` — post-2640 → draft · v9.27.0.
- `STATUS.md` — post-2640 added to editorial series 2636–2645 (5/10); version v9.26.0 → v9.27.0.
...
### Added
- `content/post-2639-gym-equipment-change-adaptation.md` — Editorial series post 2639. «Зміна залу або обладнання: як адаптувати програму без структурного зламу». Серія 2636–2645: Тренувальні рішення у реальному житті (пост 4/10). Принцип SAID і специфічність адаптації — що реально змінюється при зміні залу: траєкторія руху, нейром'язовий малюнок, доступний репертуар; що не змінюється: загальна сила, фізіологічні адаптації, прогресивне перевантаження. «Ефект нового обладнання» — системно завищений RPE протягом першого тижня (не регрес — нейром'язове калібрування). Ієрархія замінників 4 рівні: Рівень 1 (той самий тип — −5–10%), Рівень 2 (та сама механіка, інший тип — −10–15%), Рівень 3 (та сама м'язова група, інша механіка — −15–20%), Рівень 4 (за функцією). Практичний алгоритм: інвентаризація залу → технічне перше тренування RPE ~6 → два тижні калібрування → повернення за RPE. Де гнучкість доречна (замінники, послідовність), де не поступатись (механічна специфічність, частота, структура блоку, прогресивне перевантаження). Типова помилка — повний reset — і чому він коштує найдорожче. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — post-2639 card added at top of feed.

### Changed
- `docs/INCIDENT_RESPONSE.md §R-47.12` — item 3 status note updated: "(this commit — §12.6 update pending separate OBSERVABILITY patch)" → confirmed done (§12.6 v2.2 patch, v9.25.1); stale-consequence cross-ref fully resolved.
- `STATUS.md` — post-2639 added to editorial series 2636–2645 (4/10); version v9.25.1 → v9.26.0.
- `README.md` — post-2639 → draft.
- `VERSION` — 9.25.1 → 9.26.0.
...
### Added
- `content/post-2638-return-after-two-weeks-off.md` — Серія 2636–2645 «Тренувальні рішення у реальному житті» пост 3/10. «Повернення після двох тижнів без тренувань: що деадаптувалось і як відновлювати». Порядок детренування: нейром'язова ефективність −10–20% (першою), максимальна сила −1–3%, м'язова маса ~0%. Феномен м'язової пам'яті: Bruusgaard et al. (2010) PNAS — міонуклеуси зберігаються після детренування. Mujika & Padilla (2000): помітна втрата маси — після 3–4 тижнів. Протокол повернення: сесія 1 — −25% об'єму, −20% ваги, RPE ≤7; сесія 2 — RPE 7–8; сесії 3–4 — довіренові ваги. clinical-safety: NOT_REQUIRED. Blog card added.
- `blog.html` — пост 2638 доданий у верх стрічки.

### Changed
- `STATUS.md` — серія 2636–2645 прогрес 2/10 → 3/10; Total posts 2096 → 2097.
- `README.md` — post-2637 → draft · v9.24.0; post-2638 → draft · v9.25.0.
- `VERSION` — 9.24.0 → 9.25.0.
...
### Added
- `content/post-2637-business-trip-training-protocol.md` — Серія 2636–2645 «Тренувальні рішення у реальному житті» пост 2/10. «Ділова поїздка і тренування: протокол збереження прогресу». Три сценарії за тривалістю: 1–3 дні (full-body × 1, RPE 7–8), 4–7 днів (2–3 сесії, −50% об'єму), 8–14+ днів (підтримуючий план 3/тиждень). Підтримуючий об'єм: Bickel et al. (2011) — 1/9 нормального об'єму зберігає силу 32 тижні; Androulakis-Korakakis et al. (2020) — 1 важкий підхід ≥80% 1RM/тиждень достатній; Ralston et al. (2017) — систематичний огляд мінімального об'єму. Проблема RPE у незнайомому залі: Zourdos et al. (2016) ICC 0.89–0.94 тільки при стабільних умовах; перша сесія — калібрувальна на RPE 6–7. Таблиця замін: горизонтальний жим, вертикальний жим, тяга горизонтальна/вертикальна, румунська тяга — відтворюються гантелями; важке присідання — не відтворюється повноцінно. Порядок виключень: ізоляція → аксесуарні варіації → технічно складні → базові патерни. Три помилки: компенсація ваги обсягом (Androulakis-Korakakis: інтенсивність > обсяг для підтримки), пропуск усіх сесій через «неідеальні умови» (Helms 2016: комплаєнс > оптимізація), форсована перша сесія. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — пост 2637 доданий у верх стрічки.

### Changed
- `STATUS.md` — серія 2636–2645 прогрес 1/10 → 2/10; Total posts 2095 → 2096; next: post-2638 (тренування під час хвороби).
- `VERSION` — 9.23.1 → 9.24.0.
...
### Added
- `content/post-2636-training-when-sleep-disrupted.md` — Editorial series 2636–2645 «Тренувальні рішення у реальному житті» пост 1/10. «Сон зруйнований — тренуватись чи ні: протокол мінімальних втрат». Три типи порушення сну (гострий дефіцит, хронічне накопичення, фрагментація); тверда межа «не тренуватись» (<3 год або кілька поганих ночей); два рівні модифікації — Рівень А (−25% об'єму, RPE ≤8) і Рівень Б (2–3 вправи, 2–3 підходи, RPE ≤7, 40 хв max); пріоритет збереження інтенсивності над об'ємом (Schoenfeld); порядок виключення вправ; кофеїн-умова; Victor volume autoregulation. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — post-2636 card added at top of feed.
- `README.md` — post-2636 статус → draft; нова серія 2656–2665 «Тренувальний аналіз у реальному часі» додана до роадмапу.

### Changed
- `VERSION` — 9.22.0 → 9.23.0.
...
### Added
- `content/post-2629-stimulus-to-adaptation-timeline.md` — EDITORIAL SERIES 2626–2635 «Тіло як система: фізіологія адаптації без популярного спрощення» пост 4/10. «Час між стимулом і адаптацією: чому «одразу після тренування» — не про м'язи». Три непорозуміння: (1) різниця між відновленням робочої спроможності, структурною репарацією і надкомпенсацією; (2) різні часові шкали різних систем (глікоген — години: Ivy et al. 1988; MPS запуск — 1–4 год але структурна зміна тижні: Mitchell et al. 2012; нейральна адаптація — 2–8 тижнів: Sale 1988, Carroll et al. 2001; виміряна гіпертрофія CSA — 4–8 тижнів: Staron et al. 1994; сухожилки — 3–12 місяців: Magnusson & Kjaer 2003; кістка — 6+ місяців: Kerr et al. 1996); (3) training residuals (Issurin 2010): максимальна сила 30±5 днів, вибухова сила 5±3 дні, аеробна витривалість 30±5 днів, гнучкість 15±5 днів. Masking effect (Fry & Kraemer 1997): накопичена втома маскує адаптацію — deload не «дає вирости», а знімає маску. Паралельні, а не послідовні процеси: тіло реагує на тренувальну історію, не на одне тренування. Практика: новачок 3–4 рази/тиждень (нейральна система відновлюється швидко, часте тренування прискорює моторне навчання); intermediate мінімум 2× на групу (Schoenfeld et al. 2016); консервативне збільшення навантаження ≤10%/тиждень враховує дисбаланс сухожилок vs м'язів. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — картка post-2629 додана на початок стрічки.
- `README.md` — post-2629 → draft; EDITORIAL SERIES 2636–2645 «Тренувальні рішення у реальному житті: коли теорія зустрічає контекст» (10 постів) додана до роадмапу.

### Changed
- `STATUS.md` — Total posts: 2087 → 2088; v9.18.0 → v9.19.0; серія 2626–2635 STARTED 4/10; наступний: post-2630.
- `VERSION` — 9.18.0 → 9.19.0.
...
### Added
- `content/post-2626-sra-cycle-base-model.md` — EDITORIAL SERIES 2626–2635 «Тіло як система: фізіологія адаптації без популярного спрощення» пост 1/10. SRA-цикл (Stimulus–Recovery–Adaptation): базова модель (Яковлєв 1955/1977; Issurin 2008, 2010; Bompa & Haff 2009; Chiu & Barnes 2003). Що модель НЕ каже: часові рамки є різними для кожної системи (нейром'язова 24–48 год, гіпертрофія тижні, сполучна тканина місяці; training residuals Issurin 2010); одиничний стимул не дає довгострокової структурної адаптації — потрібен кумулятивний ефект; алостатичне навантаження (McEwen 1998) ігнорується в оригінальній SRA-моделі. Три версії, що плутають атлетів: «більше тренувань = більше адаптації» — нелінійна кінетика, MED/MAV/MRV (Kraemer & Ratamess 2004), non-functional overreaching (Meeusen et al. 2013); «відпочивай і м'яз ростиме» — відпочинок є необхідною умовою, але не достатньою; «muscle confusion» — без механістичного підґрунтя (Schoenfeld 2010; Krieger 2010). Нелінійна кінетика і masking effect: Fry & Kraemer (1997) — накопичена втома маскує адаптацію; deload як операційний механізм виявлення реальної підготовленості. Три прямих висновки для програмування: частота тренувань є функцією тривалості SRA-циклу (Aube et al. 2022 мета-аналіз); інтенсивність визначає тип адаптації; мезоцикл — мінімальний горизонт планування. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `content/post-2627-said-principle-specificity.md` — EDITORIAL SERIES 2626–2635 пост 2/10. SAID-принцип (Specific Adaptation to Imposed Demands): три незалежних механізми специфічності — (1) нейром'язова: рекрутування і координація специфічні до кута, типу і швидкості скорочення (Enoka 2015: ізометрика при 90° адаптується специфічно до 90°); (2) метаболічна: капіляризація і мітохондріальна щільність специфічні до задіяних м'язів і патерну (Coggan et al. 1992; Holloszy & Coyle 1984); (3) структурна: fiber type shift, ексцентрична гіпертрофія у довжину vs концентрична у товщину (Fleck & Kraemer 2004). «М'язова пам'ять» як SAID-явище: збереження міоядер (Bruusgaard et al. 2010) і моторного патерну — специфічно до задіяних м'язів і руху. Три практичні наслідки: «слабке місце» завжди потребує специфічної ідентифікації; кросс-тренінг є доповненням, не еквівалентом; перші 2–4 тижні нової вправи — нейральне навчання, а не оцінка ефективності. GPP-перенесення і межі (Siff 2003). Де SAID неповний: загальне анаболічне середовище, «вікно» початківця, injury-modified SAID. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `content/post-2628-neural-vs-structural-adaptation.md` — EDITORIAL SERIES 2626–2635 пост 3/10. Нейральна vs структурна адаптація — три рівні нейральної: рекрутування рухових одиниць (Sale 1988; нейральний резерв у нетренованих; рекрутування волокон типу II); частота розрядів і doublets (Enoka & Fuglevand 2001; вибуховий початок зусилля); міжм'язова координація — зниження коактивації антагоністів (Behm & Sale 1993). Структурна адаптація: міофібрилярна гіпертрофія (три механізми Schoenfeld 2010: механічне напруження/метаболічний стрес/пошкодження; mTORC1 → MPS) і саркоплазматична (Haun et al. 2019 незалежність двох типів). Часова динаміка: Carroll et al. (2001) — нейральна домінує перші 4–8 тижнів; гіпертрофія наздоганяє після 8–16 тижнів. Дихотомія «сила vs м'яз»: Schoenfeld et al. (2017) RCT — 3×10 і 7×3 при рівному об'ємі дають рівну гіпертрофію, але різну нейральну ефективність при 1RM. Що нейральна адаптація не пояснює: ексцентрична перевага нетренованих (Komi 2000); ранній MPS-сигнал з першого тренування (Mitchell et al. 2012); адаптації сухожилків і жорсткість (Magnusson & Kjaer 2003). Практика: початківець — стабільний патерн важливіший за різноманітність; проміжний — обидва компоненти (об'єм + важка робота); два сценарії стагнації з різними рішеннями. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — картки постів 2626–2628 додані на початок стрічки.

### Changed
- `STATUS.md` — Total posts: 2084 → 2087; поточна версія v9.17.0 → v9.18.0; серія 2626–2635 STARTED 3/10; наступний: post-2629.
- `VERSION` — 9.17.1 → 9.18.0.
...
### Added
- `content/post-2625-critical-thinking-synthesis-card.md` — Series 2616–2625 «Критичне мислення у тренуванні» post 10/10 · СИНТЕЗ. «Картка критичного мислення self-coached атлета» — фінальний інструментальний синтез серії. Не підсумок для читання, а картка запитів до 6 конкретних ситуацій: (1) «ця програма мені підходить» — ефект новизни vs ефективність, три реальні критерії (пост 2617), роль плацебо (пост 2618); (2) аналіз тижня або блоку — мінімум 4 точок даних, мультивимірна причинність (Kahneman 2011), кореляція ≠ причинність (пост 2621); (3) читання досліджень — зовнішня валідність, Cohen's d vs p-value, Ioannidis (2005), механістичний аргумент без незалежного підтвердження (пост 2624); (4) власний досвід — ізольованість змін, систематичність n=1 (пост 2619), «just-so story» тест Gould (1991), survivorship bias (пост 2620), крива компетентності (пост 2622); (5) «не спрацювало» — горизонт оцінки, що вимірювалось; (6) Victor і зовнішні джерела — формулювання запиту як вектор відповіді. Зведена таблиця 9 упереджень × ситуація активації. Мета-рівень: рівень впевненості пропорційний рівню доказів. Чесні межі серії: anchoring, availability heuristic, status quo bias — за скоупом. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2625 card added at top of feed.

### Changed
- `README.md` — post-2625 marked draft · v9.17.0 with file reference; series 2616–2625 COMPLETE.
...
### Added
- `content/post-2623-what-evidence-base-means.md` — Series 2616–2625 «Критичне мислення у тренуванні» post 8/10. «Що означає «доказова база»: як читати дослідження без PhD в спортивній науці». Ієрархія доказів (метааналіз → RCT → когортні → поперечні → n=1) з ключовим застереженням: якість виконання важливіша за тип; ієрархія оптимальна для клінічних, а не тренувальних питань. p-значення — що воно каже і чого не каже (ймовірність даних при нульовій гіпотезі, а не ймовірність ефекту); Ioannidis (2005) — при типовій статистичній силі більшість позитивних знахідок у деяких галузях є хибнопозитивними. Effect size (Cohen's d: 0.2 малий / 0.5 середній / 0.8 великий) і довірчий інтервал важливіші за p-значення. Зовнішня валідність: нетреновані вибірки більшості RCT, короткі горизонти (8–12 тижнів), лабораторні умови, сурогатні маркери; Schoenfeld et al. (2016) метааналіз частоти — ефект більший у нетренованих; Hopkins et al. (2009) — мінімум ~25 учасників на групу для d = 0.5 з 80% силою. Криза відтворюваності: publication bias, HARKing, маленькі вибірки. П'ять практичних питань при оцінці будь-якого тренувального твердження. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2624-smart-explanation-trap.md` — Series 2616–2625 «Критичне мислення у тренуванні» post 9/10. «Пастка «розумного пояснення»: чому переконливе обгрунтування ≠ правильне рішення». Газзаніга (1970-ті) «interpreter» — ліва півкуля конструює наративи навіть без доступу до справжніх причин поведінки. Конфабуляція як нормальна когнітивна функція: Nisbett & Wilson (1977) — люди не мають прямого доступу до причин власних рішень і конструюють правдоподібні пояснення post-hoc. Три типи пасток: (1) механістичне обгрунтування — реальний механізм не гарантує прогнозований результат без незалежного підтвердження (Schoenfeld et al. 2015 — темп ексцентрики мінімально впливає на гіпертрофію попри правдивий механізм); (2) мотивоване мислення (Kunda 1990) — пошук доказів для бажаного висновку; (3) пост-хок раціоналізація — впевнене пояснення результату заднім числом при кількох одночасно змінених змінних (Kahneman 2011 System 1). Два операційних тести на якість пояснення: (1) тест передбачальної сили — чи можна сформулювати конкретний тест; (2) «just-so story» тест Gould — симетрія: якщо ти легко пояснюєш і протилежний результат, пояснення несе нульову інформацію. Практична рамка: механізм → гіпотеза → анекдот → незалежні дані — різні рівні впевненості. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — posts 2623 and 2624 added at top of feed.

### Changed
- `README.md` — posts 2623 and 2624 marked draft · v9.16.0 with file references; post-2622 already marked (corrected from proposed to draft · v9.15.0).
- `content/post-2622-dunning-kruger-in-training.md` — footer "next post" teaser corrected to match README 2623 topic (evidence base, not «слухай своє тіло»).
- `VERSION` — 9.15.3 → 9.16.0.
...
### Added
- `content/post-2622-dunning-kruger-in-training.md` — Block 2616–2625 «Критичне мислення у тренуванні» post 7/10. Ефект Даннінга-Крюгера у тренуванні: де він проявляється і як його обходити. Kruger & Dunning (1999, JPSP) — оригінальний метакогнітивний дефіцит vs. популярна «гора дурнів»; три зони прояву: техніка без зворотного зв'язку (Barker et al. 2004 — новачки недооцінюють відхилення від цільового паттерну), програмна впевненість інтермедіата 1–3 роки (надмірна кастомізація, передчасна авторегуляція, «моя генетика інша»), поради з позиції некомпетентного (6–18 міс.); перенесена самоефективність між доменами (Perkins et al. 2019); чотири ознаки активного ефекту; практичний інструментарій: відеозапис із відкладеним переглядом, опис підходу вголос, передбачення і перевірка (Tversky & Kahneman 1974), пошук дискомфортних свідчень; роль Victor — запити на прогалини, а не підтвердження. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added. Next: post-2623 (Чому «слухай своє тіло» — корисна порада і небезпечний рецепт одночасно).
- `blog.html` — post-2622 card added at top of feed.

### Changed
- `STATUS.md` — Total posts 2080 → 2081; Series 2616–2625 progress 6/10 → 7/10; current version v9.13.0 → v9.15.0.
- `VERSION` — 9.14.1 → 9.15.0.
...
### Added
- `content/post-2621-correlation-vs-causation-training-journal.md` — Block 2616–2625 «Критичне мислення у тренуванні» пост 6/10. «Різниця між кореляцією і причинністю у власному тренувальному журналі». Три структурні причини помилок: конфаундери, мала вибірка і природна варіабельність (Atkinson & Nevill, 1998 — мінімум 4–6 точок), зворотна причинність. Post hoc ergo propter hoc і регресія до середнього у тренувальних даних. Три класичні приклади: «кава дає PR» (selection bias на дні готовності), «на дилоді гірше» (оцінка в момент пікової втоми), «без відпочинку — плато» (зворотня причинність). Критерії Бредфорда Гілла (1965) адаптовані для тренувального контексту: сила зв'язку, дозо-відповідна залежність, часова послідовність, оборотність, біологічна правдоподібність, відтворюваність. Чотири операційних питання перед висновком: конфаундери → зворотній напрямок → ефект зникнення при усуненні → кількість спостережень. Victor: кореляція як сигнал для гіпотез, не для висновків. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — card 2621 додано до топу фіду.
- `README.md` — Editorial series 2626–2635 «Тіло як система: фізіологія адаптації без популярного спрощення» (10 постів) додано до роадмапу.

### Changed
- `README.md` — post 2621: proposed → draft · v9.14.0.
- `VERSION` — 9.13.0 → 9.14.0.
...
### Added
- `content/post-2618-placebo-effect-in-fitness.md` — Серія 2616–2625 «Критичне мислення у тренуванні» пост 3/10. Placebo ефект у фітнесі: де він реальний, де шкідливий, де — корисний. Placebo як нейробіологічний механізм (Benedetti et al., 2005 — активація опіоїдних рецепторів; Finniss et al., 2010 NEJM — три шляхи: expectation, classical conditioning, observational learning). Де реальний і вимірюваний: ефект очікування від добавок (Beedie et al., 2006 — кофеїн/плацебо у велоспорті, +3% перформанс від плацебо; Beedie & Foad 2009 метааналіз, d≈0.5); ритуал підготовки (Clark et al., 2012 — умовний стимул і зменшення тривоги перед важким підходом); ефект переконання у програмі (Crum & Langer, 2007 — однакова активність, різна фізіологічна відповідь залежно від переконань). Де шкідливий: маскування сигналів втоми (розходження суб'єктивних і об'єктивних маркерів готовності при сильних ритуалах); фінансовий ресурс на добавки без доказової бази понад плацебо; заміщення об'єктивної оцінки прогресу суб'єктивним відчуттям. Де нейтральний: корисні умовні стимули без шкідливих наслідків — не потребують ревізії. Парадокс відкритого плацебо: Kaptchuk et al. (2010) — ефект зберігається при повній поінформованості. Клінічна межа: Moseley et al. (2002 NEJM, удавана хірургія коліна) — де placebo є медичною проблемою, а не тренувальним інструментом. Три операційних питання: чи маскує сигнал → чи спотворює атрибуцію прогресу → чи коштує ресурс без реального ефекту. Чому не варто елімінувати всі умовні стимули. clinical-safety: NOT_REQUIRED.
- `blog.html` — картка post-2618 додана вгорі стрічки.

### Changed
- `README.md` — post-2618 → draft у roadmap серії 2616–2625.
- `STATUS.md` — Series 2616–2625 progress: 2/10 → 3/10; current version → v9.11.0; next: post-2619 (Як читати n=1 у власному досвіді).
...
### Added
- `content/post-2617-what-does-program-suits-me-mean.md` — Серія 2616–2625 «Критичне мислення у тренуванні: як self-coached атлет перестає обманювати себе даними» пост 2/10. Що означає «мені підходить ця програма»: три реальні критерії замість відчуття. Чому DOMS, пампінг і «відчуття хорошого тренування» не є валідними критеріями сумісності програми — суб'єктивна оцінка сесії корелює з попередніми очікуваннями, а не з об'єктивними показниками (Beedie et al., 2003); DOMS не корелює з довгостроковими адаптаційними результатами (Schoenfeld & Contreras, 2013). Три реальних критерії відповідності: (1) прогрес по заздалегідь визначених параметрах — чому критерії потрібно фіксувати письмово до старту блоку (захист від post-hoc rationalization і confirmation bias); (2) відновлення між сесіями — операційний маркер: перший підхід у тижні 2–3 не має бути важчим при тій самій вазі, ніж у тижні 1 (системна акумуляція втоми = обсяг перевищує відновлювальну здатність); (3) adherence ≥80% — intervention fidelity як умова валідної оцінки програми (клінічна аналогія: без виконання протоколу висновок про протокол неможливий). Як поєднати три критерії: програма «підходить» якщо всі три виконані; кожен порушений — діагностика конкретного вектору (прогрес/відновлення/виконання). Про індивідуальну відповідь: HERITAGE Family Study (Bouchard et al., 1999 — ΔVO2max від -8 до +100% на однаковий протокол); Hubal et al. (2005 Medicine & Science in Sports & Exercise — гіпертрофічна варіабельність). Висновок про «не підходить» є валідним тільки після виконання всіх трьох критеріїв. Практичний мінімум: фіксація параметрів до першої сесії, RPE-контроль на тижні 2, підрахунок adherence після блоку. clinical-safety: NOT_REQUIRED.
- `blog.html` — картка post-2617 додана вгорі стрічки.

### Changed
- `README.md` — post-2617 → draft.
- `STATUS.md` — Series 2616–2625 progress: 1/10 → 2/10; current version → v9.10.0; next: post-2618 (Placebo ефект у фітнесі: де він реальний, де шкідливий, де — корисний).
- `VERSION` — 9.9.1 → 9.10.0.
...
### Added
- `content/post-2616-confirmation-bias-in-training.md` — Серія 2616–2625 «Критичне мислення у тренуванні: як self-coached атлет перестає обманювати себе даними» пост 1/10. Confirmation bias як архітектурна особливість (Wason, 1960 — задача вибору). Три форми у тренуванні: (1) вибіркова пам'ять — self-serving attribution bias (Mezulis et al., 2004 Psychological Bulletin, 266 вибірок): успіх приписується системі, невдача — зовнішнім факторам; (2) вибіркова увага під час сесії — RPE / болісність / «гарне тренування» фільтруються через очікування; (3) активний пошук підтвердження ззовні (Nickerson, 1998 Review of General Psychology). Структурна проблема self-coached: конфлікт ролей — генератор плану, виконавець і інтерпретатор в одній особі (Pelletier & Dion, 2007). Blind spot bias (Pronin et al., 2002) — чому усвідомлення не знижує упередження. Чотири протоколи: попередньо сформульована гіпотеза з критерієм спростування до старту блоку; «адвокат диявола» за 2 тижні до кінця; журнал без оцінок — тільки числа і факти відхилень; відкладена інтерпретація через 72 год. Чому досвід не захищає (Kruger & Dunning, 1999). clinical-safety: NOT_REQUIRED.
- `blog.html` — картка post-2616 додана вгорі стрічки.

### Changed
- `README.md` — Editorial series 2616–2625 (Критичне мислення у тренуванні) proposed; post-2616 → draft.
- `STATUS.md` — README roadmap: серія 2616–2625 proposed; post-2616 → draft; Series 2616–2625 progress: 1/10; current version → v9.9.0; next: post-2617 (Що означає «мені підходить ця програма»: три реальні критерії замість відчуття).
- `VERSION` — 9.8.0 → 9.9.0.
...
### Added
- `content/post-2607-technique-vs-load-boundary.md` — Серія 2606–2615 «Практичний інструментарій self-coached атлета: рішення без тренера» пост 2/10. Техніка vs. навантаження: де лежить межа між прогресом і ризиком. Концептуальний зсув: техніка як сигнал розподілу навантаження, а не естетичний стандарт. Два типи відхилення: (1) компенсаторне — стомлення цільових м'язів, кінець продуктивного підходу, Тип А; (2) структурне — нестабільність допоміжних систем до стомлення цільових груп, Тип Б, «вага поки не твоя». Три класифікаційні питання в реальному часі: де почалось відхилення; чи контрольоване; чи прогресує між повтореннями. Кумулятивне навантаження McGill (2007) — чому деградація позиції множить ризик на пасивні структури непомітно. Адаптивна межа: підготовленість стабілізаторів + фаза блоку + рівень стомленості зміщують прийнятний діапазон. Система журналу: три позначки (позиція A/Б/?, тип відхилення, рішення), 10 сек/підхід; між-блоковий аналіз «>20% підходів Б/допоміжна = навантаження перевищило ємність». Межа відповідальності системи: не замінює оцінку симптомів — при болі у суглобах, набряку, гострих відчуттях — до спеціаліста. Зворотна пастка: техніка як привід не прогресувати — стагнація без progressive overload (Issurin, 2010). Schoenfeld (2010) механічна напруга + Kraemer & Ratamess (2004) специфічність + McGill (2007) Low Back Disorders + Schmidt & Lee (2011) моторні програми. clinical-safety: CONDITIONAL PASS; sports-scientist review pending.
- `blog.html` — картка post-2607 додана вгорі стрічки.

### Changed
- `STATUS.md` — Серія 2606–2615 progress 1/10 → 2/10; current version → v9.8.0; next: post-2608 (Авторегуляція без тренера: як читати сигнали тіла і приймати рішення у сесії).
- `VERSION` — 9.7.1 → 9.8.0.
...
### Added
- `content/post-2606-block-retrospective-three-questions.md` — Серія 2606–2615 «Практичний інструментарій self-coached атлета: рішення без тренера» пост 1/10. Ретроспективний аналіз блоку: три запитання, що замінюють тренерський debriefing. Три шари аналізу: (1) compliance — ≥80% як поріг валідності будь-яких висновків (Kraemer et al., 2000 JSCR); (2) числа — різниця між E1RM-трендом, RPE-дрейфом і стагнацією та їх діагностичне значення; (3) відновлення — суб'єктивний маркер функціонального перевантаження на ранньому рівні (Meeusen et al., 2013 ECSS/ACSM). Когнітивний капкан peak-end rule (Kahneman et al., 1993) — чому пам'ять систематично хибно оцінює блок без даних. П'ять сценаріїв (A–D) з рішеннями для кожного. Шаблон ретроспективи на 20 хвилин. clinical-safety: NOT_REQUIRED; sports-scientist review pending.
- `blog.html` — картка post-2606 додана на початок стрічки.
- `README.md` — Editorial series 2606–2615 «Практичний інструментарій self-coached атлета: рішення без тренера» додана до roadmap (10 тем, post-2606 → draft, 2607–2615 → proposed).

### Changed
- `STATUS.md` — Total posts: 2075 → 2076; content engine рядок оновлено: post-2606 draft · Series 2606–2615 1/10 відкрита.
- `VERSION` — 9.6.0 → 9.7.0.
...
### Added
- `content/post-2582-individual-adaptation-variability.md` — Серія 2576–2585 «Тренувальна адаптація без ілюзій» пост 7/10. Індивідуальна варіабельність адаптаційної відповіді: чому однакова програма дає різні результати і що з цим робити без тренера. Hubal et al. (2005): розкид приросту м'язового об'єму -2% → +59% при ідентичному протоколі (n=585). Проблема середнього значення (Atkinson & Batterham, 2015; Monteiro et al., 2023). HERITAGE Family Study: спадковість тренувальної відповіді ~47% для аеробної адаптації (Bouchard et al., 1999, 2011). Генетична архітектура: ACTN3 R577X, IGF-1, миостатин як полігенні фактори. Три кластери при силовому тренуванні: extreme/moderate/non-responders при ідентичному 16-тижневому протоколі (Bamman et al., 2007). Не генетичні фактори варіабельності: хронічне недосипання і синтез м'язового білка (Dattilo et al., 2011; Simpson et al., 2017), хронічний кортизол (Duclos et al., 2003), якість техніки, реальне відновлення між сесіями. «Non-responder» як неправильна доза (Timmons et al., 2010; Ross et al., 2019). П'ятикроковий алгоритм детекції власного адаптаційного профілю. clinical-safety: NOT_REQUIRED; sports-scientist review pending.
- `blog.html` — картка post-2582 додана на початок стрічки.
...
### Added
- `content/post-2581-tendon-connective-tissue-adaptation.md` — Серія 2576–2585 «Тренувальна адаптація без ілюзій» пост 6/10. Адаптація сухожиль і сполучної тканини: чому «відстає» і що з цим робити у програмуванні. Гіповаскулярність і час напіврозпаду колагену І типу (50–100 діб; Magnusson et al., 2010). Жорсткість сухожилля (tendon stiffness) як тренований параметр, незалежна детермінанта сили (Kubo et al., 2001, 2010). Таймфрейм вимірювальних змін: жорсткість — 12–20 тижнів (Wiesinger et al., 2015; Bohm et al., 2015). Мінімальна інтенсивність для сухожильної адаптації ≥55–70% (Arampatzis et al., 2007). Дисбаланс «сильний м'яз — слабке сухожилля» як механізм перевантажувальних травм. Heavy Slow Resistance vs. ексцентрична програма (Kongsgaard et al., 2009). Що гальмує: НПЗП, кортикостероїди, різке збільшення навантаження. П'ять операційних принципів для self-coached атлета. clinical-safety: CONDITIONAL PASS; sports-scientist review pending.
- `blog.html` — картка post-2581 додана на початок стрічки.
...
### Added
- `content/post-2580-strength-vs-hypertrophy-parallel-adaptation.md` — Editorial series 2576–2585 post 5/10. «Сила і м'язова маса: чому два процеси відбуваються паралельно, але не синхронно». PCSA і п'ять змінних-посередників між розміром і силою (нейральна ефективність, специфічність руху, жорсткість сухожилля, тип волокон, архітектура). Таймфрейм-таблиця: нейральна фаза (тижні 1–4) vs. гіпертрофічна (тижні 4–12+) за Moritani & de Vries (1979), Sale (1988), Wernbom et al. (2007), Haun et al. (2019). Сила як рухова навичка: специфічність вправи, частота і нейральна практика (Colquhoun et al., 2018; Schoenfeld et al., 2016). Два disconnect: «маса є — сила відстає» (саркоплазматична гіпертрофія + відсутність нейрального стимулу, Haun et al., 2019) і «сила є — маса незначна» (нейральний механізм домінує при низькоповторній роботі, Balshaw et al., 2017). Блокова структура як синергія — гіпертрофічний блок будує структурний потенціал, силовий навчає ЦНС ним користуватись. Чотири практичних принципи для self-coached атлета. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- Blog card `post-2580` додано до `blog.html` перед `post-2579`.
...
### Added
- `content/post-2579-volume-adaptation-why-what-exhausts-becomes-baseline.md` — Editorial series 2576–2585 post 4/10. «Адаптація до об'єму: чому те, що спочатку виснажує, з часом стає базою». Повторний ефект навантаження (RBE): механічні адаптації цитоскелету (десмін/титін), нейрально-координаційне повторне калібрування, знижений запальний відповідь (Clarkson & Hubal, 2002; McHugh, 2003). MEV/MAV/MRV як змінні станів, а не фізіологічні константи — зростання меж об'єму з тренувальним стажем (Heaselgrave et al., 2019; Krieger, 2010; Schoenfeld et al., 2017). Критичне вікно ризику: м'язи адаптуються за 2–4 тижні, сухожилля — 8–12+ тижнів (Bohm et al., 2015; Wiesinger et al., 2015). Мітохондріальна адаптація (PGC-1α, Hood et al., 2016) і буферна ємність як механізми зниженої метаболічної втоми. Асиметрія деадаптації після паузи (Mujika & Padilla, 2000, 2001). «Об'ємна залежність» — пастка нескінченного нарощування і три сигнали перевищення MRV. Три операційні принципи (MEV-старт після паузи; +10–20%/тиждень; деловий тиждень як механізм). Таймфрейм-таблиця: зниження DOMS, відновлення між сесіями, зсув MEV, сухожильна адаптація, деадаптація. clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- Blog card `post-2579` додано до `blog.html` перед `post-2578`.

### Changed
- `README.md` — post-2578 статус виправлено: `proposed` → `draft · v9.0.0`, опис оновлено до фактично написаного контенту («Гіпертрофія: реальні числа…» замість «Плато як фізіологічна норма»). Post-2579 статус: `proposed` → `draft · v9.1.0`.
- `VERSION` — 9.0.1 → 9.1.0.
...
### Added
- `content/post-2577-neuromuscular-vs-structural-adaptation.md` — Editorial 2576–2585 post 2/10. «Нейром'язова і структурна адаптація: механізми, таймфрейми і наслідки для планування». Нейром'язова адаптація — п'ять механізмів: рекрутування моторних одиниць (Moritani & de Vries, 1979: нейральний механізм домінує на початку тренування), rate coding (Sale, 1988), внутрішньо- і міжм'язова координація, зниження реципрокного інгібування, адаптація органів Гольджі (Aagaard et al., 2002). Структурна адаптація: міофібрилярна гіпертрофія (mTOR-S6K1-4EBP1, Wernbom et al., 2007: суттєва від 6–8 тижнів у нетренованих), саркоплазматична гіпертрофія, адаптації сухожиль (Bohm et al., 2015: до 6 місяців), кісткова адаптація. Асиметричний таймфрейм деадаптації: нейром'язова втрачається за 2 тижні, структурна — значно повільніше; м'язова пам'ять. Мінімальна довжина блоку: 8–10 тижнів (Kraemer & Ratamess, 2004). Нейром'язова ціна ротації вправ. Специфічність адаптації до діапазону руху. Кардіореспіраторна адаптація паралельно. Практична таблиця тижнів 1–12+. Правила мінімального блоку і стабільності вправ. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2577 card added at top of feed.

### Changed
- `README.md` — post-2577 → draft · v8.99.0; Series 2576–2585 progress: 2/10.
- `STATUS.md` — v8.99.0; Series 2576–2585 progress updated to 2/10; next: post-2578 (Гіпертрофія: скільки реально можна набрати за блок і що на це впливає).
- `VERSION` — 8.98.1 → 8.99.0.
...
### Added
- `content/post-2576-what-is-adaptation.md` — Editorial 2576–2585 post 1/10. «Що означає «адаптація» — і чому більшість атлетів плутають ознаку з механізмом». Адаптація — процес, не подія: модель суперкомпенсації як структурна основа. Чому DOMS не є індикатором якості тренування (Schoenfeld & Contreras, 2013: мікроушкодження не є необхідною умовою гіпертрофії). Особистий рекорд як ілюзія: нейром'язова ефективність (гострий ефект, один день) vs. структурна адаптація (хронічний ефект, тижні). Нейром'язова vs. структурна адаптація: різні таймфрейми — нейром'язова за тижні, гіпертрофія суттєва від 6–8 тижнів, ремоделювання сухожиль до 6 місяців (Bohm et al., 2015). Чому зміна програми кожні 4 тижні тримає атлета у постійній фазі нейром'язової адаптації. Конкурентна адаптація (Wilson et al., 2012): mTOR vs AMPK при поєднанні силового тренування і великого об'єму кардіо. Маркери для довіри: тренд e1RM за 6–8 тижнів, RPE при тій самій вазі, відновлення між підходами, технічна якість під втомою. Адаптація як часовий контракт — не нагорода за зусилля. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2576 card added at top of feed.

### Changed
- `README.md` — post-2576 → draft · v8.98.0; Editorial series 2596–2605 (Річна стратегія без самообману: як self-coached атлет будує план на рік, а не на 4 тижні) proposed.
- `STATUS.md` — v8.97.0 → v8.98.0; Series 2576–2585 progress: 1/10; next post-2577.
- `VERSION` — 8.97.0 → 8.98.0.
...
### Added
- `content/post-2575-decision-architecture-synthesis.md` — Block 2566–2575 cluster синтез пост 10/10. «Синтез 2566–2575: архітектура прийняття рішень для самокерованого атлета — п'ятнадцять принципів». П'ятнадцять принципів decision architecture: три рівні рішень (сесія/блок/стратегічний); правило ≠ рекомендація; рішення в реальному часі — спрощені; сигнали зупинки — не слабкість; когнітивні упередження захищаються структурою; критерії виходу з блоку — наперед; мета-рішення важать більше за окремі; профіль того, хто приймає рішення; інструмент підбирається за часовим тиском і ставками; рішення «слідувати плану» — теж рішення; мотивація не є аргументом у тренувальних рішеннях; записане правило сильніше за невербалізоване відчуття; мінімальний трекінг краще максимального що не виконується; система не замінює програму; надмірна інженерія — ознака тривоги. Операційний контрольний список на п'ять кроків. **EDITORIAL SERIES 2566–2575 COMPLETE 10/10.** Наступна серія: Editorial 2576–2585 «Тренувальна адаптація без ілюзій». clinical-safety: NOT_REQUIRED.
- `blog.html` — post-2575 card added at top of feed.

### Changed
- `VERSION` — 8.96.1 → 8.97.0.
- `STATUS.md` — Series 2566–2575 progress updated to 10/10 COMPLETE; next: Editorial 2576–2585 «Тренувальна адаптація без ілюзій», post-2576 (фізіологія адаптації в числах і часових рамках).
...
### Added
- `content/post-2574-tools-for-better-decisions-without-coach.md` — Block 2566–2575 post 9/10. «Інструменти для кращих рішень без тренера: від простих правил до ситуаційних алгоритмів». Три рівні інструментів: Рівень 1 — евристики (Gigerenzen & Gaissmaier, 2011: прості правила з одним-двома критеріями часто перевершують складні алгоритми при дефіциті ресурсів); Рівень 2 — дерева рішень (Helms et al., 2016; Zourdos et al., 2016: RPE/RIR-орієнтоване self-regulation надійніше за жорсткі відсотки; Suchy et al., 2018: конкретні бінарні критерії надійніші за розмиті при фізичній втомі); Рівень 3 — ситуаційні алгоритми з петлею зворотного зв'язку (Ericsson et al., 1993: ключ експерта — механізм корекції). Klein (1998) RPD-model: евристики — це закодований досвід, а не спрощення. Marcora (2009): когнітивна втома підвищує сприймане навантаження — вибір інструменту під реальний когнітивний контекст є компетентністю. Kahneman (2011): система, яка не використовується, не варта нічого. Матриця вибору за часовим тиском і ставками. Пастка надмірної інженерії. Де ці інструменти не замінять тренера (техніка, нетипові симптоми, стратегічне планування). clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — post-2574 card added at top of feed.

### Changed
- `README.md` — post-2574 → draft · v8.96.0; Editorial series 2586–2595 (Тренувальна автономія: алгоритми, що замінюють тренера у рутинних рішеннях) proposed.
- `STATUS.md` — Series 2566–2575 progress updated 8/10 → 9/10; next: post-2575 (Синтез 2566–2575). Current version → v8.96.0.
- `VERSION` — 8.95.0 → 8.96.0.
...
### Added
- `content/post-2573-decision-maker-profile.md` — Block 2566–2575 post 8/10. «Профіль того, хто приймає рішення: як тренувальний вік і стиль мислення формують стратегію». Три виміри профілю: тренувальний вік (Fitts & Posner 1967 три стадії — когнітивна/асоціативна/автономна; Dreyfus & Dreyfus 1986 шкала новачок→експерт; Schön 1983 knowing-in-action), когнітивний стиль (Scott & Bruce 1995 Decision-Making Style — аналітичний vs. інтуїтивний; Kahneman 2011 Система 1/2), профіль ризику (Zourdos et al. 2016 RPE калібровка; ризик-аверсивний — дозвільні ворота; ризик-толерантний — жорсткі ліміти без інтерпретаційного зазору). Адаптація архітектури рішень: новачок — ≤5 правил без вкладених умов; середній — if-then з журналом; просунутий — мета-система з числовими якорями; аналітичний — часовий ліміт між підходами; інтуїтивний — імпульсний шлюз з обов'язковою числовою перевіркою. Ідентифікація через патерн журналу (фактичний vs. плановий RPE, час затримки між підходами, частота відхилень). clinical-safety: NOT_REQUIRED. sports-scientist review pending. Blog card added.
- `blog.html` — post-2573 card added at top of feed.

### Changed
- `STATUS.md` — Series 2566–2575 progress updated 7/10 → 8/10; next: post-2574 (Інструменти для кращих рішень без тренера).
- `VERSION` — 8.94.1 → 8.95.0.
...
### Added
- `content/post-2570-cognitive-biases-training-decisions.md` — Серія 2566–2575 «Прийняття рішень у тренуванні», пост 5/10. П'ять когнітивних упереджень, що систематично ламають тренувальні рішення: (1) підтверджувальне упередження — відбір даних під вже прийняте рішення; (2) евристика доступності — яскраві спогади замість статистичної медіани; (3) sunk cost — факт приїзду в зал як неправомірний аргумент на користь інтенсивності; (4) present bias (Laibson, 1997) — гіперболічне дисконтування майбутнього, симетричне перетренування і уникнення; (5) прив'язка до якоря — PR або попередня сесія як хибний орієнтир. Чому тренування особливо вразливе: фізична втома пригнічує System 2 (Kahneman, 2011). Три конструктивні захисти: число до висновку, рамка блоку замість сесії, попередньо визначені критерії. Tversky & Kahneman (1974), Kahneman (2011), Laibson (1997), Thaler (1980), Marcora (2009). clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2571-block-exit-decision.md` — Серія 2566–2575 «Прийняття рішень у тренуванні», пост 6/10. Рішення про вихід із блоку: коли 4 тижні замало, а 6 — вже забагато. Блок завершений не за часом — за адаптаційним потенціалом. Три маркери: SRA-крива вирівнялась (Zatsiorsky & Kraemer, 2006), технічна стеля досягнута, накопичена втома перевищує адаптаційний виграш. Дві симетричні помилки: ранній вихід (адаптація не стабілізувалась — Issurin, 2010: адаптація на тижнях 3–5) і пізній вихід (Frimel et al., 2005: НМ-адаптація уповільнюється після 6 тижнів, втома зростає). Операційний протокол: δRPE-перевірка щотижня з тижня 3, чотири маркери втоми, матриця рішень 4 сценарії. Deload vs. вихід: deload якщо відновлення проблема; вихід якщо стимул проблема. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — posts 2570–2571 cards added at top of feed.

### Changed
- `README.md` — posts 2570–2571 → draft · v8.93.0.
- `STATUS.md` — серія 2566–2575 progress 6/10; posts 2569, 2570, 2571 entries added.
- `VERSION` — 8.92.1 → 8.93.0.
...
### Added
- `content/post-2569-stop-signals-green-flags-session.md` — Серія 2566–2575 «Прийняття рішень у тренуванні», пост 4/10. Стоп-сигнали і зелені прапори для рішення «зупинити або продовжити сесію». Що не є стоп-сигналом: дискомфорт, небажання, порівняння з піковою сесією. Справжні стоп-сигнали: технічна деградація, що не відновлюється після паузи; δRPE +3 і вище після регулювання ваги; прогресивне погіршення між підходами; RPE першого підходу ≥ 9. Зелені прапори: стабільна або покращувана форма між підходами, δRPE ≤ 0, автоматизована техніка. «Зручна зупинка» як когнітивне упередження: мозок генерує превентивний сигнал «стоп» на основі прогнозованої майбутньої важкості (Noakes, 2012 central governor model). Операційний протокол перевірки сигналу: чотири запитання. Enoka & Duchateau (2008) NM fatigue & coordination; Borg (1998) RPE; Marcora et al. (2009) psychobiological model. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2569 card added at top of feed.

### Changed
- `README.md` — post-2569 → draft · v8.92.0.
- `STATUS.md` — version bump to v8.92.0; серія 2566–2575 progress 4/10.
- `VERSION` — 8.91.0 → 8.92.0.
...
### Added
- `content/post-2567-plan-rule-vs-recommendation.md` — Серія 2566–2575 «Прийняття рішень у тренуванні», пост 2/10. Структурні і параметричні компоненти програми: різниця між тим, що є каркасом адаптаційного сигналу (структурне — тренувальні дні, основні рухи, логіка прогресії, фаза блоку), і тим, що є розрахунком на конкретний день (параметричне — точна вага, обсяг аксесуарів, паузи). Тест для будь-якого елементу: що відбудеться з адаптаційним ефектом, якщо змінити це? Дві помилки: параметричний елемент тримати як структурний (техніка розсипається, але «програма каже 4×8»); структурний змінювати як параметричний (систематичний пропуск основного руху). Issurin (2010) блокова periodization; Haff & Triplett (2015) NSCA essentials. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2568-in-session-load-regulation.md` — Серія 2566–2575 «Прийняття рішень у тренуванні», пост 3/10. Протокол регулювання навантаження на основі δRPE (відхилення отриманого RPE від очікуваного за логом). Перший робочий підхід — діагностичний маркер, не розминка. Три правила: δRPE ±1 — іди за планом; δRPE +2 — знижуй вагу на 5–8%, зберігай обсяг підходів; δRPE +3 і вище або технічна деградація — конвертуй у технічну сесію (60–70% навантаження, RPE ≤7). Розмежування «заслуженої зупинки» (є конкретний δRPE) і «зручної зупинки» (немає числа, тільки відчуття). Помилка ентузіазму: чому легка сесія — не сигнал додавати. Зворотній бік: регулювання обсягу підходів при стабільній вазі. Zourdos et al. (2016) RIR validation; Helms et al. (2016) RPE-based periodization. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — posts 2567–2568 cards added at top of feed.

### Changed
- `README.md` — posts 2567–2568 → draft · v8.91.0.
- `VERSION` — 8.90.1 → 8.91.0.
...
### Added
- `content/post-2532-morning-resting-hr-readiness-signal.md` — Серія 2526–2535 «Сигнали готовності без wearables», пост 7/10. Ранковий пульс спокою як безкоштовний маркер автономного стану. Протокол: лежачи, до підйому, 60 с після 2–3 хв стабілізації. Achten & Jeukendrup (2003): +5–7 уд/хв вище базової лінії — клінічно значущий поріг. Операційні пороги: ±3 норма, +4–6 уважна розминка, +7–9 знижений об'єм, +10+ активний відпочинок. Базова лінія: 14+ послідовних вимірювань. Основні джерела шуму: алкоголь, температура, зневоднення, ситуаційний стрес. Ранковий HR — попередній скринінг, рішення про сесію — після розминки. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2533-weekly-readiness-rhythm.md` — Серія 2526–2535 «Сигнали готовності без wearables», пост 8/10. Тижневий ритм готовності: виявлення особистого паттерну відновлення. Mujika & Padilla (2001): швидкість відновлення між тренованими атлетами суттєво відрізняється при ідентичних стимулах. Три виміри: внутрішньотижневий, внутрішньоблоковий, сезонний. Методологія: 6–8 тижнів трекінгу RHR + RPE₁ + тренд + день тижня + тиждень блоку. Три практичних принципи: важка сесія в дні з вищою готовністю, легкі — у дні з нижчою, зниження на тижні 3 — норма. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `content/post-2534-when-to-override-readiness-signals.md` — Серія 2526–2535 «Сигнали готовності без wearables», пост 9/10. Операційна межа між «долати» і «слухати» сигнали. Marcora & Staiano (2010): суб'єктивна готовність і реальна фізіологічна готовність — незалежні конструкти. Центральний арбітр: тренд розминки. Три категорії: мотиваційний спротив / знижена готовність / конвергентні сигнали. Правило двох маркерів. Пост явно виключає гострі стани. clinical-safety: CONDITIONAL PASS pending review. sports-scientist review pending.
- `content/post-2535-readiness-signals-synthesis.md` — Серія 2526–2535 «Сигнали готовності без wearables», пост 10/10 — СЕРІЯ ЗАВЕРШЕНА. Повна операційна система: п'ятирівневий стек + матриця рішень 7 рядків. Мінімально-достатня версія: рівні 1, 3, 5. Шеститижнева ініціалізація. П'ять типових помилок системи. Наступна серія: Editorial 2536–2545 «Прийняття рішень без тренера». Серія 2526–2535 завершена 9/10 (пост-2530 DOMS — очікує clinical-safety). clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — posts 2532–2535 cards added at top of feed.

### Changed
- `STATUS.md` — v8.89.0; серія 2526–2535 COMPLETE 9/10 (2530 pending); next: Editorial 2536–2545.
- `README.md` — posts 2532–2535 → draft.
- `VERSION` — 8.89.0 → 8.90.0.
...
### Added
- `content/post-2566-three-levels-training-decision.md` — Серія 2566–2575 «Прийняття рішень у тренуванні», пост 1/10. Три рівні інформації для рішення «тут і зараз»: (1) сесійний — RPE першого підходу vs. очікуваний, технічна якість, динамічний тренд в сесії; (2) тижневий — дрейф RPE на однакових навантаженнях за 7–14 днів, технічна деградація під накопиченою втомою, відхилення від кривої прогресу; (3) контекстуальний — якість сну за 3–5 ночей, загальний стрес поза залом, фаза тренувального циклу. Три типові конфігурації (A/B/C) і операційні рішення для кожної. Синтетичний 4-кроковий протокол прийняття рішення. editorial-brutalist, 13-min read. clinical-safety: NOT_REQUIRED. sports-scientist review pending.
- `blog.html` — post-2566 card added at top of feed.
- `README.md` — Editorial series 2566–2575 «Прийняття рішень у тренуванні: архітектура вибору для самокерованого атлета» proposed (10 posts); post-2566 → draft.

### Changed
- `VERSION` — 8.88.0 → 8.89.0.
- `STATUS.md` — version bump; content engine table updated.
