...
│   │
│   │   ── ROADMAP (наступні editorial теми · post-498+) ───────────────
│   ├── post-498: Тренування після 30 — що насправді змінюється, а що ні: тестостерон, відновлення, сухожилля, прогрес ✓ (виконано · v3.63.0 · clinical-safety NOT REQUIRED)
│   ├── post-499: Хронічна перевтома vs детренованість — де межа між «тренуюсь з дефіцитом» і «не тренуюсь взагалі» ✓ (виконано · v3.64.0 · clinical-safety NOT REQUIRED)
│   ├── post-500: Сполучна тканина не встигає за м'язами — і це змінює все у програмуванні ✓ (виконано · v3.64.0 · clinical-safety APPROVED WITH EDITS)
│   ├── post-501: Вибір між програмою і принципами — коли слідувати шаблону, а коли розуміти механіку ✓ (виконано · v3.65.0 · clinical-safety NOT REQUIRED)
│   ├── post-502: Velocity-based training без девайсу — bar speed estimation і практична авторегуляція ✓ (виконано · v3.66.0 · clinical-safety NOT REQUIRED)
│   ├── post-503: Пауза між підходами — що насправді каже наука ✓ (виконано · v3.66.0 · clinical-safety NOT REQUIRED)
│   ├── post-504: Перший рік у залі — що реально відбувається і чому твої очікування хибні ✓ (виконано · v3.67.0 · clinical-safety NOT REQUIRED)
│   │
│   │   ── ROADMAP (наступні editorial теми · post-505+) ───────────────
│   ├── post-505: Другий рік у залі ✓ (v3.68.0)
│   ├── post-506: Адаптаційне відставання ✓ (v3.69.0)
│   ├── post-507: Тренування і кортизол ✓ (v3.70.0)
│   ├── post-508: Самопрограмування з нуля — мета-гайд для людини без тренера ✓ (v3.71.0)
│   │
│   │   ── ROADMAP (наступні editorial теми · post-510+) ───────────────
│   ├── [x] post-509: Сила і серцево-судинне здоров'я — що насправді відбувається з тиском і VO₂max
│   ├── [x] post-510: Сезонне програмування — чому тіло тренується по-різному влітку і взимку ✓ (v3.73.0)
│   ├── [x] post-511: Зворотне планування — чому серйозні аматори починають програму з неправильного кінця ✓ (v3.74.0)
│   ├── [x] post-512: Ізоляційні вправи — коли вони справді потрібні, а коли це театр ✓ (v3.75.0)
│   │
│   │   ── ROADMAP (наступні editorial теми · post-513+) ───────────────
│   ├── [x] post-513: Чому план на 6 тижнів рідко доживає до кінця — і як це виправити ✓ (v3.76.0)
│   ├── [x] post-514: Ексцентрична фаза — чому більшість людей її ігнорують і що вони втрачають ✓ (v3.76.0)
│   ├── [x] post-515: Тренування при хронічному стресі — як коригувати об'єм коли нервова система перевантажена ✓ (v3.76.0) [clinical-safety REVIEWED — PASS]
│   ├── [x] post-516: Суперсети — коли вони прискорюють прогрес, а коли саботують відновлення ✓ (v3.76.0)
│   ├── [x] post-517: Початківець vs проміжний — як зрозуміти що ти перейшов межу і що з цим робити ✓ (v3.76.0)
│   │
│   │   ── ROADMAP (наступні editorial теми · post-518+) ───────────────
│   ├── [x] post-518: Скільки вправ за тренування — де закон убутньої віддачі перестає працювати на вас ✓ (v3.77.0)
│   ├── [x] post-519: Пропущені тренування — одна сесія, тиждень, місяць: що реально втрачається і як повернутись ✓ (v3.79.0)
│   ├── [x] post-520: Тренування двічі на день — коли це має сенс для аматора і коли це самознищення ✓ (v3.79.0)
│   │
...
│   ├── [x] post-525: Резинові стрічки як інструмент, а не замінник штанги — де межа специфічності ✓ (v3.82.0)
│   ├── [x] post-526: ATL:CTL і управління тренувальним навантаженням — чому «більше» не дорівнює «краще» ✓ (v3.83.0)
│   ├── [x] post-527: П'ять ознак того, що ви тренуєтесь недостатньо — MEV-дефіцит і як його виявити ✓ (v3.84.0)
│   ├── [x] post-528: Прогресія ізоляційних вправ — чому «більше ваги» не єдиний варіант ✓ (v3.85.0)
│   ├── [x] post-529: Сила на одну ногу vs дві — коли унілатеральна робота стає основою, а не допоміжною ✓ (v3.85.0)
│   ├── [x] post-530: Форсовані повторення — коли асистована відмова дає більше, а коли зменшує адаптацію ✓ (v3.86.0)
│   ├── [x] post-531: Розминка і перший підхід — нейром'язова підготовка vs «просто підняти легше» ✓ (v3.87.0)
│   ├── [x] post-532: Дропсети без помічника — механіка і коли вони виправдані ✓ (v3.88.0)
│   ├── [x] post-533: Тренування і хронічний біль у спині — що можна, чого не можна, чого уникають дослідження ✓ clinical-safety CONDITIONAL_PASS (v3.89.0)
│   │
...
- [x] post-442: Подвійна прогресія — алгоритм «репи → вага → репи»; Zourdos et al. (2016) RIR-заземлення; rep creep і дрейф RIR; три маркери деловаду; порівняння з лінійною і хвильовою periodизацією; clinical-safety NOT REQUIRED · v3.43.0
- [x] post-443: Силові стандарти і самооцінка прогресу — алометричне масштабування Stone et al. (1991), Vanderburgh & Dooman (2000); Wilks і IPF GL; Swinton et al. (2014); три питання самооцінки (RIR-proximity, volume-at-RPE, техніка); коли стандарти є пасткою; clinical-safety NOT REQUIRED · v3.43.0
- [x] post-444: Суперсети для гіпертрофії — агоністичні vs антагоністичні; Weakley et al. (2017) збереження об'єму при −27% часу; чому агоністичні суперсети знижують якість другої вправи; операційний протокол для 45-хвилинного тренування; Brentano et al. (2017) pre-exhaust не підвищує EMG; Robbins et al. (2010) PAP антагоністичних пар; clinical-safety NOT REQUIRED · v3.44.2
- [x] post-445: Детренованість і повернення до тренувань — Bickel et al. (2011) 1/9 об'єму зберігає адаптації 32 тижні; Issurin (2008) резидуальні ефекти 25–35 днів; чому перший тиждень після перерви ≠ надолуження; матриця рішень за тривалістю перерви; м'язова пам'ять і міонуклеарний домен; clinical-safety NOT REQUIRED · v3.44.4
- [x] post-446: Дроп-сети механіка і застосування — Fink et al. (2018) еквівалентна гіпертрофія при меншому часі; три механізми (метаболічний стрес, Henneman size principle MU recruitment при втомі, клітинне набрякання mTOR); операційний алгоритм 10-20% дропів; ієрархія вправ (машини > ізоляція > компаунд штанга — проблема); clinical-safety NOT REQUIRED · v3.44.4
- [x] post-447: Конкурентне тренування і практик — Wilson et al. (2012) мета-аналіз; interference effect при різних режимах кардіо (біг vs велосипед); AMPK/mTORC1 молекулярний конфлікт; матриця рішень; тижневий план для аматора; clinical-safety NOT REQUIRED · v3.44.4
- [x] post-448: Шум і сигнал у тренувальному прогресі — Sale (1988) інтраіндивідуальна варіабельність сили ±5–10% день-до-дня; Facer-Childs et al. (2018) циркадіанні ефекти; Schoenfeld et al. (2017) гіпертрофічний сигнал потребує 8–12 тижнів; три рівні адаптації (нейром'язова / гіпертрофія / сполучна тканина); 3-тижневий ковзний огляд; recency bias і loss aversion; коли зміна програми виправдана; clinical-safety NOT REQUIRED · v3.44.5
- [x] post-449: Психологія зупинки підходу — Marcora, Staiano & Manning (2009) perceived effort і центральна регуляція; Gandevia (2001) спінальний/суперспінальний компонент центральної втоми; twitch interpolation; Zourdos et al. (2016) систематична недооцінка RIR у кінці підходу; Hatzigeorgiadis et al. (2011) self-talk мета-аналіз 32 дослідження; Allen, Lamb, Westerblad (2008) периферійна втома ATP/H⁺/Pi; RIR-калібрування і матриця добре/погано каліброваного атлета; clinical-safety NOT REQUIRED · v3.45.2
...
- [x] post-460: Програма вичерпала себе — 5 ознак акомодації vs 3 зовнішніх фактори плато; Spiering et al. (2008) нейром'язова акомодація; RPE-крива, тижневий тоннаж і 3RM тест як діагностика; Van Dongen (2003) сон, Morton (2018) білок, Wilson (2012) concurrent training; алгоритм тестового тижня і переходу; clinical-safety NOT REQUIRED · v3.49.1
- [x] post-461: Тренування з партнером vs соло — Zajonc (1965) drive theory; Bond & Titus (1983) мета-аналіз 241 дослідження; Köhler-ефект (Feltz 2011) +24–40% об'єму; три умови коли партнер гальмує прогрес; ефект присутності на RPE; clinical-safety NOT REQUIRED · v3.49.2
- [x] post-462: Ізометричне тренування для силового атлета — де воно справді виправдане: паузи в точках слабкості, тендинопатії, обмеження обладнання; кутова специфічність ISO (Lum & Barbosa 2019); протокол інтеграції в силовий блок; clinical-safety NOT REQUIRED · v3.50.0
- [x] post-463: Як читати наукову статтю за 10 хвилин — 5-фільтрова схема для атлета-самокоуча: sample size, study design, effect size vs p-value, conflict of interest, external validity; чому мета-аналіз ≠ «наука сказала»; clinical-safety NOT REQUIRED · v3.50.0
- [x] post-464: Кінематика присідання — чому «коліна не виходять за носки» неправильний кий; анатомічна варіабельність кульшового суглоба; вимірювані маркери безпечної глибини; як калібрувати під свою морфологію; clinical-safety NOT REQUIRED · v3.50.0
- [x] post-465: Тренування в спеку — терморегуляція, performance decline (Cheung & McLellan 1998), адаптація до теплового стресу; протокол коригування навантаження в літній сезон; heat acclimation як тренувальний інструмент; clinical-safety NOT REQUIRED · v3.50.0
...
- [x] post-470: Ефект Готорна у тренуванні — чому будь-який новий метод «точно працює» перші 4–6 тижнів; observer effect, self-monitoring bias, regression to the mean; як відокремити реальну адаптацію від новизни; clinical-safety NOT REQUIRED · v3.50.3
- [x] post-471: Відсоток vs RPE vs VBT — порівняльна таблиця точності і де кожен метод дає систематичну похибку; клінічна безпека NOT REQUIRED · v3.51.0
...
- [x] post-474: Тренування вдома без обладнання — прогресія, яка справді веде до гіпертрофії; де межа bodyweight і коли потрібне додаткове навантаження; clinical-safety NOT REQUIRED · v3.51.0
- [x] post-475: Ти не є середнє значення — Bouchard HERITAGE Study (1999) JAP: −0.2..+1.5 L/min VO₂max при однаковій програмі; Churchward-Venne et al. (2015) AJP: 20–40% CoV; ACTN3/ACE поліморфізми; n=1 методологія для персональної калібровки; clinical-safety NOT REQUIRED · v3.52.0

    ── ROADMAP (наступні теми · post-476+) ───────────────────────────
- [x] post-476: Темп і час під навантаженням — TUT гіпотеза, Burd et al. (2012), Roig et al. (2009) BJSM eccentric meta, Wilk et al. (2020) JHK; ексцентрика &gt; концентрика, темп як вторинний модулятор. clinical-safety NOT REQUIRED · v3.53.0
- [x] post-477: MEV, MAV, MRV — Schoenfeld et al. (2017) JSCR dose-response, Krieger (2010) set-volume meta; три пороги обсягу; мезоциклова структура; n=1 калібрування. clinical-safety NOT REQUIRED · v3.53.0
...
- [x] post-502: Velocity-based training без девайсу — bar speed estimation і практична авторегуляція. clinical-safety NOT REQUIRED · authored v3.66.0
- [x] post-503: Пауза між підходами — що насправді каже наука. clinical-safety NOT REQUIRED · authored v3.66.0
- [x] post-504: Перший рік у залі — Sale (1988) нейром'язова ефективність, вікно новачка, три помилки, перехід до другого року. clinical-safety NOT REQUIRED · authored v3.67.0

    ── ROADMAP (наступні теми · post-505+) ───────────────────────────
- [x] post-505: Другий рік у залі — нейром'язовий резерв, сповільнення прогресу, нелінійна періодизація. clinical-safety NOT REQUIRED · authored v3.68.0
- [x] post-506: Адаптаційне відставання — чому результати тренування приходять із запізненням і як це використовувати. clinical-safety NOT REQUIRED · authored v3.69.0
- [x] post-507: Тренування і кортизол — коли хронічний стрес зупиняє адаптацію (і коли ні). clinical-safety NOT REQUIRED · authored v3.70.0
- [x] post-508: Самопрограмування з нуля — мета-гайд для людини без тренера. clinical-safety NOT REQUIRED · authored v3.71.0
...
- [x] post-545: Тижневий чекін як мінімальний протокол самоаналізу — 15 хвилин щонеділі; Ericsson et al. (1993) deliberate practice; Zourdos et al. (2016) RPE як тренд; Halson (2014) моніторинг відновлення; Issurin (2010) мезоцикловий огляд; чотири питання + шаблон; мезоцикловий огляд; clinical-safety NOT REQUIRED · authored v3.98.0
- [x] post-546: Перший рік без тренера — що справді вчишся і де більшість помиляється; Fitts & Posner (1967) три стадії моторного навчання; Zourdos et al. (2016) RIR-калібрування; Cheung et al. (2003) DOMS vs справжні сигнали; Kraemer & Ratamess (2004); Wulf & Prinz (2001) зовнішній фокус уваги; алгоритм «правильне важко vs неефективне важко»; clinical-safety NOT REQUIRED · authored v3.98.0
- [x] post-547: Другий рік тренувань — де закінчується логіка початківця; Kraemer & Ratamess (2004) principle of variation і тренувальний статус; Seynnes et al. (2007) ранні структурні адаптації; Israetel et al. (2019) MEV/MAV/MRV зони і зміщення з ростом досвіду; Schoenfeld et al. (2019) хронічний обсяг vs варіації; Issurin (2010) блокова структура; таблиця початківець vs середній; 4 типові застряги другого року; clinical-safety NOT REQUIRED · authored v3.99.0
...
- [x] post-569: Постановка цілей у силовому тренінгу — Locke & Latham (1990) goal-setting theory; process vs outcome goals; proximity effects; чому «хочу бути сильніший» гірше за «+5 кг на присіданні за 8 тижнів»; clinical-safety NOT REQUIRED · authored v4.9.0
...
- [x] post-576: Регуляція зусилля і самоконтроль — Baumeister et al. ego depletion (1998 JPSP); Hagger et al. (2010) meta-analysis d≈0.62; Hagger et al. (2016) multi-lab replication failure d=0.04; Inzlicht & Schmeichel (2012) motivational model; Marcora et al. (2009) JAP mental fatigue + RPE; Job et al. (2010) implicit theory moderator; pre-commitment protocol; clinical-safety NOT REQUIRED · authored v4.12.0
- [x] post-577: Перфекціонізм і тренувальна поведінка — Frost et al. (1990) MPS; Stoeber & Otto (2006) Sport Exerc Psychol Rev адаптивний vs дезадаптивний; Flett & Hewitt (2005); Hall et al. (1998) socially prescribed perfectionism; Gotwals et al. (2012) Sport MPS-2; як perfectionism формує вибір програми та ставлення до пропущених тренувань; clinical-safety NOT REQUIRED · authored v4.13.0
- [x] post-578: Румінація і тренувальна концентрація — Nolen-Hoeksema (1991) RSQ; Nolen-Hoeksema et al. (2008) ARP review; Borkovec et al. (2002) worry vs rumination; Smallwood & Schooler (2006) task-unrelated thought; Hatzigeorgiadis et al. (2011) self-talk meta-analysis d≈0.48; Wulf (2013) external vs internal focus; pre-set routine protocol; clinical-safety NOT REQUIRED · authored v4.13.0
- [x] post-579: Соціальне порівняння у залі — Festinger (1954) social comparison theory; Suls & Wheeler (2000) upward/downward/lateral; Buunk & Gibbons (2007) sport context; White et al. (2009) perceived attainability mediator; Martin Ginis et al. (2008) gym mirrors + attention; journal як internal baseline; clinical-safety NOT REQUIRED · authored v4.13.0
- [x] post-580: Дисоціативна vs асоціативна увага у довгих підходах — Morgan et al. (1987) класичний elite/recreational split; Stevinson & Biddle (1998) перевірка у силовому тренінгу; Brick et al. (2016) чотиривимірна таксономія і тактичне переключення; Wulf (2013) зовнішній фокус vs внутрішній; Terry et al. (2020) музика як дисоціативний модулятор; протокол структурованої уваги per-set; clinical-safety NOT REQUIRED · authored v4.14.0
- [x] post-581: Контрфактуальне мислення і тренувальний прогрес — Roese (1997) Psychol Bull upward vs downward counterfactuals; afective vs preparatory function; Kray et al. (2003) action-oriented vs ruminative; Byrne (2005) counterfactual closure; Ntoumanis et al. (2008) perceived competence як модератор; 3-кроковий алгоритм переробки «якби» у план; clinical-safety NOT REQUIRED · authored v4.16.0
- [x] post-582: Емоційна регуляція і тренувальна поведінка — Gross (1998) process model ER; cognitive reappraisal vs expressive suppression; Gross & John (2003) асиметрія стратегій; Baumeister et al. (1998) ego depletion; Totterdell & Leach (2001) reappraisal/distraction > rumination у спорті; вплив suppression/rumination на інтероцептивну точність і RIR; pre-set ER check; clinical-safety NOT REQUIRED · authored v4.16.0
- [x] post-583: Психологія «доброго» і «поганого» тренування — Ekkekakis et al. (2011) Psychol Bull афективні відповіді нелінійні і не корелюють зі стимулом; Hardy & Rejeski (1989) feeling states vs RPE; Dunlop et al. (2021) peak-end rule у ретроспективній оцінці; три спотворення; 3-питання post-set check; clinical-safety NOT REQUIRED · authored v4.16.0
    ── ROADMAP (наступні теми · post-584+) ───────────────────────────
- [x] post-584: Атлетична ідентичність і тренувальна послідовність — Brewer, Van Raalte & Lerner (1993) AIMS (когнітивний/афективний/поведінковий компоненти); Tajfel & Turner (1979) Social Identity Theory — самокатегоризація як дефолтна поведінка; Anderson & Cychosz (1994) рольова vs атлетична ідентичність; Oyserman et al. (2002) Possible Selves — деталізований образ майбутнього «я» як мотиватор; Grove et al. (1997) over-identification і вразливість до травми; механізм «я є атлет» > щоденне прийняття рішення; clinical-safety NOT REQUIRED · authored v4.17.0
- [x] post-585: Мотиваційний клімат і автономія у тренуванні — Deci & Ryan (1985, 2000) SDT; мотиваційний континуум від зовнішньої до внутрішньої регуляції; три базові потреби (автономія/компетентність/зв'язок); Ntoumanis (2001) autonomy-supportive vs controlling клімат; Mageau & Vallerand (2003) сім поведінок підтримки автономії; Teixeira et al. (2012) IJBNPA 66 досліджень; інтроєктована vs ідентифікована регуляція; Polivy & Herman (1985) what-the-hell effect; Martela & Ryan (2015) shame-based regulation виснаження; clinical-safety NOT REQUIRED · authored v4.18.0
    ── ROADMAP (наступні теми · post-586+) ───────────────────────────
- [ ] post-586: Увага і задавлення під тиском (choking) — Beilock & Carr (2001) JEPG reinvestment theory; explicit monitoring hypothesis; Masters (1992) implicit motor learning; умови де внутрішній фокус погіршує виконання vs де допомагає; клінічні наслідки для ваговіддачі і техніки під PRmax; clinical-safety NOT REQUIRED
- [ ] post-587: Потік (flow) у силовому тренінгу — Csikszentmihalyi (1990) flow model; challenge-skill balance; Jackson & Eklund (2002) Dispositional Flow Scale; умови входу і виходу з flow у тренуванні; практичні маніпуляції для self-coached атлета; clinical-safety NOT REQUIRED
- [ ] post-588: Внутрішній діалог і ефективність тренування — Hardy et al. (2001) функції self-talk (instructional vs motivational); Hatzigeorgiadis et al. (2011) meta-analysis d≈0.48; реструктуризація негативного самодіалогу; протокол pre-set і between-sets self-talk; clinical-safety NOT REQUIRED
- [ ] post-589: Психологія ваги на штанзі — систематичне недо- і перевантаження; fear of failure vs fear of success; Atkinson (1957) achievement motivation; практичний алгоритм калібрування RIR-якоря; clinical-safety NOT REQUIRED
- [ ] post-590: Уявний тренінг (mental rehearsal) і силове тренування — Driskell et al. (1994) meta-analysis; Feltz & Landers (1983); обмеження і реальний потенціал для аматора; clinical-safety NOT REQUIRED
- [ ] post-591: Стрес і тренувальна продуктивність — Lazarus & Folkman (1984) challenge vs threat appraisal; Blascovich & Mendes (2000) CV реактивність; Murphy (1988) competitive anxiety і сила; де стрес допомагає і де шкодить; clinical-safety NOT REQUIRED
- [ ] post-592: Психологія тренувального плато — when to persist vs when to pivot; learned helplessness (Seligman 1975) vs mastery orientation (Dweck); SMART vs GROW goals у plateau contexts; три операційних виходи; clinical-safety NOT REQUIRED
