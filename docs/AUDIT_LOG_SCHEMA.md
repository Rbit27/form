...
- `enterprise.offboard_chain_sla_breach` → AL-OFFL-01 P1 PagerDuty `form-enterprise` → compliance-officer + enterprise-architect; dedup key `offboard-chain-breach-{tenant_id}` 4h cooldown; auto-resolves in PagerDuty when `offboarding_initiated_at` is populated and next hourly job 44 run detects no breach for that tenant. DEC-030 trail continues per run while breach persists — full audit chain preserved without PagerDuty storm. If breach persists beyond working-hours first response, INCIDENT_RESPONSE R-44 (job 44 stale recovery runbook, §53.10 item 6 — to be authored) covers forced manual offboarding initiation.
