...
| 9 | Register all 6 new DEC-030 event types in `docs/AUDIT_LOG_SCHEMA.md` event registry; validate HMAC chain in staging with a test sequence: stream registration → CAEP event received → session revoked → user purged; confirm `sso.caep_user_purged` is emitted before `tenant_users.status` update (ordering constraint §23.8) | platform-engineer + security-engineer | **P0** | M4 | [x] **Done (documentation — 2026-06-20): `docs/AUDIT_LOG_SCHEMA.md §CAEP / SSF` (v2.21) registers all 6 original events + `sso.caep_reregistration_queued` from §36 DEC-072; vocabulary extension `'cache_eviction_failed_on_risc_hijacking'` added to `sso.google_directory_sync_error`; ordering invariant §23.8 and SOC 2 evidence artefacts CC6-E-CAEP-001..004 documented. Implementation (caep-receiver.ts, HMAC chain staging test, `caep_user_purged` ordering) tracked separately.** |
...
#### Cross-reference — Operational Recovery

| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 9 | Cross-reference `docs/INCIDENT_RESPONSE.md R-34` as the operational recovery runbook for `turh_retention_purge` stale condition: when `system.cron_job_stale` fires for `job_name = 'turh_retention_purge'` (job 32, `0 4 * * *`, 26-h freshness window), compliance-officer follows R-34 in full. Two-severity matrix: P1 (stale, R-34-C1 zero rows with `user_id IS NOT NULL` past 7yr — SOC 2 CC6.3 evidence gap only; pseudonymised backlog self-clears on restoration), P0 (stale AND R-34-C1 count > 0 — concurrent Erasure Worker failure; GDPR Art. 5(1)(e) overcollection; Erasure Worker pseudonymisation of affected rows required before job restoration; Art. 33 assessment within 72 h). | compliance-officer | **P1** | M9 | [x] **Done — `docs/OBSERVABILITY.md §12.6` (v4.9.3, 2026-06-21): job 32 stale-consequence cross-ref column updated with `INCIDENT_RESPONSE R-34 (job 32 stale recovery runbook — §R-34.5)`. R-34 is the authoritative recovery runbook; R-34.11 item 4 is the corresponding obligation fulfilled.** |
...
| 6 | Integration tests `src/tests/sso/scim-bulk-deprovision-guard.test.ts`: (a) deprovision count at limit − 1 → 200 OK; (b) deprovision count at limit → 422 `BULK_DEPROVISION_GUARD_TRIGGERED` + `scim.bulk_deprovision_blocked` emitted; (c) threshold = 100 → guard always allows; (d) active override → 200 OK on count at limit + `scim.bulk_deprovision_override_used` emitted on first over-limit request + override columns NULLed; (e) pg_cron job 34 clears expired rows; (f) `getGuardConfig()` cache miss → correct Supabase fallback. | platform-engineer | **P0** | M13 | [ ] |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 11 | After first non-zero GUARD-E-001 export: review IdP configurations for tenant(s) that triggered the guard; determine if threshold adjustment is warranted; if threshold raised for a specific tenant, document rationale in `docs/DECISION_LOG.md`. | customer-success + enterprise-architect | **P2** | First trigger | [ ] |
| 12 | Resolve OQ-R24-02 (Admin Dashboard "undo deprovision" action — P2): evaluate feasibility; if approved, design `bulk_deprovision_undo_eligible_until` column on `tenant_sso_configs` and a re-provision API endpoint. | enterprise-architect + platform-engineer | **P2** | After 3 enterprise customers | [ ] |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 4 | Register `system.scim_guard_cfg_cache_stale` (LOW, 1yr) advisory event in `docs/AUDIT_LOG_SCHEMA.md §System`. Deploy updated event registry to `emit-audit-event` Worker. | compliance-officer + platform-engineer | **P1** | M13 | [x] **Done — 2026-06-20 (SOC2_READINESS §98).** §98.3 registers the CC7.2 monitoring role and stale-advisory cross-check SQL in the GUARD-E-001 quarterly collection procedure; §91.2 extended with contracted-seats assertion SQL and stale-advisory SQL. `docs/AUDIT_LOG_SCHEMA.md §System` physical event registration (name, severity, retention, payload schema) remains pending per SOC2_READINESS §98.5 item 1 (P1/M13). |
| 5 | Author `compliance/fieldwork/scim-bulk-deprovision-guard.md` fieldwork guide: include §35.7 three-step audit traceability reconstruction protocol; include §34.11 GUARD-E-001 collection SQL; cross-reference `compliance/evidence/scim-guard/GUARD-E-001_<YYYY-QN>.csv` storage path. Index in `compliance/evidence/auditor-onboarding/README.md`. | compliance-officer | **P1** | M13 | [x] **Done — 2026-06-20.** `compliance/fieldwork/scim-bulk-deprovision-guard.md` v1.0 authored. Sections: (a) BDG architecture with five DEC-030 events + override protocol; (b) §35.7 three-step reconstruction protocol with `psql` queries; (c) full GUARD-E-001 collection SQL (primary §91.2 + §98.2 contracted-seats assertion + §98.3 stale-advisory); (d) zero-event attestation JSON procedure; (e) privacy floor attestation table. Indexed in `compliance/evidence/auditor-onboarding/README.md` (v1.1). Closes `docs/SOC2_READINESS.md §98.5` item 3. |
...
| 6 | Update MSA CAEP addendum (`docs/MSA_TEMPLATE.md §Addendum 3`) per §36.3.2 Part 1: replace "< 30 seconds" with "near-real-time (< 60 seconds in normal operation)"; add IdP PUSH requirement sentence | compliance-officer + legal | **P1** | M5 | [x] **Done — 2026-06-20.** `docs/MSA_TEMPLATE.md` v0.4 → v0.5: **ADDENDUM 6 — CAEP/SSF Real-Time Session Control SLA Addendum v1.0** added as the correct new addendum (the §36.6 reference to "Addendum 3" was a drafting error — Addendum 3 is EU Data Residency Terms; CAEP SLA becomes Addendum 6). §6.1 baseline vs. CAEP paths; §6.2 "less than 60 seconds in normal operation" (DEC-072 §36.3.2 Part 1 verbatim), PUSH prerequisite + AL-CAEP-03 reference, polling excluded, SLA credit mechanism; §6.3 Customer Obligations (maintain PUSH; notify CSM on PUSH change; confirm `active` stream); §6.4 Evidence (sso.caep_event_received STANDARD/7yr + sso.caep_session_revoked HIGH/7yr; CC6-E-CAEP-002; privacy floor); §6.5 Governing law; §6.6 Outside counsel review (four points; filing path). Order Form: CAEP SLA checkbox added. Exhibits incorporated: Addendum 6 entry added. Internal Notes: Addendum 6 guidance row added. |
...
*v2.8.2 (2026-06-25): Job-number correction and IR cross-reference backfill. (1) `bdg_override_expiry_sweep` job number corrected from "job 33" to **"job 34"** throughout §34.3 (SQL comment and COMMENT ON COLUMN) and §34.11 (checklist items 1 and 6). Rationale: `docs/OBSERVABILITY.md §12.6` v0.4 patch (2026-06-19) is the canonical authority for pg_cron job numbers; that patch assigned job 33 to `evidence_cron_freshness_check` (already canonical) and renumbered `bdg_override_expiry_sweep` to job 34. The OBSERVABILITY v0.4 patch note explicitly flagged: "The in-text job number citations in SSO_SCIM §34.3 and DATA_MODEL §35.4 remain at '33' in their source documents — authors should update those references at next authoring pass." This patch fulfils that obligation for SSO_SCIM §34.3 and §34.11. (2) R-34 recovery runbook cross-reference added to §29.10 as item 9 (P1, M9 — [x] Done: OBSERVABILITY §12.6 v4.9.3 already registers this cross-ref; §29.10 item 9 documents the obligation and its fulfilment). Mirrors the §36.6 item 9 pattern for `caep_reregister_sweep` / R-32 established at v2.8 (2026-06-19). (3) R-33 recovery runbook cross-reference added to §34.11 as item 10 (P1, M13 — [x] Done: OBSERVABILITY §12.6 v0.8 already registers this cross-ref; §34.11 item 10 documents the obligation and its fulfilment). §34.11 P2 items renumbered 10–11 → 11–12 for numbering continuity. Document header: v2.8.1 → v2.8.2. Cross-references: `docs/OBSERVABILITY.md §12.6` (v0.4 canonical job-number registry — job 34 = `bdg_override_expiry_sweep`; v4.9.3 job 32 R-34 cross-ref; v0.8 job 34 R-33 cross-ref); `docs/INCIDENT_RESPONSE.md R-33` (job 34 stale recovery runbook — §R-33.5; §34.11 item 10 [x] Done); `docs/INCIDENT_RESPONSE.md R-34` (job 32 stale recovery runbook — §R-34.5; §29.10 item 9 [x] Done). Owner: compliance-officer.*
