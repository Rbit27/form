...
- **Reverse cost:** Low. If outside counsel rejects Option A: revert to Option B per §45.3.2 scenario 1 (new contract required); remove §45 from COST_MODEL.md; drop migration 0087 columns; remove TU-CHAIN-01 from `emit-audit-event` Worker; close OQ-EXP-03 as 🔴 with Option B mandate. No customer-facing change required (pure upgrade governance is internal).
