...
| SCIM token hash secret (`SCIM_TOKEN_HASH_SECRET`) | 180 days | Scheduled; immediate on suspected compromise | Manual: Cloudflare Workers Secrets re-deployment to `scim-worker`; rotation requires re-hashing all active `tenant_scim_tokens.token_hash` rows during maintenance window (SSO_SCIM_IMPLEMENTATION §27.3; §27.14 item 3) | security-engineer | Cloudflare Workers Secrets deployment audit log; `tenant_scim_tokens` row count before/after re-hash migration |
...
| `HMAC_KDF_SALT` (256-bit HKDF salt for per-tenant audit chain verification key derivation — `HKDF-SHA256(IKM=FORM_AUDIT_HMAC_SECRET, info=tenant_id, salt=HMAC_KDF_SALT, length=32)`) | Annual — coordinate with `FORM_AUDIT_HMAC_SECRET` rotation (same maintenance window; derived per-tenant keys are invalidated on either rotation; enterprise tenants notified ≥ 30 days in advance via Admin Dashboard banner and ENTERPRISE_ONBOARDING §3.3 CSM brief update) | Scheduled; immediate out-of-cycle rotation on confirmed compromise of `FORM_AUDIT_HMAC_SECRET` or `HMAC_KDF_SALT` (both must be rotated together on master-secret compromise — DEC-030 chain records a rotation-boundary key ID) | Manual: generate new 256-bit CSPRNG value; deploy via Cloudflare Workers Secrets to `emit-audit-event` Worker; Admin Dashboard per-tenant key derivation panel regenerates derived keys automatically after deployment; notify all enterprise tenants with active SIEM export via CSM; record key ID boundary event in `audit_log_events` | security-engineer (coordinate with compliance-officer for tenant notification) | Cloudflare Workers Secrets deployment audit log; DEC-030 key-rotation boundary record in `audit_log_events`; enterprise tenant notification records (Admin Dashboard banner + CSM note) |
| `ADOPTION_NOTES_SALT` (256-bit salt for `SHA-256(notes_plaintext \|\| ADOPTION_NOTES_SALT)` pseudonymization in `enterprise_adoption_snapshots.notes_hash`) | Annual — coordinate with `FORM_AUDIT_HMAC_SECRET` annual rotation window; same calendar entry as `HMAC_KDF_SALT`; immediate out-of-cycle rotation on confirmed compromise | Scheduled. Note: rotation breaks linkage between pre- and post-rotation `notes_hash` values — existing `enterprise_adoption_snapshots` rows with prior-salt hashes remain valid SOC 2 evidence records, since plaintext notes are never stored or reversible from either hash; no re-hash migration is required on rotation | Manual: generate new 256-bit CSPRNG value; deploy via Cloudflare Workers Secrets to `evidence-collection-cron` Worker (the sole emitter of `enterprise_adoption_snapshots.notes_hash`); confirm not present in `wrangler.toml` or any config file before deploying; record rotation in 1Password Operations vault | security-engineer + compliance-officer | Cloudflare Workers Secrets deployment audit log; 1Password Operations vault rotation timestamp; SOC2_READINESS §96.6 item 2 |
...
| SCIM token hash secret (`SCIM_TOKEN_HASH_SECRET`) | Cloudflare Workers Secret — `scim-worker` only | security-engineer; `scim-worker` reads at SCIM bearer-token validation time only; never returned in any API response, log, or error trace | No | 1Password Operations vault |
...
| `HMAC_KDF_SALT` (HKDF salt for per-tenant audit chain verification key derivation) | Cloudflare Workers Secret — `emit-audit-event` Worker (primary emitter of DEC-030 events; salt used at per-tenant key derivation display time in Admin Dashboard → Security → Audit Export panel) | security-engineer; per-tenant derived key is computed on demand at display time and never stored server-side after display; never returned in any API response outside the Admin Dashboard display-once panel | No — single-person custody acceptable; salt is not secret in the cryptographic sense (it is non-confidential per HKDF RFC 5869 §3.1) but is treated as a sensitive operational secret to preserve forward secrecy of the per-tenant key derivation | 1Password Operations vault |
| `ADOPTION_NOTES_SALT` (SHA-256 pseudonymization salt for `enterprise_adoption_snapshots.notes_hash`) | Cloudflare Workers Secret — `evidence-collection-cron` Worker only | security-engineer; `evidence-collection-cron` Worker reads at adoption snapshot INSERT time only; never returned in any API response, log, or error trace; compliance-officer confirms deployment for SOC2_READINESS §96.6 item 2 | No | 1Password Operations vault |
...
| v1.2 | 2026-06-14 | security-engineer | `SCIM_TOKEN_HASH_SECRET` registered. Added to §5 Key Rotation Schedule (180-day cycle; rotation requires re-hashing all active `tenant_scim_tokens.token_hash` rows) and §6 Key Custody (`scim-worker` Cloudflare Workers Secret; 1Password Operations vault backup; no dual custody). Closes SSO_SCIM_IMPLEMENTATION §27.14 checklist item 3 (P0 M4). Cross-ref: SSO_SCIM_IMPLEMENTATION §27.3 (auth pipeline — HMAC-SHA256 hash of SCIM bearer token before KV lookup); DATA_MODEL §4.2 (`tenant_scim_tokens.token_hash` column). SOC2_READINESS §56.3 key inventory updated with corresponding row. |
| v1.3 | 2026-06-20 | security-engineer + compliance-officer | `HMAC_KDF_SALT` and `ADOPTION_NOTES_SALT` registered. **`HMAC_KDF_SALT`:** 256-bit HKDF salt for per-tenant audit chain verification key derivation (`HKDF-SHA256(IKM=FORM_AUDIT_HMAC_SECRET, info=tenant_id, salt=HMAC_KDF_SALT, length=32)`); annual rotation coordinated with `FORM_AUDIT_HMAC_SECRET` (same maintenance window; enterprise tenants notified ≥ 30 days in advance; DEC-030 key-rotation boundary record emitted); deployed to `emit-audit-event` Worker; no dual custody (HKDF salt is non-confidential per RFC 5869 §3.1 but treated as sensitive operational secret); 1Password Operations vault backup. Closes `docs/SOC2_READINESS.md §93.5` item 2 (P0/M9 — prerequisite for HMAC-VERIFY-ALGO-001 test vector authoring and Admin Dashboard per-tenant key display) and `docs/OBSERVABILITY.md §50.10` item 3 (P0/M10 — CRYPTOGRAPHY_POLICY §5 registration required before Admin Dashboard key panel). **`ADOPTION_NOTES_SALT`:** 256-bit salt for `SHA-256(notes_plaintext \|\| ADOPTION_NOTES_SALT)` pseudonymization in `enterprise_adoption_snapshots.notes_hash` (DATA_MODEL §41.4); annual rotation in same window as `FORM_AUDIT_HMAC_SECRET` and `HMAC_KDF_SALT`; rotation breaks pre/post-rotation hash linkage but does not invalidate existing evidence rows (plaintext never stored or reversible); deployed to `evidence-collection-cron` Worker; no dual custody; 1Password Operations vault backup. Closes `docs/SOC2_READINESS.md §96.6` item 2 (P0/M10) and `docs/DATA_MODEL.md §41.9` item 1 (P0/M10). Cross-ref: `docs/OBSERVABILITY.md §50` (HMAC chain verification algorithm — `HMAC_KDF_SALT` rotation boundary and test vector context); `docs/AUDIT_LOG_SCHEMA.md §DEC-030` (chain construction — `emit-audit-event` Worker as `HMAC_KDF_SALT` consumer); `docs/DATA_MODEL.md §41.4` (`notes_hash` column definition — `ADOPTION_NOTES_SALT` consumer); `docs/DATA_MODEL.md §41.9` item 1 (P0/M10 registration obligation — 🟢 Closed); `docs/COST_MODEL.md §40.8` (DEC-030 events for `enterprise_adoption_snapshots` — `evidence-collection-cron` Worker as `ADOPTION_NOTES_SALT` consumer); `docs/SOC2_READINESS.md §93.5` item 2 (P0/M9 — `HMAC_KDF_SALT` — 🟢 Closed); `docs/SOC2_READINESS.md §96.6` item 2 (P0/M10 — `ADOPTION_NOTES_SALT` — 🟢 Closed). |

---

v1.3 · 2026-06-20 · Owner: security-engineer + compliance-officer. Reviewed annually per §15 compliance calendar and on any material architecture change.
