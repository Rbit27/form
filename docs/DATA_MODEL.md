...
| ID | Question | Priority | Owner | Resolution path |
|---|---|---|---|---|
| **OQ-ERA-01** | ~~**DSAR requests from users who have already exercised Art. 17 erasure.**~~ **🟢 RESOLVED — ENTERPRISE_ADMIN_API.md §14.1 (2026-06-11).** DSAR Worker returns HTTP 404 with `DSAR_404` error code for any `user_id` where `users.erased_at IS NOT NULL` or row is hard-deleted. 404 (not 410) prevents timing-based enumeration. No DEC-030 event emitted; no DSAR SLA clock starts. Pre-erasure DSAR exports remain accessible via job ID. Soft-delete window (days 1–30) also routes 404 to avoid exporting partially-nullified data. `docs/ENTERPRISE_ADMIN_API.md §14.1` is the canonical Worker routing spec including TypeScript routing logic and SOC 2 P8.0 evidence note. | P2 → ✅ | compliance-officer + platform-engineer | **Resolved** — ENTERPRISE_ADMIN_API.md §14.1 (2026-06-11) |
...
## 33. `tenant_users_role_history` — SCIM Role Change Audit Trail Schema

### 33.1 Purpose & Scope

This section is the canonical DDL definition for `tenant_users_role_history`, created to resolve **OQ-SSO-27.2** (see `docs/SSO_SCIM_IMPLEMENTATION.md §28`, DEC-049). The table records old/new role values for every SCIM `PUT /Users` full-replace and group PATCH operation where the effective role of a `tenant_users` row changes.

**Why a separate table rather than the DEC-030 chain:**

The `scim.user_updated` DEC-030 HMAC-chained event records `fields_changed: ['role']` (attribute name only — no values). Storing `old_role`/`new_role` in the HMAC chain payload would make role assignments visible in any bulk auditor chain export, creating cross-tenant leakage risk for business-sensitive organisational hierarchy data. The `tenant_users_role_history` table is RLS-gated (`compliance_reviewer` and `tenant_admin` read; `form_system` INSERT; no `form_api` access) and cross-referenced to the chain via `scim_request_id`.

**Scope:** Role changes originating from:
- SCIM `PUT /Users` full-replace (§27.5 in SSO_SCIM_IMPLEMENTATION.md)
- SCIM Group PATCH role re-evaluation (§27.6 / §19)
- Admin dashboard role edits (`changed_by = 'admin_ui'`)
- JIT provisioning role assignment (§11, `changed_by = 'jit_provisioning'`)

Not in scope: consumer user permission changes (no enterprise tenant context).
...
-- Resolves: OQ-SSO-27.2 (DEC-049, 2026-06-14)
-- Cross-ref: docs/SSO_SCIM_IMPLEMENTATION.md §28; docs/AUDIT_LOG_SCHEMA.md §6.SCIM-Lifecycle
...
### 33.3 Column Reference

| Column | Type | Nullable | Description |
|---|---|---|---|
| `id` | UUID | NOT NULL | Surrogate PK; generated by `gen_random_uuid()` |
| `tenant_id` | UUID | NOT NULL | FK → `tenants.id` ON DELETE CASCADE; partitions row visibility by tenant |
| `user_id` | UUID | YES | FK → `users.id` ON DELETE SET NULL; NULL after GDPR Art. 17 user delete (step 1) |
| `user_id_pseudonym` | TEXT | YES | `SHA-256(user_id \|\| ERASURE_PSEUDONYM_SALT)` keyed HMAC; populated during Art. 17 erasure Worker; `chk_turh_user_xor_pseudonym` ensures exactly one of `user_id`/`user_id_pseudonym` is non-null |
| `scim_request_id` | TEXT | NOT NULL | Ties this row to the `scim.user_updated` DEC-030 HMAC-chained event; used for SOC 2 auditor cross-reference (§28.3.3 two-step lookup) |
| `changed_by` | TEXT | NOT NULL | Source of the role change: `'scim_sync'` (SCIM Worker), `'admin_ui'` (admin dashboard), `'jit_provisioning'` (§11 JIT flow), `'form_system'` (programmatic internal) |
| `old_role` | TEXT | NOT NULL | Previous effective role value (from §5 role enum: `'admin'`, `'manager'`, `'member'`) |
| `new_role` | TEXT | NOT NULL | New effective role value; `chk_turh_role_changed` enforces `old_role <> new_role` |
| `changed_at` | TIMESTAMPTZ | NOT NULL | UTC timestamp of role change; DEFAULT NOW() set at INSERT |
...
|---|---|---|
| `pk_tenant_users_role_history` | PRIMARY KEY (id) | Surrogate PK; UUID prevents sequential scan attacks |
| `fk_turh_tenant` | FK tenant_id → tenants.id ON DELETE CASCADE | Removes role history when tenant is fully offboarded |
| `fk_turh_user` | FK user_id → users.id ON DELETE SET NULL | Preserves role history rows after user delete (GDPR Art. 17 partial-erasure pattern); `user_id_pseudonym` populated by Erasure Worker before SET NULL takes effect |
| `chk_turh_user_xor_pseudonym` | `(user_id IS NOT NULL) != (user_id_pseudonym IS NOT NULL)` | Enforces exactly one identifier per row — prevents ghost rows with no user reference |
| `chk_turh_role_changed` | `old_role <> new_role` | Rejects no-op inserts; prevents audit noise and storage waste |

---

### 33.5 Indexes

| Index | Columns | Filter | Use case |
|---|---|---|---|
| `idx_turh_tenant_user` | `(tenant_id, user_id)` | `WHERE user_id IS NOT NULL` | Per-user role history queries (admin dashboard, tenant_admin role audit) |
| `idx_turh_tenant_time` | `(tenant_id, changed_at DESC)` | — | SOC 2 observation period queries; time-range audit exports |
| `idx_turh_scim_request` | `(scim_request_id)` | — | DEC-030 chain cross-reference lookup (§28.3.3 step 2) |

---

### 33.6 Row-Level Security

```sql
ALTER TABLE tenant_users_role_history ENABLE ROW LEVEL SECURITY;

-- Compliance reviewer: unrestricted read — SOC 2 auditor fieldwork
...
-- form_system: INSERT only — append-only guarantee; no UPDATE, no DELETE
CREATE POLICY turh_system_insert ON tenant_users_role_history
  FOR INSERT TO form_system WITH CHECK (true);

-- form_api: no policy → no access (Postgres deny-by-default with RLS enabled)
```

**Append-only guarantee:** No UPDATE or DELETE policy is created for any role. The audit trail is tamper-evident at the table level (RLS prevents mutation) and corroborated at the chain level by the corresponding `scim.user_updated` DEC-030 event.

---

### 33.7 GDPR Art. 17 Erasure Path

When a user hard-delete is processed by the Erasure Worker:

1. **Pre-delete pseudonymisation:** Compute `user_id_pseudonym = SHA-256(user_id || ERASURE_PSEUDONYM_SALT)` for all rows where `user_id = :erased_user_id`.
2. **Populate pseudonym:** `UPDATE tenant_users_role_history SET user_id_pseudonym = :pseudonym WHERE user_id = :erased_user_id`.
3. **Trigger FK SET NULL:** The FK `ON DELETE SET NULL` fires on `users` delete, setting `user_id = NULL`.
4. `chk_turh_user_xor_pseudonym` now requires `user_id_pseudonym IS NOT NULL` (since `user_id IS NULL`) — the CHECK passes after step 2.

The resulting row has `user_id = NULL`, `user_id_pseudonym = '<hash>'` — the user's role history is pseudonymised under GDPR Art. 4(5), not deleted. Role transition audit trail is preserved for SOC 2 CC6.3 evidence (role changes existed) without retaining any identifier that can be re-linked to the natural person.

Emit `gdpr.erasure_role_history_pseudonymised` DEC-030 event (STANDARD, 7yr) per tenant after completing the batch UPDATE:

```typescript
await emitAuditEvent(env, {
  action: 'gdpr.erasure_role_history_pseudonymised',
  severity: 'STANDARD',
  tenant_id: tenantId,
  user_id_pseudonym: pseudonym,
  rows_pseudonymised: affectedCount,
  erasure_request_id: dsarId,
});
```
...
### 33.9 SOC 2 Evidence Mapping

| Criterion | Evidence | Control statement |
|---|---|---|
| **CC6.3** — Removes access when appropriate | SCIM-ROLE-E-001 (role history sample) + SCIM-ROLE-E-002 (DEC-030 cross-reference) | Role changes are captured in append-only history; `compliance_reviewer` RLS gate ensures auditor access without bulk export of sensitive values |
| **CC6.6** — Restricts logical access based on role | SCIM-ROLE-E-001 | Role transition audit trail demonstrates SCIM sync operated within authorised role boundaries |
| **PI1.1** — Authorised access to sensitive data | SCIM-ROLE-E-001 + SCIM-ROLE-E-002 | `form_api` has no RLS policy on this table — no API-layer exposure of role assignment history |
...
*v1.0 (2026-06-14): §33 `tenant_users_role_history` — SCIM Role Change Audit Trail Schema. Canonical DDL section added to resolve OQ-SSO-27.2 (DEC-049, 2026-06-14) from `docs/SSO_SCIM_IMPLEMENTATION.md §27.15`. Full schema coverage: DDL (`0068_tenant_users_role_history.sql`); column reference (9 columns with GDPR Art. 17 pseudonymisation path); five constraints (`pk`, `fk_tenant` CASCADE, `fk_user` SET NULL, `chk_user_xor_pseudonym`, `chk_role_changed`); three indexes (tenant_user partial, tenant_time, scim_request); six RLS policies (compliance_reviewer SELECT; tenant_admin own-tenant SELECT; form_system INSERT; form_api zero access); GDPR Art. 17 four-step erasure path with `gdpr.erasure_role_history_pseudonymised` DEC-030 event; retention proposal (7yr, OQ-SSO-28.2 legal sign-off required, M9); SOC 2 CC6.3/CC6.6/PI1.1 evidence mapping (SCIM-ROLE-E-001, SCIM-ROLE-E-002); five-item implementation checklist (2× P0/M5, 2× P1/M9-M10, 1× P2/M10); two open questions (OQ-TURH-01 changed_by ENUM P1, OQ-TURH-02 role column type alignment P1). Cross-references: `docs/SSO_SCIM_IMPLEMENTATION.md §28` (SCIM integration context, DEC-049, recordRoleChange() helper); `docs/SSO_SCIM_IMPLEMENTATION.md §27.15` (OQ-SSO-27.2 — resolved by DEC-049); `docs/AUDIT_LOG_SCHEMA.md §6.SCIM-Lifecycle` (scim.user_updated event — scim_request_id cross-reference anchor); `docs/CRYPTOGRAPHY_POLICY.md §5` (ERASURE_PSEUDONYM_SALT — pseudonymisation key); `docs/DATA_MODEL.md §12` (Art. 17 two-phase erasure — §33 adds role history to the erasure pipeline); `docs/DECISION_LOG.md DEC-049`. Owner: enterprise-architect + security-engineer + platform-engineer + compliance-officer.*
