...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Register `enterprise.mid_contract_termination_risk_flagged`, `enterprise.contract_amended`, and `enterprise.early_termination_fee_waived` in `docs/AUDIT_LOG_SCHEMA.md §6` with Zod schemas from §35.9. Deploy to `emit-audit-event` Worker. | enterprise-architect | **P0** | M10 | [x] Done — `docs/AUDIT_LOG_SCHEMA.md` v1.7 (2026-06-12). `emit-audit-event` Worker deployment is an engineering implementation task. |
| 2 | Add ETF clause to `docs/MSA_TEMPLATE.md`: liquidated damages framing (§35.5.1), declining balance rate schedule (§35.5.3), minimum floor, seat reduction policy (§35.3). Have outside counsel review for enforceability in DE/US jurisdictions. | compliance-officer + outside counsel | **P0** | M10 | [x] **Done (documentation) — 2026-06-12, MSA_TEMPLATE.md v0.2.** §11.4 Early Termination Fee clause added: liquidated damages framing (§8.1), ETF calculation on contracted ACV (§8.2), declining balance rate schedule M1–M36+ (§8.3), minimum floor of one month contracted ACV (§8.4), seat reduction policy ≤10% deferred / >10% triggers ETF on ACV delta (§8.5), waiver authority matrix (§8.6 — founder approval for waivers > USD 5,000; founder + lead investor for waivers > USD 100,000; ev_analysis_completed enforcement closes §35.10 item 4), outside counsel review checkpoint at §8.7 (OQ-ETF-01/OQ-ETF-02; required before first multi-year contract M10), DEC-030 audit record requirement at §8.8. ETF clause renumbered as §8 within §11.4; former §11.4 (Effect of Termination) shifted to §11.5. Cross-ref: MSA_TEMPLATE.md v0.2 (2026-06-12) version note. **Remaining:** outside counsel DE/US jurisdictional enforceability review required per §8.7 before first multi-year contract execution (M10). |
| 3 | Add `enterprise.mid_contract_termination_risk_flagged` to `docs/OBSERVABILITY.md §12.6` pg_cron registry. Implement the weekly CHS < 20 check with 30-day deduplication. PagerDuty P1 alert (CSM + CS lead) on emission. | devops-lead + data-engineer | **P0** | M10 | [x] **Done (documentation) — 2026-06-12, OBSERVABILITY.md §36 v0.1.** pg_cron job 25 `mid_contract_termination_risk_check` registered in §12.6: schedule `0 9 * * 1` (Mondays 09:00 UTC), 8-day freshness window, CC5.2/CC7.2; emits `enterprise.mid_contract_termination_risk_flagged` DEC-030 HIGH 7yr per flagged tenant; 30-day per-tenant dedup via `tenants.mid_contract_risk_alerted_at`; PagerDuty P1 `form-customer-success` → CSM lead + CS lead; Slack `#enterprise-health` HIGH. AL-ETF-01 alert rule defined in OBSERVABILITY §36.3. `INCIDENT_RESPONSE R-29` cross-reference added to §12.6 job 25 entry and §36.5 item 6 — done v4.8.3, 2026-06-21. Cross-ref: OBSERVABILITY §36 v0.1 (2026-06-12); OBSERVABILITY §12.6 v0.7 (job 25 registered). **Remaining:** pg_cron job 25 physical deploy to production (platform-engineer + devops-lead, M10 — engineering implementation task tracked separately in §36.5 items 2/3/4/5). |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Register all four §37.9 DEC-030 events in `docs/AUDIT_LOG_SCHEMA.md §6`. Deploy event types to `emit-audit-event` Worker. | platform-engineer | **P0** | M7 | [x] Done — `docs/AUDIT_LOG_SCHEMA.md` v2.3 (2026-06-13). `emit-audit-event` Worker deployment is an engineering implementation task. |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Register all four §38.8 DEC-030 events in `docs/AUDIT_LOG_SCHEMA.md §6`; deploy event types to `emit-audit-event` Worker. | platform-engineer + compliance-officer | **P0** | M10 | [x] Done — `docs/AUDIT_LOG_SCHEMA.md` v2.7 (2026-06-14). `emit-audit-event` Worker deployment is an engineering implementation task. |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Register all four §39.8 DEC-030 events in `docs/AUDIT_LOG_SCHEMA.md §Enterprise` (`deal_closed_won`, `deal_closed_lost`, `win_loss_analysis_recalibrated`) and `§Privacy` (`no_go_criteria_applied`); deploy event types to `emit-audit-event` Worker; write integration test for WIN-CHAIN-01 warning and PIPE-CHAIN-02 HTTP 422 on missing `decision_log_ref`. | platform-engineer + compliance-officer | **P0** | M9 | [x] Done — `docs/AUDIT_LOG_SCHEMA.md` v2.9 (2026-06-15). Integration tests and `emit-audit-event` Worker deployment are engineering implementation tasks. |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 5 | File WIN-E-001 after first closed-won deal: export `enterprise.deal_closed_won` chain events for the quarter; verify `floor_respected: true` in all records; file at `compliance/evidence/winloss/WIN-E-001_<YYYY-QN>.csv`. | compliance-officer | **P1** | M10 | [x] **Documentation done — `docs/SOC2_READINESS.md §87.2` (v3.12.0, 2026-06-18): artefact definition, collection method, auditor narratives for CC5.2 and CC1.4, and §87.7 implementation checklist item 2 registered. First annual filing gate tracked in §87.7 item 2.** |
| 6 | File WIN-E-003 quarterly from M10 (even if zero no-go events — zero count is itself CC1.4 evidence): export `privacy.no_go_criteria_applied` events for the quarter; file at `compliance/evidence/winloss/WIN-E-003_<YYYY-QN>.csv`. If zero events, file a signed attestation noting `n = 0`. | compliance-officer | **P1** | Quarterly from M10 | [x] **Documentation done — `docs/SOC2_READINESS.md §87.4` (v3.12.0, 2026-06-18): artefact definition, zero-count attestation protocol, auditor narratives for CC1.4 and CC9.2, and §87.7 implementation checklist item 3 registered. First quarterly filing gate (Q3 2026) tracked in §87.7 item 3.** |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Register all four §40.8 DEC-030 events in `docs/AUDIT_LOG_SCHEMA.md §Enterprise`: `enterprise.adoption_snapshot_filed` (STANDARD, 3yr), `enterprise.adoption_milestone_reached` (STANDARD, 3yr), `enterprise.adoption_health_downgraded` (HIGH, 3yr), `enterprise.qbr_completed` (STANDARD, 3yr); also register `system.csm_followup_overdue` (LOW, 1yr) in `§System`. Deploy updated event registry to `emit-audit-event` Worker. Write integration test confirming `qbr_completed` returns HTTP 422 when `privacy_floor_verified` is absent. | platform-engineer + compliance-officer | **P0** | M10 | [x] Done (documentation — AUDIT_LOG_SCHEMA.md v2.18, 2026-06-19); implementation gates: migration 0078 (item 2), evidence-collection-cron extension (item 3), Admin Console modal (item 4), integration test — tracked separately |
...
| 4 | Implement RENEW-NOTICE-01 monitoring cron: extend `evidence-collection-cron` with 95-day lookahead on `enterprise_contracts.renewal_date`; fire PagerDuty P1 `form-enterprise` if no notice event on file; dedup `renew-notice-missing-{tenant_id}` 72h. Compliance-officer receives alert → 5-day SLA to emit notice. | devops-lead | **P0** | M11 | [x] **Done (documentation) — 2026-06-21, OBSERVABILITY.md v4.8.1.** Implementation: pg_cron job 39 `renewal_notice_check` (daily 09:00 UTC, OBSERVABILITY §51.2) — adopted pg_cron + pg_net rather than Cloudflare Worker extension per OBSERVABILITY §51.2.1 decision (table co-location + consistency with jobs 24/25). AL-RENEW-01 PagerDuty `form-enterprise` routed to compliance-officer; dedup `renew-notice-missing-{tenant_id}` 72h; auto-resolve on notice filed; REN-OBS-E-001 quarterly evidence. §6.2 `contract_renewal_health` subsection integrated. Remaining: pg_cron job 39 physical deploy (platform-engineer + devops-lead, M11). |

#### P1 — Before SOC 2 observation period (M13)

| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 5 | File first REN-E-001 annual renewal chain export after first renewal cycle; assert `floor_respected: true` for every row; note `rate_basis` distribution; file at `compliance/evidence/renewals/REN-E-001_<YYYY>.csv`. Register REN-E-001, REN-E-002, REN-E-003 in `docs/SOC2_READINESS.md §79.4` master evidence table. | compliance-officer | **P1** | M13 | [x] **§79.4 registration done — 2026-06-21 (SOC2_READINESS v3.24.2)**: REN-E-001 (CC5.2/CC1.4), REN-E-002 (CC4.1/CC2.2), REN-E-003 (CC4.1/A1.1) rows added; `renewals/` R2 subfolder and Vanta mirror list updated. **Remaining:** first REN-E-001 evidence filing pending first renewal cycle (est. M12). |
...
| 8 | Add price escalation clause to `docs/MSA_TEMPLATE.md §8.6`: "CPI+1%, capped at 5% per year, applied at second and subsequent annual renewals within a multi-year term; 90-day advance written notice required; FORM will provide the calculation methodology and BLS source on request." Outside counsel review before any MSA is signed with an enterprise customer. | compliance-officer + founder | **P1** | M5 (before first MSA signature) | [x] **Done — 2026-06-21.** `docs/MSA_TEMPLATE.md §4.6` v0.6: seven-sub-clause Multi-Year Price Escalation clause added to Article 4 (Order Forms, Fees, and Payment). Formula `min(Prior Rate × (1 + CPI + 0.01), Prior Rate × 1.05)` (CPI-U All Urban Consumers, hard 5% annual cap non-waivable), 90-day written notice requirement, BLS reference month date-only (no URL), escalation transparency right (15-day window), `enterprise.renewal_escalation_calculated` DEC-030 audit record, and §4.6.7 outside counsel checkpoint (OQ-REN-02). §4.5 updated to defer to §4.6 for multi-year cases. |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Register three new §43.7.2 DEC-030 events in `docs/AUDIT_LOG_SCHEMA.md §Enterprise`: `enterprise.winback_initiated` (HIGH, 7yr), `enterprise.winback_converted` (CRITICAL, 7yr), `enterprise.deletion_certificate_issued` (CRITICAL, 7yr). Write WINBACK-CHAIN-01 and DELETION-CHAIN-01 invariant blocks in `emit-audit-event` Worker. Integration test: `enterprise.winback_converted` returns HTTP 422 when no prior `enterprise.winback_initiated` exists within 365 days. | platform-engineer + compliance-officer | **P0** | M10 | [x] **Documentation done — 2026-06-23 (AUDIT_LOG_SCHEMA v2.39)**: All five events registered in new `### Enterprise Post-Churn & Deletion SLA events` section — `enterprise.winback_initiated` (HIGH, 7yr), `enterprise.winback_converted` (CRITICAL, 7yr), `enterprise.deletion_certificate_issued` (CRITICAL, 7yr), plus `enterprise.deletion_sla_warning` (HIGH, 7yr) and `enterprise.deletion_sla_critical` (CRITICAL, 7yr) from §43.10 item 3 remaining obligation. Full Zod v2 schemas, WINBACK-CHAIN-01 and DELETION-CHAIN-01 invariant specs, DELETION-SLA-01 alert routing, SOC 2 auditor narratives (C1.2/CC4.1/CC5.2/CC6.1), and emitter assignments documented. **Remaining (platform-engineer):** implement `emit-audit-event` Worker WINBACK-CHAIN-01 predecessor check (HTTP 422 `WINBACK_CHAIN_01_VIOLATION`), DELETION-CHAIN-01 `days_since_trigger ≤ 35` guard, and integration tests before first enterprise churn event is processed (M10). |
| 2 | Create `docs/DATA_MODEL.md §44` with canonical DDL for migration `0085_enterprise_churn_events.sql`: full EXPLAIN ANALYZE on all three indexes; RLS policy verification (form_api REVOKED, tenant_manager excluded); SOC 2 TSC mapping table; FEHS sourcing from `tenant_engagement_snapshots`. Apply migration to staging; validate `ece_deletion_sla_respected` constraint triggers on day 36 test case. | platform-engineer + enterprise-architect | **P0** | M10 | [x] **Documentation done — 2026-06-23 (DATA_MODEL v1.26)**: `docs/DATA_MODEL.md §44` authored with full DDL, six-item staging checklist (including day 36 ece_deletion_sla_respected violation), EXPLAIN ANALYZE for all three indexes, RLS verification queries, FEHS sourcing query (§44.7), SOC 2 TSC mapping, and chain invariant compliance queries (OFFBOARD-CHAIN-01, WINBACK-CHAIN-01, DELETION-CHAIN-01). **Remaining:** apply migration 0085 to staging, run §44.3.2 validation, apply to production. |
| 3 | Implement DELETION-SLA-01 monitoring as pg_cron **job 43** (`deletion_sla_monitor`, every 6 hours): query `enterprise_churn_events WHERE deletion_requested_at IS NOT NULL AND deletion_completed_at IS NULL AND (now() - deletion_requested_at) > INTERVAL '25 days'`; fire P1 PagerDuty `form-enterprise` at day 25 (5-day warning before SLA); escalate to P0 at day 29 (1 day before GDPR SLA breach). Register job 43 in `docs/OBSERVABILITY.md §12.6` pg_cron registry. | devops-lead + platform-engineer | **P0** | M10 | [x] **Documentation done — 2026-06-23 (OBSERVABILITY v5.0.1, §12.6 v1.4 patch)**: job 43 `deletion_sla_monitor` registered in §12.6 canonical registry. Schedule `0 */6 * * *`; 7h freshness window; P1 at day 25, P0 at day 29; emits `enterprise.deletion_sla_warning` HIGH/7yr and `enterprise.deletion_sla_critical` CRITICAL/7yr; privacy invariant (tenant_id + integer day count + alert_tier only — no individual user data); closes DATA_MODEL §44.11 OBSERVABILITY obligation (🟡 → 🟢). **Remaining:** deploy `deletion_sla_monitor` pg_cron function + pg_net PagerDuty hook to production (platform-engineer). ~~register `enterprise.deletion_sla_warning` and `enterprise.deletion_sla_critical` DEC-030 event types in AUDIT_LOG_SCHEMA.md §Enterprise~~ — 🟢 Done (AUDIT_LOG_SCHEMA v2.39, 2026-06-23): both events registered alongside §43.7.2 events in `### Enterprise Post-Churn & Deletion SLA events` section. |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 5 | Register DEL-E-001, WIN-E-001, CHN-E-001 in `docs/SOC2_READINESS.md §79.4` master evidence table. Add `deletions/`, `winback/`, `offboarding/` R2 subfolders to §80.3. Update Vanta mirror list in §80.4. | compliance-officer | **P1** | M11 | [x] **Documentation done — 2026-06-23 (SOC2_READINESS v3.27.0, §102)**: DEL-E-001 registered as-is (C1.2/CC4.1); WIN-E-001 (winback alias from §43.9) registered as **WBK-E-001** in §79.4 to avoid collision with existing deal-close WIN-E-001 (§87.2 — `winloss/`); CHN-E-001 registered as-is (CC6.1/CC7.1). Three R2 subfolders added to §80.3 (with retroactive `offboarding/` gap closure for OFB-E-005/006). Five artefacts added to §80.4 Vanta mirror list (DEL-E-001/WBK-E-001/CHN-E-001/OFB-E-005/OFB-E-006). **Remaining:** physical R2 folder creation + IAM verification (§102.7 item 1 P1/M11); zero-count CHN-E-001 attestation after first observation quarter (§102.7 item 2 P1). |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Add `contract_discount_type VARCHAR(30) CHECK (contract_discount_type IN ('none','multi_year','upfront','loyalty_reentry'))` to `enterprise_contracts` DDL (migration 0086 additive column); enforce mutual exclusivity constraint: `CHECK (NOT (contract_discount_type = 'loyalty_reentry' AND contract_years > 1 AND contract_discount_type = 'multi_year'))` — simplified as `CHECK (contract_discount_type <> 'loyalty_reentry' OR contract_years = 1)` (loyalty discount Year 1 only, multi-year term may be signed but Year 1 rate is loyalty-discounted; Year 2 escalation applies normally). Add to `DATA_MODEL.md §41.6` as additive column note. | platform-engineer | **P0** | M9 | [x] **Done — DATA_MODEL §45 authored 2026-06-25 (v1.30)**. Migration 0086 DDL canonical registration complete. Note: mutual-exclusivity DDL CHECK omitted (DATA_MODEL §45.1 principle 3 — `billing_period` vs `contract_years` discrepancy + §44.3 multi-year-permitted; REENTRY-CHAIN-01 in Worker is the real gate). §45.3.1 migration script, §45.3.2 six-item staging checklist, §45.4 column semantics, §45.5 RLS (unchanged), §45.6 REENTRY-CHAIN-01 Worker enforcement, §45.7 SOC 2 cross-ref (REN-E-001), §45.8 implementation checklist, §45.9 cross-reference obligations authored. DATA_MODEL.md TOC extended §42–§45. Header corrected v1.28 → v1.30. |
| 2 | Extend `PricingExceptionApprovedPayload` Zod schema in `emit-audit-event` Worker to include `exception_type: z.enum(['standard_discount','loyalty_reentry','pilot_credit','floor_exception'])` (additive field; back-fills `'standard_discount'` for prior events). Register `exception_type` enum extension in `docs/AUDIT_LOG_SCHEMA.md §Enterprise` (`enterprise.pricing_exception_approved` payload section). | platform-engineer + compliance-officer | **P0** | M9 | [x] **Done — AUDIT_LOG_SCHEMA.md v2.45 (2026-06-25)**. `exception_type` (`standard_discount`\|`loyalty_reentry`\|`pilot_credit`\|`floor_exception`) added to `enterprise.pricing_exception_approved` payload table. Required and must equal `loyalty_reentry` when `contract_discount_type = 'loyalty_reentry'` on the associated `enterprise_contracts` row per REENTRY-CHAIN-01 (DEC-081, COST_MODEL §44.5). Optional for pre-DEC-081 events (back-fills to `standard_discount`). Zod Worker implementation remains platform-engineer obligation before first winback outreach (M9). |
...
*v2.14 (2026-06-26): §35.10 checklist sync — items 2 and 3 marked `[x] Done`, reflecting documentation obligations fulfilled by prior authoring sprints. Item 2 (ETF clause to `docs/MSA_TEMPLATE.md`): closed by MSA_TEMPLATE.md v0.2 (2026-06-12), which added §11.4 Early Termination Fee covering §35.5.1 liquidated damages framing, §35.5.3 declining balance rate schedule, §35.3 seat reduction policy, §35.5.5 approval authority, and §8.7 outside counsel review checkpoint; outside counsel enforceability review remains required before first multi-year contract (M10). Item 3 (Add `enterprise.mid_contract_termination_risk_flagged` to `docs/OBSERVABILITY.md §12.6` pg_cron registry): closed by OBSERVABILITY.md §36 v0.1 (2026-06-12), which registered pg_cron job 25 `mid_contract_termination_risk_check` in §12.6 with 8-day freshness window, CC5.2/CC7.2, PagerDuty P1 `form-customer-success`, and Slack `#enterprise-health` HIGH; INCIDENT_RESPONSE R-29 cross-reference added v4.8.3 (2026-06-21); physical deploy of job 25 to production remains pending platform-engineer (M10, tracked in OBSERVABILITY §36.5 items 2–5). No schema, model, or economic changes. Header v2.13 → v2.14. Owner: compliance-officer + enterprise-architect.*
...
*v2.11 (2026-06-21): §42.10 item 4 documentation closure — RENEW-NOTICE-01 monitoring cron. Closes OBSERVABILITY.md §51.7 item 5 (P0/M11). §42.10 item 4 status updated to `[x] Done (documentation)`: OBSERVABILITY.md §51 (v4.8.0, 2026-06-21) registered the RENEW-NOTICE-01 monitoring control as pg_cron job 39 `renewal_notice_check` (daily 09:00 UTC) rather than the originally specified `evidence-collection-cron` Cloudflare Worker extension — implementation decision documented in OBSERVABILITY §51.2.1 (pg_cron + pg_net adopted for table co-location and pattern consistency with jobs 24/25). §42.10 item 4 remaining physical deploy obligation (pg_cron job 39 to production) is clarified as still pending (platform-engineer + devops-lead, M11). No schema, model, or economic changes. Document header v2.10 → v2.11. Cross-references: `docs/OBSERVABILITY.md §51` (v4.8.0 — RENEW-NOTICE-01 pg_cron job 39 full spec); `docs/OBSERVABILITY.md §51.2.1` (implementation decision: pg_cron over Cloudflare Worker); `docs/OBSERVABILITY.md §51.7 item 5` (obligation — now [x] Done). Owner: devops-lead + compliance-officer.*
