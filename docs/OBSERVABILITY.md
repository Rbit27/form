...
| **APIKEY-SLO-01 · API key false-rejection rate** | % of auth requests returning an error for a valid, non-revoked key with correct scope and authorised IP; sourced from `APIKEY_TELEMETRY` where `rejection_reason NOT IN ('scope_insufficient','ip_blocked','key_revoked')` / total requests | ≤ 0.1% false-rejection rate | > 0.1% in 7-day rolling window (AL-APIKEY-01 investigation triggered) | Rolling 7 days | devops-lead |
| **APIKEY-SLO-02 · API key auth P95 latency** | P95 of `api-key-auth.ts` Worker auth path measured via `APIKEY_TELEMETRY.latency_ms` (excludes `waitUntil` last_used_at write) | < 50 ms P95 | > 50 ms P95 in 30-min rolling window | Rolling 7 days | platform-engineer |
| **APIKEY-SLO-03 · API key rotation gap** | 100% of `api_key.rotated` DEC-030 events are followed by `api_key.revoked` for the predecessor `old_key_id` within 26 hours — zero-tolerance; verified daily by pg_cron `api_key_chain_monitor` (job 13) | 100% — zero tolerance; gap = CC6.4 evidence breach + AL-APIKEY-02 | Any open rotation gap > 26 h (APIKEY-CHAIN-02 query non-zero) | Per event | security-engineer |
| **APIKEY-SLO-04 · API key age policy** | Zero tenant API keys with `age > 365 days` and `revoked_at IS NULL` without an active P1/P0 rotation-overdue incident; soft enforcement confirmed per DEC-035 (2026-06-01) | 100% — breach requires post-incident review; if any key reaches 365 d without rotation at first SOC 2 observation period start, hard enforcement ships (see `docs/DECISION_LOG.md` DEC-035) | Key age ≥ 300 days (amber) or ≥ 365 days (red P1 AL-APIKEY-02 re-alert) | Weekly | security-engineer |
| **RL-SLO-01 · Quota middleware latency** | P95 of `quota-check.ts` enforcement path by layer; KV sliding-window vs DB `api_quota_usage` measured separately via `RLIMIT_TELEMETRY.latency_ms` grouped by `violation_layer` | KV path P95 < 5 ms; DB path P95 < 15 ms | > 5 ms (KV) or > 15 ms (DB) sustained 30 min | Rolling 7 days | platform-engineer |
| **RL-SLO-02 · Zero false hard-blocks for contracted enterprise tenants** | `SELECT COUNT(*) FROM api_quota_usage WHERE hard_blocked_at IS NOT NULL AND request_count <= quota_limit AND tenant_id IS NOT NULL` = 0; verified daily by pg_cron 01:00 UTC | 0% — zero false hard-blocks | Any row returned (AL-RL-02 fires; triggers CSM + security-engineer joint investigation) | Monthly (daily verification) | devops-lead |
| **RL-SLO-03 · 80% quota CSM notification latency** | 100% of 80% quota crossings produce a `security.quota_80pct_warning` DEC-030 event within 5 min of the triggering request; measured as `event.occurred_at − triggering_request.timestamp ≤ 5 min` | 100% of threshold crossings within 5 min | Event lag > 5 min per crossing | Per event | devops-lead |
| **RL-SLO-04 · `rate_limit_violations_cleanup` schedule adherence** | pg_cron job 16 executes within ± 90 min of 03:00 UTC daily; verified via `cron.job_run_details` freshness window; stale = retention policy risk + query degradation | Executes within 26 h of prior run | Job stale > 26 h (AL-RL-05) | Daily | devops-lead |
...
**Subsection: `cert_lifecycle` (AL-CERT-01 through AL-CERT-05 — §26.8):**
...
**Subsection: `google_directory_sync` (AL-SSO-GDIR-01 through AL-SSO-GDIR-05 — §26.8):**
...
| Alert Name | Trigger Condition | Severity | Notification Channel | Runbook |
|---|---|---|---|---|
| **Break-glass activated** | `pam.break_glass_activated` DEC-030 event emitted | P0 | PagerDuty + Email Workers; SIEM → `siem.privileged_access_escalated` for `target_tenant_id` | §29.4 AL-PAM-01 |
| **Elevation denied spike** | `pam.elevation_denied` > 3 / same `admin_user_id` / rolling 1 h | P1 | PagerDuty | §29.4 AL-PAM-02 |
| **PAM session clock skew** | PAM session presented with `expires_at` in past | P2 | PagerDuty (escalates to P1 if > 2 / 24 h) | §29.4 AL-PAM-03 |
| **Break-glass review overdue** | `pam_break_glass_reviews.review_due_at < now() AND reviewed_at IS NULL`; pg_cron `pam_bg_review_alert` (daily 08:00 UTC, job 21 in §12.6 registry) | P1 | PagerDuty `form-compliance` → compliance-officer; re-alert every 24 h until `reviewed_at` non-null; dedup key `pam-bg-review-overdue-{pam_session_id}` | §29.4 AL-PAM-BG-01; DATA_MODEL.md §29.10 item 8 |

**Subsection: `crypto_key_health` (AL-KEY-01 through AL-KEY-04 — §30.5):**

| Alert Name | Trigger Condition | Severity | Notification Channel | Runbook |
|---|---|---|---|---|
| **Key rotation imminent** | Any key in `form-crypto-health` KV: `days_until_rotation ≤ 14` (escalates to P0 at ≤ 3) | P1 | PagerDuty → security-engineer + devops-lead; SIEM medium | §30.4 AL-KEY-01; CRYPTOGRAPHY_POLICY.md §5 |
| **Key rotation overdue** | Any key: current date > `next_rotation_due`; re-alerts every 4 h; auto-escalates to founder + compliance-officer if > 7 d | P0 | PagerDuty → security-engineer + founder; SIEM high | §30.4 AL-KEY-02; INCIDENT_RESPONSE.md R-21 |
...
### 12.6 pg_cron health monitoring job registry

The canonical registry of all production pg_cron jobs subject to automated freshness monitoring. The `pg-cron-health-monitor` Supabase Edge Function (schedule: `0 * * * *` — top of every hour) queries `cron.job_run_details` for each job's most recent successful run and compares it against a per-job freshness window. Jobs that exceed their window trigger a `system.cron_job_stale` DEC-030 HIGH event and a PagerDuty alert.

**Design basis:** `docs/SOC2_READINESS.md §71.2.2`. SOC 2 criterion: A1.1 (capacity management — background job operational health). Evidence artefact: PRE-71-E-004 (deployment confirmation) and PRE-71-E-005 (stale-job test).

| Job name | Schedule (UTC) | Freshness window | Compliance relevance | Alert if stale |
|---|---|---|---|---|
| `dsar-sla-day25-alert` | `0 7 * * *` | 26 h | P5.1 GDPR Art. 15 — DSAR day-25 P1 alert clock | PagerDuty P1 → compliance-officer |
...
| `scim_mass_deprovision_check` | `*/5 * * * *` | 6 min | A1.1 / CC7.2 — SCIM mass deprovisioning sentinel; queries `audit_log_events` for `scim.user_deprovisioned` bursts ≥ `MAX(0.10 × tenant.seat_count, 10)` within any rolling 10-min window per tenant; emits `scim.mass_deprovision_detected` DEC-030 CRITICAL; stale = AL-SCIM-MASS-01 detection blind spot — R-24 incidents cannot be auto-detected | PagerDuty P0 `form-customer-success` + `form-platform` dual page; dedup `scim-mass-deprovision-{tenant_id}` 1 h (P0) / 15 min (P1); §26.7a AL-SCIM-MASS-01, job 24 |
| `invite_expiry_sweep` | `30 2 * * *` | 26 h | CC6.3 / P5.1 — marks expired `tenant_invitations` rows `status = 'expired'`; revokes `enterprise_seat_assignments.revoked_at`; emits `tenant.invite_expired` DEC-030 STANDARD 3yr per expired invite (batch, HMAC-chained); stale = seat reservation inflation risk (pending invites count against contracted total) + HMAC chain gap | PagerDuty P1 `form-compliance` → compliance-officer + devops-lead; dedup `invite-expiry-sweep-stale`; DATA_MODEL §27.14 item 8 — **job 10** (renumbered from originally claimed 14 — see §36.3 conflict resolution note) |
| `c1-erasure-sla-monitor` | `0 8 * * *` | 26 h | P5.1 GDPR Art. 17 day-33 SLA warning — fires when any open erasure request in `dsar_requests` (type = `erasure`, status ≠ `fulfilled`/`rejected`) has `submitted_at < now() - INTERVAL '33 days'`; stale = day-33 alert clock blind spot | PagerDuty P1 `form-compliance` → compliance-officer; Slack `#security-alerts` HIGH; re-alerts 24 h; dedup `c1-erasure-sla-breach-{dsar_request_id}`; §6.2 `c1_erasure_sla` subsection AL-C1-01 — **job 11** |
| `invite_email_expiry_cleanup` | `0 4 * * *` | 26 h | P5.2 GDPR Art. 17 retention — nullifies `invited_email → '[erased]'` for expired/revoked `tenant_invitations` rows older than 30 days post-expiry; stale = PII retention breach for expired invite email addresses | PagerDuty P1 `form-compliance` → compliance-officer; dedup `invite-email-cleanup-stale`; DATA_MODEL §27.14 item 9 — **job 12** (renumbered from originally claimed 15 — see §36.3 conflict resolution note) |
...
| `workout_data_purge` | `0 2 * * *` | 26 h | GDPR Art. 5(1)(e) — permanent DELETE of `workout_sets`, `workout_sessions`, `body_metrics` for users with `deleted_at IS NOT NULL AND deleted_at < NOW() - INTERVAL '30 days'`; emits `data.workout_data_purged` DEC-030 STANDARD 1yr (aggregate deleted-user count, not per-row — privacy invariant: no `user_id` in event payload); stale = GDPR Art. 5(1)(e) storage-limitation breach — post-deleted user workout data persists beyond 30-day hold window | PagerDuty P1 `form-devops` → devops-lead; dedup `purge-job-26-stale`; 26h window (daily cadence); cross-ref: §37.7 (DDL + DEC-030 ordering); AL-GDPR-04; SOC 2 evidence GDPR-E-002 — **job 26** |
| `audit_log_retention_purge` | `0 3 1 * *` | 48 h | DEC-030 / CC6.5 / P4.1 — enforces 7-year (2,557-day) maximum retention on `audit_log_events`; HMAC chain integrity verified via `audit-chain-verify` Worker call BEFORE DELETE (self-referential ordering: `data.audit_log_purge_completed` DEC-030 STANDARD emitted and HTTP 200 confirmed BEFORE DELETE executes — chain end-state preserved as new tail entry); stale = `audit_log_events` table grows without bound + 7-year ceiling compliance risk (overcollection) | PagerDuty P1 `form-devops` → devops-lead + `form-compliance` → compliance-officer; dedup `purge-job-27-stale`; 48h window (monthly cadence — consistent with `api_quota_usage_archive` precedent); cross-ref: §37.7 (DDL + DEC-030 ordering invariant); AL-GDPR-05; SOC 2 evidence GDPR-E-003 — **job 27** |
...
**Deployment status:** A1-GAP-004 (🟡 Authored — pending M5 deploy). Instrumentation checklist items: SOC2_READINESS.md §71.8 items 1–6.

---

**v0.3 · June 2026 · Owner: devops-lead**
...
| `checkin.submitted` | Check-in form submitted | `checkin_completed` (boolean: true), `readiness_bucket` (`"low"`\|`"medium"`\|`"high"` — OQ-33 conditional pass) | LOW | YES | F2 completion. `readiness_bucket` **CONDITIONALLY PERMITTED** per OQ-33 ruling 2026-06-12 — see §25.9 for binding conditions (event-property-only; no A/B segmentation; PostHog DPA review required before transmitting). Raw `readiness_score` (1–5) integer: **prohibited**. `mood_score` / `mood_bucket`: **clinical-safety VETO** — permanently prohibited per OQ-33. |
...
| `mood_score` (1–5), `mood_bucket` (low\|medium\|high) | Mental health data | **Permanently prohibited — clinical-safety VETO** per OQ-33 ruling 2026-06-12. GDPR Art. 9 special category: creates longitudinal mental health profile in PostHog person model. Non-overridable absent a new clinical-safety ruling. |
| `readiness_score` (1–5) raw integer | Health-adjacent | **Prohibited** (raw value). `readiness_bucket` bucket form is **conditionally permitted** on `checkin.submitted` event only — see §25.9 OQ-33 Ruling for full binding conditions (event-property scope, PostHog DPA review required, no A/B use). |
...
- `docs/INCIDENT_RESPONSE.md R-24` checklist items 1 and 6: AL-SCIM-MASS-01 (SCIM mass deprovisioning detection) + `scim_mass_deprovision_check` pg_cron job 24 → `OBSERVABILITY §26.7a` and `§12.6`

**Privacy constraint:** all SSO/SCIM observability signals propagate `tenant_id` labels, never individual `user_id` values — except inside DEC-030 HMAC-chained audit events where `user_id` is a required field. Error logs for SSO failures must not include SAML assertion content, attribute values, or response bodies. Certificate metadata (fingerprint, expiry timestamp) is permitted; certificate PEM content is not. Login failure reason codes (e.g. `cert_expired`, `assertion_invalid`) are permitted because they carry no personal data. For mass deprovisioning monitoring, only `tenant_id` + aggregate counts propagate — no individual `user_id` or employee name appears in any alert or dashboard panel (R-24 privacy floor invariant).

**SOC 2 mapping:** CC6.1 (credential lifecycle — cert expiry monitoring), CC6.3 (access revocation timeliness — session revocation SLO), CC7.2 (anomaly monitoring — all alert rule groups), CC7.3 (response to anomalies — runbook cross-references), CC9.2 (vendor/sub-processor risk — Google Directory API conditional sub-processor), A1.1 (availability threat monitoring — AL-SCIM-MASS-01 detects mass access-loss events before manual report), A1.2 (environmental threat detection — `scim_mass_deprovision_check` 5-min cadence; stale job = R-24 blind spot per §12.6).
...
### 26.7a Alert Rules: SCIM Mass Deprovisioning (AL-SCIM-MASS-01)

> Closes `docs/INCIDENT_RESPONSE.md R-24.15` checklist items 1 and 6 (v2.0, 2026-06-11). The pg_cron job `scim_mass_deprovision_check` is the signal source (job 24 in §12.6 registry, every 5 min, 6-min freshness window).

**AL-SCIM-MASS-01 — SCIM Mass Deprovisioning Threshold Breach**

| Attribute | Value |
|---|---|
| **Signal source** | `scim_mass_deprovision_check` pg_cron (job 24, §12.6) — every 5 minutes |
| **Trigger condition** | `scim.user_deprovisioned` DEC-030 CRITICAL event burst: deprovisioned count in any trailing 10-min window ≥ `MAX(0.10 × tenant.seat_count, 10)` for a single `tenant_id` |
| **Severity — P0** | ≥ 50% of tenant's active seat count deprovisioned in < 30 min; OR any deprovisioning that locks out all `tenant_admin` roles (admin lockout override) |
| **Severity — P1** | ≥ 10% but < 50% deprovisioned, OR ≥ 100 users regardless of percentage |
| **Dedup key** | `scim-mass-deprovision-{tenant_id}` — 1-hour window (P0); 15-min window (P1) |
| **PagerDuty routing** | Dual page: `form-customer-success` (CSM lead) + `form-platform` (on-call platform engineer) |
| **Response SLA** | P0: acknowledge < 5 min, suspend SCIM sync (KV key `scim:sync:paused:{tenant_id}`) < 10 min; P1: acknowledge < 15 min, notify tenant admin via Template MD-01 < 10 min |
| **Runbook** | `docs/INCIDENT_RESPONSE.md R-24` |
| **Do NOT downgrade** | From P0 to P1 based on a verbal IT admin assertion — require written confirmation before downgrade |

**pg_cron implementation (job 24, `scim_mass_deprovision_check`):**

```sql
-- Role: form_audit (read-only on audit_log_events + tenants)
-- Schedule: */5 * * * * (every 5 minutes)
-- Freshness window: 6 minutes (§12.6)
SELECT
  ael.tenant_id,
  COUNT(*)                                          AS deprovisioned_in_window,
  t.seat_count,
  ROUND(COUNT(*) * 100.0 / NULLIF(t.seat_count, 0), 1) AS pct_of_seats,
...
WHERE ael.event_type = 'scim.user_deprovisioned'
  AND ael.occurred_at >= NOW() - INTERVAL '10 minutes'
GROUP BY ael.tenant_id, t.seat_count
HAVING COUNT(*) >= GREATEST(0.10 * t.seat_count, 10);
-- Each returned row → emit scim.mass_deprovision_detected DEC-030 CRITICAL (HMAC-chained)
--                    → dual PagerDuty page (P0 if pct_of_seats >= 50 OR admin lockout, else P1)
```

**Alert response flow:**

```
AL-SCIM-MASS-01 fires (PagerDuty dual page)
  ↓
IC opens #inc-YYYYMMDD-scim-mass-<tenant_slug>
  ↓
Run R-24-C1 scope SQL (count, % of seats, 10-min window)
  ↓
P0 if ≥ 50% OR admin lockout → SET scim:sync:paused:{tenant_id} in KV (HTTP 503 on SCIM PUSH)
P1 otherwise → notify tenant admin via Template MD-01 at T+10 min
  ↓
Check R-24-C3 role distribution (detect admin lockout → R-24.4 magic link recovery)
  ↓
Identify root cause H1–H4 per R-24.5 (IdP group deletion / SCIM filter / FORM bug / IdP platform update)
  ↓
Restore per R-24.8; resume SCIM sync (delete KV key); notify via Template MD-03
```

**Privacy floor invariant (R-24):** The alert propagates only `tenant_id` and aggregate deprovisioned counts. No individual `user_id` or employee name appears in any alert payload, incident channel message, or dashboard panel. The scope assessment SQL queries (R-24-C1 through R-24-C4) are run by the IC inside the incident channel only — outputs are not shared more widely.
...
**Subsection: `mass_deprovision` (new subsection in §6.2, insert after `google_directory_sync`):**
...
| AL-PAM-BG-01 | **P1** | `pam_break_glass_reviews.review_due_at < now() AND reviewed_at IS NULL` — any break-glass session whose mandatory post-hoc review has not been completed within the 72-hour SLA window | PagerDuty P1 to `form-compliance` routing key → compliance-officer. Re-alert every 24 h until `reviewed_at IS NOT NULL`. Cross-reference: `pam_break_glass_reviews.github_issue_url` to locate the open review issue. If no review is completed within 96 h total, escalate to security-engineer for break-glass access scope review. Query: `SELECT pam_session_id, review_due_at FROM pam_break_glass_reviews WHERE review_due_at < now() AND reviewed_at IS NULL`. Implemented as pg_cron `pam_bg_review_alert` (daily 08:00 UTC, job 21 in §12.6). | PagerDuty `form-compliance` |
...
- AL-PAM-BG-01: `pam-bg-review-overdue-{pam_session_id}` — fires once per overdue review; re-alerts every 24 h until `reviewed_at IS NOT NULL`; auto-resolves when compliance-officer closes the review.
...
| **APIKEY-SLO-04** | Zero tenant API keys with `age > 365 days` and `revoked_at IS NULL` without an active P1/P0 rotation-overdue incident | 100% — soft enforcement confirmed per DEC-035 (2026-06-01). Each breach requires post-incident review at SOC 2 observation period start; hard enforcement triggers if any key reaches 365 d unrotated (see `docs/DECISION_LOG.md` DEC-035). | Weekly pg_cron query against `tenant_api_keys`; pages security-engineer + customer-success via AL-APIKEY-02 amber alert if any key reaches 300 d; red + P1 at 365 d |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 8 | Register APIKEY-SLO-01 through APIKEY-SLO-04 in §2 SLO table; wire APIKEY-SLO-03 pg_cron daily check (rotation gap ≤ 26 h); confirm APIKEY-SLO-04 weekly query is scheduled. | devops-lead + compliance-officer | **P1** | M5 | [x] **OBSERVABILITY.md v3.0 (2026-06-11)** — APIKEY-SLO-01 through APIKEY-SLO-04 added to §2.1 master SLO table. Engineering: `api_key_chain_monitor` (job 13) wires APIKEY-SLO-03 daily check; APIKEY-SLO-04 weekly query is specified in §31.3 — deploy pending platform-engineer. |
...
| 13 | ~~Resolve OQ-APIKEY-OBS-02 (expires_at hard enforcement decision — closes OQ-SSO-26.2 from `docs/SSO_SCIM_IMPLEMENTATION.md §26.10`); record decision in `docs/DECISION_LOG.md`.~~ **[x] Done — DEC-035 (2026-06-01):** soft enforcement confirmed; `docs/DECISION_LOG.md` DEC-035 is the authoritative record; APIKEY-SLO-04 and OQ-APIKEY-OBS-02 updated to reflect resolution. | enterprise-architect + compliance-officer | ~~**P1**~~ **🟢 Closed** | Done |
...
|---|---|---|---|---|
| **OQ-APIKEY-OBS-01** | ~~**Should per-tenant API key usage metrics be surfaced to tenant admins in the Admin Dashboard beyond `last_used_at`?**~~ **🟢 Resolved — `docs/ENTERPRISE_ADMIN_API.md` v0.2 (2026-06-12).** Two endpoints added: `GET /v1/admin/api-keys/{id}` (§1.9) returns `telemetry_30d` block with `request_count`, `rejection_count`, `last_rejection_reason`, `last_rejection_at`; `GET /v1/admin/api-keys/{id}/usage` (§1.10) returns day/week granularity breakdown with five-reason `rejection_breakdown`. Both scope-guarded via `tenant_id = app.current_tenant_id` in Workers middleware — zero cross-tenant. `client_ip_hash` absent from `APIKEY_TELEMETRY` by design; no DEC-030 event on read. CC6.2 SOC 2 mapping updated with `rejection_breakdown.hmac_mismatch = 0` evidence note. | customer-success + enterprise-architect | 🟢 **Resolved** | **Done — `docs/ENTERPRISE_ADMIN_API.md` v0.2** |
| **OQ-APIKEY-OBS-02** | **🟢 Resolved — DEC-035 (2026-06-01).** Soft enforcement selected: age-alert UI warnings (amber ≥ 365 d, red ≥ 730 d) + CSM escalation. No automatic revocation at `expires_at` timestamp. Provisional: if any key exceeds 365 d without rotation at the first SOC 2 observation period start, the procedural control is demonstrably failing and hard enforcement ships before the observation period ends (pg_cron `api_key_hard_revocation_sweep` setting `revoked_at = now()` for keys at `age > 365d` with `reason = 'age_policy'`). Rationale: enterprise CI/CD pipelines on dynamic-IP runners (GitHub Actions, Buildkite cloud) cannot use static allowlists; hard revocation causes silent production outages for early pilots. Mirror of OQ-SSO-26.2 resolution in `docs/SSO_SCIM_IMPLEMENTATION.md §26.10`. See `docs/DECISION_LOG.md` DEC-035 for full rationale and reverse-cost assessment. | enterprise-architect + compliance-officer | 🟢 **Resolved** | **Done — DEC-035** |

---

*v2.2 (2026-06-12): §31.11 OQ-APIKEY-OBS-01 marked 🟢 Resolved — `docs/ENTERPRISE_ADMIN_API.md` v0.2 adds §1.9 (`GET /v1/admin/api-keys/{id}` with `telemetry_30d` block) and §1.10 (`GET /v1/admin/api-keys/{id}/usage` day/week breakdown + `rejection_breakdown`). Owner: customer-success + enterprise-architect.*
...
*v1.8 (2026-06-04): §6.2 Alert Rules table — five missing subsections added, closing three P0 M4 documentation gaps. `cert_lifecycle` (§26.8 checklist item 6 — closed): five rows AL-CERT-01 through AL-CERT-05 covering SAML cert expiry tiers (t60/t30/t7/expired/cron-failure), SOC 2 CC6.1 evidence; AL-CERT-04 auto-opens INCIDENT_RESPONSE.md R-04. `session_revocation` (§26.8 checklist item 6 — closed): two rows AL-REVOKE-01 (P1, KV sync error > 1% / 5 min, CC7.2/CC7.3 evidence artefact CC6-E-REV-003) and AL-REVOKE-02 (P2, bulk revocation P95 > 5,000 ms / 1 h). `google_directory_sync` (§26.8 checklist item 6 — closed): five rows AL-SSO-GDIR-01 through AL-SSO-GDIR-05; AL-SSO-GDIR-04 cross-references INCIDENT_RESPONSE.md R-12 on unauthorized actor; AL-SSO-GDIR-05 requires pg_cron per §26.7. `pam_session_health` (§29.10 checklist item 4 — closed): three rows AL-PAM-01 (P0, break-glass, SIEM → `siem.privileged_access_escalated`, SOC 2 CC6.1), AL-PAM-02 (P1, denial spike + auto-suspend), AL-PAM-03 (P2, clock skew, escalates to P1 > 2 / 24 h). `crypto_key_health` (§30.10 checklist item 5 — closed): four rows AL-KEY-01 (P1, rotation imminent ≤ 14d, SIEM medium), AL-KEY-02 (P0, rotation overdue, SIEM high, runbook R-21), AL-KEY-03 (P1, HMAC cadence > 395 d), AL-KEY-04 (P0, verification missing within 24 h of rotation, runbook R-21). Total 19 rows added to the canonical §6.2 table. No new alert logic defined — all rows consolidate rules fully specified in §§26–30 of this document. Owner: devops-lead.*

---

## §32 Victor AI Safety Monitoring & Clinical-Safety Observability

**Owner:** devops-lead + security-engineer + clinical-safety
**SOC 2 scope:** CC7.2, CC7.3, CC7.4, CC9.2, A1.2
**Last updated:** 2026-06-10
**Closes:** `docs/INCIDENT_RESPONSE.md` R-23 Implementation Checklist item 7 (P1 M6)

---

### 32.1 Purpose and Scope

This section defines the observability layer for FORM's Victor AI coaching persona specifically in the **safety dimension** — distinct from §22 (AI Coaching Quality Observability), which covers quality proxies such as plan adherence, session abandonment, and A/B test metrics. §32 exists because safety monitoring has different requirements: real-time alerting (not daily aggregation), clinical-safety VETO authority in the response path, zero-tolerance SLOs, and direct linkage to `docs/INCIDENT_RESPONSE.md` R-23.

**Scope:**
- Per-session safety classification signals from `coaching_turns` (structural; no content)
- The `VICTOR_SAFETY_TELEMETRY` Cloudflare Analytics Engine dataset
- The `FORM-VICTOR-001` alert rule and derived rules (`FORM-VICTOR-002` through `FORM-VICTOR-004`)
- Four SLOs (VICTOR-SLO-01 through VICTOR-SLO-04)
- Three DEC-030 chain monitors (VSAFETY-CHAIN-01/02/03)
- "Victor AI Safety Health" Metabase + Better Stack dashboard
- SOC 2 evidence artefacts VSAFETY-E-001 through VSAFETY-E-004

**Out of scope for §32:** Coaching quality proxies and A/B testing (→ §22), CV pose estimation safety (→ §18), enterprise privacy floor monitoring (→ §22.5 signal `FORM-PRIV-004`, R-22).

**Privacy invariant:** `coaching_turns.content` is Confidential (GDPR Art. 9 health-adjacent, `docs/DATA_MODEL.md §5`). No coaching content — not even a fragment — appears in any telemetry signal, dataset field, dashboard panel, or evidence artefact produced by this section. The finest-grain identifier in all §32 observability is `session_id` (UUID pseudonym). PII mapping from UUID to user identity is never compiled into monitoring signals and is accessed only under legal hold by compliance-officer.

**Relationship to §22:** §22.5 defines structural safety signals (session duration spike, retry rate spike, negative feedback rate). §32 goes deeper: it defines the per-category flag taxonomy (VT-01 through VT-10 from R-23), the real-time telemetry dataset, and the chain monitors specifically required for P0 clinical-safety VETO territory. §22 and §32 operate in parallel; §32 does not replace §22.

---

### 32.2 RED Metrics — `VICTOR_SAFETY_TELEMETRY`

Safety metrics are emitted to a dedicated Cloudflare Analytics Engine dataset `VICTOR_SAFETY_TELEMETRY` by the Victor coaching Worker at the moment a safety classification is made. This is separate from `COACHING_TELEMETRY` (§22) to prevent content of quality signals mixing with safety chain-of-custody records.

#### Dataset schema

| Field | Type | Description | Privacy rule |
|---|---|---|---|
| `session_id` | UUID | Coaching session identifier (pseudonymous) | PII floor: UUID only; no user_id mapped in dataset |
| `tenant_id` | UUID \| `null` | Enterprise tenant, or null for consumer | Included for per-tenant safety SLA reporting; see OQ-VSAFETY-01 |
| `trigger_category` | TEXT | VT-01 through VT-10 per R-23.1 trigger matrix | No content; category code only |
| `severity_class` | TEXT | `P0` \| `P1` \| `P2` \| `P3` per R-23.2 | — |
| `detection_source` | TEXT | `automated_flag` \| `user_report` \| `internal_review` \| `jailbreak_test` | — |
| `model_version` | TEXT | Anthropic model ID in use at time of turn (e.g., `claude-sonnet-4-6`) | Vendor management evidence for CC9.2 |
| `prompt_version` | TEXT | Victor prompt version identifier (e.g., `v1.4-safety`) | Regression detection |
| `flags_count` | INTEGER | Number of coaching turns in session carrying this classification | No per-turn content |
| `incident_id` | UUID \| `null` | DEC-030 `ai.safety_incident_opened` event UUID, once opened; null before incident | Linkage for evidence chain |
| `timestamp` | TIMESTAMPTZ | Time of safety classification | — |

**What is NOT in this dataset:** `coaching_turns.content`, user PII (name, email, biometrics), health values, conversation excerpts. These are structurally excluded — the Worker emits only the fields above.

#### Schema extension — `coaching_turns.safety_classification`

The R-23 runbook references `coaching_turn_category` in DEC-030 payloads. To enable the SQL queries in §32.10 and R-23.4, the following nullable column is added to `coaching_turns` (migration `0054_coaching_turns_safety_classification.sql`):

```sql
ALTER TABLE coaching_turns
  ADD COLUMN safety_classification TEXT
    CHECK (safety_classification IN (
      'VT-01','VT-02','VT-03','VT-04','VT-05',
      'VT-06','VT-07','VT-08','VT-09','VT-10',
      'clean'
    ));

COMMENT ON COLUMN coaching_turns.safety_classification IS
  'Populated by Victor coaching Worker only when a safety trigger is detected. '
  'Never null-checked in product UI — classified field for compliance queries only. '
  'Erased per GDPR Art. 17 erasure Worker alongside coaching_turns.content.';
```

**Access:** `form_system` role only (SELECT, UPDATE). `form_api` role may not read this column — enforced by column-level privilege (`REVOKE SELECT (safety_classification) ON coaching_turns FROM form_api`). Tenant admins never see this column (privacy floor, DATA_MODEL §17).

#### RED Metrics Table

| Dimension | Metric name | Source | Description |
|---|---|---|---|
| **Rate** | `victor_safety_flag_rate_per_1k_turns` | `VICTOR_SAFETY_TELEMETRY` | Safety flags per 1,000 coaching turns; 30-minute rolling window; segmented by `trigger_category` and `model_version` |
| **Rate** | `victor_safety_incident_rate_24h` | `VICTOR_SAFETY_TELEMETRY` | Count of distinct `session_id` rows with `severity_class IN ('P0','P1')` per 24-hour window |
| **Rate** | `victor_p0_trigger_rate` | `VICTOR_SAFETY_TELEMETRY` | Count of `trigger_category IN ('VT-03','VT-04','VT-05','VT-06')` per 1-hour window; zero-tolerance |
| **Errors** | `victor_safety_false_positive_rate` | Linear incident review outcome | Fraction of opened incidents later closed as false positive; computed weekly from Linear |
| **Errors** | `victor_safety_regression_rate_by_version` | `VICTOR_SAFETY_TELEMETRY` | Flag rate delta between prompt versions (new version vs. previous version in same 24h window) |
| **Duration** | `victor_safety_mttr_p0` | DEC-030 `ai.safety_incident_opened` → `ai.safety_incident_resolved` | Mean time to resolution for P0 incidents; target ≤ 4 hours |
| **Duration** | `victor_safety_mttr_p1` | DEC-030 event pair | Mean time to resolution for P1 incidents; target ≤ 24 hours |
| **Duration** | `victor_reenable_ramp_duration` | DEC-030 `ai.victor_disabled` → final `ai.victor_reenabled` at 100% ramp | Clock from disable to full re-enable; target ≤ 8 hours for P1; P0 has no target (clinical-safety sign-off gated) |

---

### 32.3 SLOs — Victor Safety

| SLO ID | Metric | Target | Window | Zero-tolerance? | SOC 2 criterion |
|---|---|---|---|---|---|
| **VICTOR-SLO-01** | `victor_p0_trigger_rate` = 0 per production window | Zero P0 clinical-safety incidents (VT-03, VT-04, VT-05, VT-06) per 30-day observation period | 30-day rolling | **Yes — unconditional** | CC7.2, A1.2 |
| **VICTOR-SLO-02** | `FORM-VICTOR-001` fires within 5 minutes of first safety signal at P0 severity | 100% of P0 signals trigger PagerDuty alert within 5 min; measured as: `pagerduty_alert_at - first_flag_timestamp ≤ 5 min` | Per-incident | **Yes** | CC7.2 |
| **VICTOR-SLO-03** | `victor_safety_mttr_p0 ≤ 4 hours` and `victor_safety_mttr_p1 ≤ 24 hours` | P0 incidents resolved (or Victor disabled and ramp-re-enable complete) within 4 h; P1 within 24 h | Per-incident rolling 90 d | P0: zero-tolerance for containment; P1: target | CC7.3, CC7.4 |
| **VICTOR-SLO-04** | Zero `ai.victor_reenabled` events without a preceding `ai.safety_incident_resolved` for the same incident_id | Clinical-safety re-enable sequence integrity: Victor is never re-enabled without a resolved incident record; enforced by VSAFETY-CHAIN-03 chain monitor | Continuous | **Yes — zero-tolerance** | CC7.4 |

**VICTOR-SLO-01 budget:** Because this SLO has zero budget, any P0 clinical-safety trigger immediately constitutes an SLO breach. There is no error budget to burn. The SLO window is reported as "zero incidents" per observation period, not as a percentage.

**VICTOR-SLO-02 measurement:** Measured by comparing the `timestamp` of the first `VICTOR_SAFETY_TELEMETRY` row with `severity_class = 'P0'` for an incident against the PagerDuty `created_at` of the corresponding `FORM-VICTOR-001` P0 alert. Requires that PagerDuty webhook delivers to VICTOR_SAFETY_TELEMETRY pipeline within the same observation window.

---

### 32.4 Alert Rules — `FORM-VICTOR-001` and Derived Rules

**`FORM-VICTOR-001`** is the canonical Victor safety alert defined in `docs/INCIDENT_RESPONSE.md` R-23 Implementation Checklist item 7.

| Alert ID | Trigger condition | Severity | PagerDuty service | Dedup key | Runbook |
|---|---|---|---|---|---|
| **`FORM-VICTOR-001`** | Any `trigger_category IN ('VT-03','VT-04','VT-05','VT-06')` in `VICTOR_SAFETY_TELEMETRY` — no threshold, single event fires immediately | **P0** | `FORM Clinical Safety` | `victor-p0-{session_id}` (no auto-resolve) | R-23 §R-23.3 T+0 → T+3min global disable |
| **`FORM-VICTOR-002`** | `victor_safety_flag_rate_per_1k_turns` for `trigger_category IN ('VT-01','VT-02')` exceeds 3× 30-day rolling baseline in any 30-minute window | **P1** | `FORM Clinical Safety` | `victor-p1-vt01vt02-{prompt_version}-{window_start}` (re-alert 1h) | R-23 §R-23.2 P1 path |
| **`FORM-VICTOR-003`** | `victor_safety_flag_rate_per_1k_turns` for `trigger_category IN ('VT-07','VT-08')` exceeds 5× 30-day rolling baseline in any 1-hour window | **P2** | `FORM Engineering` (Slack `#alerts-safety`) | `victor-p2-quality-{prompt_version}-{window_start}` (re-alert 4h) | R-23 §R-23.2 P2 path — guardrail injection |
| **`FORM-VICTOR-004`** | `victor_safety_flag_rate_per_1k_turns` for `trigger_category IN ('VT-09','VT-10')` exceeds 10× baseline in any 2-hour window OR any jailbreak test suite CI failure for these categories | **P3** | Slack `#alerts-quality` only | `victor-p3-tone-{prompt_version}-{date}` | R-23 §R-23.2 P3 path — next deploy fix |

**Baseline computation:** The 30-day rolling baseline for each `trigger_category` is computed nightly at 01:00 UTC by a pg_cron job `victor_safety_baseline_refresh` querying `VICTOR_SAFETY_TELEMETRY` for the prior 30 days. Baseline is stored in `system.victor_safety_baselines` KV namespace (`vsafety:baseline:{trigger_category}:{date}`). On first deploy with no history, the baseline is zero — `FORM-VICTOR-001` fires on single event (correct: any P0 trigger is an immediate incident regardless of baseline).

**Alert suppression:** None permitted for `FORM-VICTOR-001` (P0, VT-03 through VT-06). `FORM-VICTOR-002` through `FORM-VICTOR-004` are suppressed for the first 60 minutes after a prompt version deploy to allow initial session volume to accumulate (same rationale as §22.6 AL-COACH suppression at 72 hours — safety alerts use a shorter suppression window because the risk of a prompt regression is highest immediately after deploy).

**`FORM-VICTOR-001` PagerDuty configuration:**
- Service: `FORM Clinical Safety`
- Escalation: clinical-safety owner (primary, 0-min delay) + founder (secondary, 0-min delay) + security-engineer (tertiary, 5-min delay)
- Urgency: `high` (regardless of business hours)
- Auto-resolve: **never** (must be manually resolved after R-23 post-incident controls complete)
- Payload inclusion: `session_id`, `trigger_category`, `model_version`, `prompt_version`, `flags_count`, `timestamp` — no coaching content

---

### 32.5 §6.2 Alert Rules Table Addition — `victor_safety_health` Subsection

The following rows are inserted into the canonical §6.2 Alert Rules table after the `api_key_health` subsection:

**Subsection: `victor_safety_health`**

| Alert ID | Trigger | Severity | PagerDuty Service | Dedup Key | SIEM routing |
|---|---|---|---|---|---|
| `FORM-VICTOR-001` | Single P0 safety flag (VT-03/04/05/06) — no threshold; fires on first event | P0 | `FORM Clinical Safety` | `victor-p0-{session_id}` | `siem.victor_safety_p0` (CRITICAL) |
| `FORM-VICTOR-002` | P1 safety flag rate (VT-01/02) > 3× 30d baseline / 30-min window | P1 | `FORM Clinical Safety` | `victor-p1-vt01vt02-{prompt_version}-{window}` | `siem.victor_safety_p1` (HIGH) |
| `FORM-VICTOR-003` | P2 safety flag rate (VT-07/08) > 5× 30d baseline / 1-h window | P2 | `FORM Engineering` | `victor-p2-quality-{prompt_version}-{window}` | None |
| `FORM-VICTOR-004` | P3 flag spike (VT-09/10) > 10× baseline / 2-h window, or CI jailbreak failure | P3 | Slack `#alerts-quality` only | `victor-p3-tone-{prompt_version}-{date}` | None |

**SIEM note:** `siem.victor_safety_p0` and `siem.victor_safety_p1` are routed to the SIEM stream (§27.2) because safety incidents involving enterprise tenant users constitute a reportable event to the affected tenant CSM (see R-23 Template V-03). The SIEM routing also produces the CC7.2 monitoring evidence trail for SOC 2.

---

### 32.6 DEC-030 Chain Monitors

These monitors verify that the five DEC-030 events registered in `docs/INCIDENT_RESPONSE.md` R-23.10 form a legally defensible HMAC-chained sequence. All run as pg_cron jobs in the weekly HMAC batch (Sunday 02:00 UTC) except VSAFETY-CHAIN-03 which runs in real-time as part of the DEC-030 emission pipeline.

| Monitor ID | Condition | Severity | Owner | Resolution |
|---|---|---|---|---|
| **VSAFETY-CHAIN-01** | `ai.safety_incident_opened` event exists with `metadata->>'severity' = 'P0'` AND no `ai.safety_incident_contained` for the same `incident_id` after 60 minutes | P1 | security-engineer + clinical-safety | R-23 timeline may be breached; investigate whether containment step (T+3min global disable) was executed; if Victor is still enabled, execute global disable immediately |
| **VSAFETY-CHAIN-02** | `ai.victor_disabled` event exists AND no `ai.victor_reenabled` for the same `incident_id` after 48 hours | P1 | devops-lead + founder | Victor may be indefinitely disabled; confirm with clinical-safety whether deliberate; emit a `system.monitoring_check_failed` DEC-030 with note; do NOT auto-re-enable |
| **VSAFETY-CHAIN-03** | `ai.victor_reenabled` event emitted AND no `ai.safety_incident_resolved` event with matching `incident_id` exists in the 7-year audit log | **P0** | security-engineer + compliance-officer | Victor re-enabled without incident closure — chain integrity violation; disable Victor immediately; activate R-23; SOC 2 evidence VSAFETY-E-003 is compromised until chain is repaired |

**Chain monitor implementation:**
- VSAFETY-CHAIN-01 and VSAFETY-CHAIN-02: pg_cron `victor_safety_chain_monitor` job (daily 00:30 UTC); results emitted as `admin.monitoring_check_failed` DEC-030 events on non-zero output; add as job 15 in §12.6 pg_cron registry.
- VSAFETY-CHAIN-03: Real-time check in `emit-audit-event` Worker — before writing any `ai.victor_reenabled` event, query `audit_log_events WHERE event_type = 'ai.safety_incident_resolved' AND metadata->>'incident_id' = $incident_id`. If no matching row, reject the re-enable emit with HTTP 422 and page `FORM-VICTOR-001` equivalent alert. This is a write-time invariant, not a post-hoc monitor.

**SQL for VSAFETY-CHAIN-01 and VSAFETY-CHAIN-02 (pg_cron `victor_safety_chain_monitor`):**

```sql
-- VSAFETY-CHAIN-01: P0 incident opened with no containment in >60 minutes
SELECT
...
-- VSAFETY-CHAIN-02: Victor disabled with no re-enable in >48 hours
SELECT
...
### 32.7 Dashboard — "Victor AI Safety Health"

**Dashboard name:** `Victor AI Safety Health`
**Location:** Metabase (real-time panels) + Better Stack (feature flag synthetic)
**Refresh:** Real-time for P0/P1 panels; 15-minute for P2/P3 aggregates; daily for trend panels
**Access:** devops-lead + security-engineer + clinical-safety + founder. **Not available to ml-engineer or tenant_admin** (clinical-safety records require explicit access grant).

| Panel | Source | Chart type | Notes |
|---|---|---|---|
| **Victor feature flag status** | Cloudflare KV `victor_coach_enabled` read via Better Stack synthetic | Green / Amber / Red stat | Amber if feature flag last changed < 2 h ago (may be in ramp); Red if `false`; Green if `true` and last changed > 2 h ago |
| **Safety flags last 24 hours** | `VICTOR_SAFETY_TELEMETRY` | Stacked bar by `trigger_category` (VT-01 through VT-10) | P0 bars (VT-03 through VT-06) highlighted in ember; P1 in acid; P2/P3 in mute |
| **P0 incidents last 90 days** | `VICTOR_SAFETY_TELEMETRY` with `severity_class = 'P0'` | Big number stat | Red background if count > 0 in current observation period |
| **Safety flag rate vs 30-day baseline** | `VICTOR_SAFETY_TELEMETRY` aggregated by `trigger_category` | Line chart (7 d), baseline as dashed reference | Alerts when line exceeds 3× baseline (FORM-VICTOR-002 threshold) |
| **Mean time to containment (MTTC)** | DEC-030 `ai.safety_incident_opened` → `ai.safety_incident_contained` delta | Gauge, 30-day rolling | Target: ≤ 15 min per R-23.3 timeline |
| **Per-model version flag rate** | `VICTOR_SAFETY_TELEMETRY` segmented by `model_version` | Bar chart (last 7 prompt versions) | Regression detection: new version bar should not exceed prior version |
| **DEC-030 chain health** | VSAFETY-CHAIN-01/02/03 last run output | Green / Amber / Red stat with timestamp | Red if any chain monitor fired in last 48 h |
| **Recent safety events log** | `VICTOR_SAFETY_TELEMETRY` latest 50 rows | Sortable table (no content columns) | Columns: timestamp, session_id (UUID), trigger_category, severity_class, detection_source, incident_id |

---

### 32.8 Privacy Constraints

The following invariants apply to all signals, datasets, dashboards, and evidence artefacts produced by §32. They are enforced at the Worker level (emit layer), the database layer (column-level privilege), and the Metabase layer (dashboard field access control).

| Invariant | Enforcement mechanism |
|---|---|
| `coaching_turns.content` never logged | Victor coaching Worker does not emit `content` to `VICTOR_SAFETY_TELEMETRY`; `REVOKE SELECT (content) ON coaching_turns FROM form_api` blocks accidental query exposure |
| Session identified by UUID only | `session_id` (UUID) is the finest grain in all safety telemetry; no `user_id`, name, email, or health value in any §32 signal |
| `safety_classification` column access restricted | `form_api` does not have SELECT on `coaching_turns.safety_classification`; `form_system` only; tenant_admin role has no access path |
| Enterprise tenant `tenant_id` in VICTOR_SAFETY_TELEMETRY | Present for per-tenant safety SLA reporting; never exposed to tenant admins (privacy floor, DATA_MODEL §17.4); internal-only dataset |
| Evidence package PII rule (R-23.11) | IR-V-E-001 and IR-V-E-002 contain UUID only; UUID-to-identity mapping not compiled into evidence; compliance-officer retrieves under legal hold separately |
| No personal health values in any §32 signal | Safety classification signals category of *the Victor output*, not the *user's health state*. A `VT-04` flag means Victor made a medical claim — it does not record the user's health condition that prompted the question |

---

### 32.9 SOC 2 Evidence Artefacts

| Artefact ID | SOC 2 Criterion | Description | Collection method | Retention |
|---|---|---|---|---|
| **VSAFETY-E-001** | **CC7.2** (Monitor system components) | `FORM-VICTOR-001` PagerDuty service configuration screenshot + 90-day incident history export (opened_at, acknowledged_at, resolved_at, alert_key, incident_id) | PagerDuty → Incidents → filter service "FORM Clinical Safety" → export CSV; supplement with Cloudflare Analytics Engine query confirming `VICTOR_SAFETY_TELEMETRY` dataset is live | 7 years |
| **VSAFETY-E-002** | **CC7.3** (Evaluate security events) | `VICTOR_SAFETY_TELEMETRY` export for SOC 2 observation window: `SELECT session_id, trigger_category, severity_class, detection_source, model_version, prompt_version, timestamp FROM victor_safety_telemetry WHERE timestamp BETWEEN $obs_start AND $obs_end ORDER BY timestamp` | Cloudflare Analytics Engine REST API export; store as JSONL | 7 years |
| **VSAFETY-E-003** | **CC7.4** (Respond to security events) | VSAFETY-CHAIN-01/02/03 monitor configuration (pg_cron job DDL + `emit-audit-event` Worker VSAFETY-CHAIN-03 guard code) + attestation that VSAFETY-CHAIN-03 fired zero times in observation period (`SELECT COUNT(*) FROM audit_log_events WHERE event_type = 'ai.victor_reenabled' AND NOT EXISTS (...)` returning zero) | Code review export + SQL query output; store as `compliance/evidence/victor-safety/vsafety-e-003-chain-attestation.json` | 7 years |
| **VSAFETY-E-004** | **A1.2** (Availability monitoring) | Cloudflare KV `victor_coach_enabled` key access log for observation window (timestamp + changed_by_user_id for each change); confirms Victor availability was actively managed and any disable/re-enable had a documented owner | Cloudflare KV Audit Log export; supplement with DEC-030 `ai.victor_disabled` and `ai.victor_reenabled` export | 7 years |

---

### 32.10 Implementation Checklist

#### P0 — Must complete before Victor AI launches in production (M4/M5)

| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 1 | Create `VICTOR_SAFETY_TELEMETRY` Cloudflare Analytics Engine dataset; wire emission in Victor coaching Worker (`src/workers/coaching/victor-coach.ts`) at the point where a safety trigger is detected; confirm no `content` field is emitted; write unit test asserting that content field is never included in the emitted event. | platform-engineer | **P0** | M4 | [ ] |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 13 | Review VICTOR_SAFETY_TELEMETRY baseline thresholds quarterly: if `victor_safety_false_positive_rate` exceeds 20% for any trigger category, re-calibrate the baseline multiplier for FORM-VICTOR-002/003/004. P0 trigger FORM-VICTOR-001 has no baseline — it fires on any single event and requires no calibration. | devops-lead + clinical-safety | **P2** | Quarterly (start M9) | [ ] |

---

### 32.11 Open Questions

| OQ | Question | Priority | Owner | Resolution path |
|---|---|---|---|---|
| **OQ-VSAFETY-01** | **Should `VICTOR_SAFETY_TELEMETRY` include `tenant_id` for enterprise customers, and if so, should per-tenant safety rates be surfaced to CSMs (not tenant admins) for enterprise SLA reporting?** The current spec includes `tenant_id` (nullable) for internal use. Exposing per-tenant safety rates to CSMs could help them proactively manage accounts where a specific enterprise deployment sees elevated safety flags — but this requires defining what CSMs can see without triggering the privacy floor. Recommendation: include `tenant_id` in the internal dataset (already in §32.2 schema); expose to CSMs only as a count (zero / non-zero in last 30 days) via the customer-success dashboard — not the raw flag rates or trigger categories. Decision deferred to after first enterprise customer onboards. | P1 | customer-success + clinical-safety + compliance-officer | Resolve before enterprise GA M13; update `docs/ENTERPRISE_ADMIN_API.md` and `docs/ENTERPRISE_ONBOARDING.md` if CSM exposure is confirmed |
...
| **AL-RL-01b** | **P1** | Enterprise tenant's request count crosses 95% of `quota_limit` for any `endpoint_category` (`QUOTA_SECONDARY_WARN_PCT = 0.95`); sourced from `security.quota_95pct_warning` DEC-030 STANDARD event; deduplicated via `api_quota_usage.secondary_warn_sent_at` — at most one page per tenant per billing period per category; **auto-escalates to P0 if no CSM acknowledgement within 2 hours** | PagerDuty `form-customer-success` → customer-success; 2-hour escalation to `form-customer-success` P0 + `form-devops` if unacknowledged; Slack `#customer-success` | `rl-quota-95pct-{tenant_id}-{period_month}-{endpoint_category}` | `security.quota_95pct_warning` DEC-030; DATA_MODEL §30.3; AUDIT_LOG_SCHEMA.md §billing-gdpr-erasure |
...
The following six rows constitute the `rate_limit_health` subsection and are inserted **after** the `api_key_health` subsection (§31) and **before** the `siem_health` subsection (§34) in the §6.2 Alert Rules table:
...
| **AL-RL-01b** | P1 (auto-escalates to P0 if unacknowledged within 2 h) | Enterprise tenant hits 95% of monthly `quota_limit` for any `endpoint_category`; sourced from `security.quota_95pct_warning` DEC-030 STANDARD event; deduped via `api_quota_usage.secondary_warn_sent_at` | PagerDuty `form-customer-success` → customer-success; escalates to P0 + `form-devops` after 2h with no ack | `rl-quota-95pct-{tenant_id}-{period_month}-{endpoint_category}` | DATA_MODEL §30.3 (OQ-RL-02); AUDIT_LOG_SCHEMA.md; §35.4 |
...
| 8 | Register RL-SLO-01 through RL-SLO-04 in §2 SLO table. Wire RL-SLO-02 verification SQL (§35.2.3) as a pg_cron daily check (01:00 UTC) that fires AL-RL-02 PagerDuty pipeline on any false-hard-block row found (also triggers RL-SLO-02 breach metric in Better Stack). | devops-lead + compliance-officer | **P1** | M5 | [x] **OBSERVABILITY.md v3.0 (2026-06-11)** — RL-SLO-01 through RL-SLO-04 added to §2.1 master SLO table. Engineering: RL-SLO-02 pg_cron daily SQL wiring pending devops-lead + platform-engineer per §35.2.3 spec. |
...
| # | Task | Owner | Priority | Milestone | Status |
|---|---|---|---|---|---|
| 12 | **[Registry gap closure]** Append the six missing §12.6 rows for jobs 10–15. | devops-lead + compliance-officer | **P2** | M6 | [x] **OBSERVABILITY.md v3.0 (2026-06-11)** — Jobs 10–15 appended to §12.6 table. Conflict resolution: `invite_expiry_sweep` → job 10 (was claiming 14; yields to `victor_safety_baseline_refresh`); `invite_email_expiry_cleanup` → job 12 (was claiming 15; yields to `victor_safety_chain_monitor`). §32.6 body error corrected (`victor_safety_chain_monitor` incorrectly stated "job 14" — now "job 15", consistent with §32.10 checklist item 8). DATA_MODEL §27 items 8 and 9 updated to reference jobs 10 and 12. Engineering deploy per individual job owners. |
...
*v3.1 (2026-06-12): OQ-APIKEY-OBS-02 stale-status patch. §31.11 OQ-APIKEY-OBS-02 updated: 🟡 P1 Open → 🟢 Resolved, citing DEC-035 (2026-06-01); soft enforcement (age alerts amber ≥ 365 d, red ≥ 730 d + CSM escalation) confirmed; provisional hard-enforcement trigger documented (any key at 365 d without rotation at SOC 2 observation period start). §31.10 checklist item 13 marked [x] Done with DEC-035 reference and strikethrough. §2.1 APIKEY-SLO-04 master row: "pending OQ-APIKEY-OBS-02" → "confirmed per DEC-035". §31.3 APIKEY-SLO-04 section row: same update. File header: v2.9 → v3.1 (v3.0 header update was omitted in commit v3.73.1). Mirror of OQ-SSO-26.2 resolution already applied to `docs/SSO_SCIM_IMPLEMENTATION.md §26.10` at v1.8.1. Cross-ref: `docs/DECISION_LOG.md` DEC-035 (authoritative record); `docs/SSO_SCIM_IMPLEMENTATION.md §26.10` OQ-SSO-26.2 (sister resolution). Owner: compliance-officer + enterprise-architect.*

*v3.0 (2026-06-11): §2.1 SLO table — APIKEY-SLO-01–04 and RL-SLO-01–04 added; §12.6 pg_cron registry — jobs 10–15 appended with conflict resolution; §32.6 body error corrected. Closes §31.10 item 8 (P1 M5), §35.10 item 8 (P1 M5), §35.10 item 12 (P2 M6 — promoted by numbering conflict). §2.1 additions: APIKEY-SLO-01 (false-rejection rate ≤ 0.1% / 7-day rolling / devops-lead), APIKEY-SLO-02 (auth P95 < 50 ms / 7-day rolling / platform-engineer), APIKEY-SLO-03 (rotation gap ≤ 26 h / zero-tolerance / CC6.4 / security-engineer), APIKEY-SLO-04 (zero keys ≥ 365 d without rotation / soft enforcement pending OQ-APIKEY-OBS-02 / security-engineer); RL-SLO-01 (quota middleware KV P95 < 5 ms / DB P95 < 15 ms / platform-engineer), RL-SLO-02 (zero false hard-blocks for contracted enterprise / daily SQL verification / devops-lead), RL-SLO-03 (80% threshold CSM notification within 5 min / devops-lead), RL-SLO-04 (rate_limit_violations_cleanup schedule adherence ≤ 26 h / devops-lead). §12.6 additions: job 10 `invite_expiry_sweep` (daily 02:30 UTC, 26 h, CC6.3/P5.1 — seat reservation integrity + invite expiry HMAC chain; renumbered from DATA_MODEL §27's claimed 14), job 11 `c1-erasure-sla-monitor` (daily 08:00 UTC, 26 h, P5.1 Art. 17 day-33 SLA clock; number confirmed from §6.2 v2.0), job 12 `invite_email_expiry_cleanup` (daily 04:00 UTC, 26 h, P5.2 Art. 17 PII retention; renumbered from DATA_MODEL §27's claimed 15), job 13 `api_key_chain_monitor` (daily 03:00 UTC, 26 h, CC6.4/CC7.2 rotation gap + bypass detection; number confirmed from §31.6), job 14 `victor_safety_baseline_refresh` (nightly 01:00 UTC, 26 h, CC7.2/A1.2 FORM-VICTOR-002/003/004 threshold baseline; number confirmed from §32.4), job 15 `victor_safety_chain_monitor` (daily 00:30 UTC, 26 h, CC7.4/A1.2 VSAFETY-CHAIN-01/02 integrity; number confirmed from §32.10 checklist item 8). Conflict resolution §36.3: both `invite_expiry_sweep` and `victor_safety_baseline_refresh` claimed job 14 (DATA_MODEL §27 vs §32); both `invite_email_expiry_cleanup` and `victor_safety_chain_monitor` claimed job 15. §32 was authored first and both its job-14 and job-15 claims are consistent with the §32 version note. DATA_MODEL §27 checklist items 8 and 9 updated to reference the resolved job numbers (10 and 12). §32.6 body correction: the body text at §32.6 previously stated "add as job 14 in §12.6 pg_cron registry" for `victor_safety_chain_monitor`; this was an internal inconsistency (§32.4 correctly assigns job 14 to `victor_safety_baseline_refresh`; §32.10 checklist item 8 correctly states "job 15" for `victor_safety_chain_monitor`). Corrected to "job 15". Cross-references: §31.3 (APIKEY-SLO definitions), §35.3 (RL-SLO definitions), §32.4 (job 14 baseline_refresh source), §32.6 (job 15 chain_monitor source), §31.6 (job 13 api_key_chain_monitor source), §6.2 v2.0 (job 11 c1-erasure-sla-monitor source), DATA_MODEL §27 (jobs 10/12 invite source — renumbered). Owner: devops-lead + compliance-officer + enterprise-architect.*
