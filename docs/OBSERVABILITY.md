# FORM · Observability & Monitoring Taxonomy v2.9
...
| **APIKEY-SLO-04 · API key age policy** | Zero tenant API keys with `age > 365 days` and `revoked_at IS NULL` without an active P1/P0 rotation-overdue incident; soft enforcement per OQ-APIKEY-OBS-02 pending SOC 2 observation period outcome | 100% — breach requires post-incident review; hard enforcement decision documented in `docs/DECISION_LOG.md` before SOC 2 observation period | Key age ≥ 300 days (amber) or ≥ 365 days (red P1 AL-APIKEY-02 re-alert) | Weekly | security-engineer |
